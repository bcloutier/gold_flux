...
call print_error_mesg(mod_name,message,FATAL,__FILE__,__LINE__)
#define __NOTE__(message) \
call print_error_mesg(mod_name,message,NOTE,__FILE__,__LINE__)
#define __ASSERT__(x, message) \
if(.NOT.(x))call print_error_mesg(mod_name,message,FATAL,__FILE__,__LINE__)
...
character(len=128), parameter :: version  = '$Id: rivers.F90,v 15.0 2007/08/14 04:00:14 fms Exp $'
...
!   </ERROR>
     ...
     do j = 1, size(moved_to,2)
        do i = 1, size(moved_to,1)
           if(g_moved_to(i,j)/=0) &
              write(stdlog(),"(a,i3,x,i3,a,i3,x,i3,a)")                         &
