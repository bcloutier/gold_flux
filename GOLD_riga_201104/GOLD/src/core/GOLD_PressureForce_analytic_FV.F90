...
type, public :: PressureForce_AFV_CS ; private
  logical :: bulkmixedlayer ! If true, a refined bulk mixed layer is used with
                            ! tv%nk_Rml mixed- and buffer- layers.
...
!   Rml[je+1] and (if tv%eqn_of_state is set) T[ie+1], S[ie+1],
!   T[je+1], and S[je+1].
...
!  real :: oneatm = 101325.0  ! 1 atm in Pa (kg/ms2)
  ...
  do k=1,nz
    ...
    elseif (CS%bulkmixedlayer) then
      if (k<=nkmb) then
        do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
          alpha_anom = 1.0/tv%Rml(i,j,k) - alpha_ref
          dp(i,j) = G%H_to_Pa * h(i,j,k)
          dza(i,j,k) = alpha_anom*dp(i,j)
          intp_dza(i,j,k) = 0.5*alpha_anom*dp(i,j)**2
        enddo ; enddo
        do j=js,je ; do I=Isq,Ieq
          aaL = 1.0/tv%Rml(i,j,k) - alpha_ref
          aaR = 1.0/tv%Rml(i+1,j,k) - alpha_ref
          intx_dza(i,j,k) = C1_6 * (dp(i,j)*(2.0*aaL + aaR) + &
                                  dp(i+1,j)*(2.0*aaR + aaL))
        enddo ; enddo
        do J=Jsq,Jeq ; do i=is,ie
          aaL = 1.0/tv%Rml(i,j,k) - alpha_ref
          aaR = 1.0/tv%Rml(i,j+1,k) - alpha_ref
          inty_dza(i,j,k) = C1_6 * (dp(i,j)*(2.0*aaL + aaR) + &
                                  dp(i,j+1)*(2.0*aaR + aaL))
        enddo ; enddo
      else
        do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
          alpha_anom = 1.0/max(G%Rlay(k), tv%Rml(i,j,nkmb)) - alpha_ref
          dp(i,j) = G%H_to_Pa * h(i,j,k)
          dza(i,j,k) = alpha_anom*dp(i,j)
          intp_dza(i,j,k) = 0.5* alpha_anom*dp(i,j)**2
        enddo ; enddo
        do j=js,je ; do I=Isq,Ieq
          aaL = 1.0/max(G%Rlay(k), tv%Rml(i,j,nkmb)) - alpha_ref
          aaR = 1.0/max(G%Rlay(k), tv%Rml(i+1,j,nkmb)) - alpha_ref
          intx_dza(i,j,k) = C1_6 * (dp(i,j)*(2.0*aaL + aaR) + &
                                  dp(i+1,j)*(2.0*aaR + aaL))
        enddo ; enddo
        do J=Jsq,Jeq ; do i=is,ie
          aaL = 1.0/max(G%Rlay(k), tv%Rml(i,j,nkmb)) - alpha_ref
          aaR = 1.0/max(G%Rlay(k), tv%Rml(i,j+1,nkmb)) - alpha_ref
          inty_dza(i,j,k) = G%g_Earth*C1_6 * (dp(i,j)*(2.0*aaL + aaR) + &
                                             dp(i,j+1)*(2.0*aaR + aaL))
        enddo ; enddo
      endif
    else
  ...
  if (CS%GFS_scale < 1.0) then
    ...
    elseif (CS%bulkmixedlayer) then
      do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
        dM(i,j) = (CS%GFS_scale - 1.0) * &
          (p(i,j,1)*(1.0/tv%Rml(i,j,1) - alpha_ref) + za(i,j))
      enddo ; enddo
    else
      do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
        dM(i,j) = (CS%GFS_scale - 1.0) * &
...
! formal calculations with hydrostatically unstable profiles.
  ...
  if (CS%GFS_scale < 1.0) then
    ...
    elseif (CS%bulkmixedlayer) then
      do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
        dM(i,j) = (CS%GFS_scale - 1.0) * (G_Rho0 * tv%Rml(i,j,1)) * e(i,j,1)
      enddo ; enddo
    else
      do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
...
! Have checked that rho_0 drops out and that the 1-layer case is right. RWH.
  ...
  do k=1,nz
    ...
    elseif (CS%bulkmixedlayer) then
      if (k<=nkmb) then
        do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
          dpa(i,j) = (tv%Rml(i,j,k) - rho_ref)*dz(i,j)
          intz_dpa(i,j) = 0.5*(tv%Rml(i,j,k) - rho_ref)*dz(i,j)*h(i,j,k)
          rho_star(i,j,k) = G_Rho0*tv%Rml(i,j,k)
        enddo ; enddo
        do j=js,je ; do I=Isq,Ieq
          raL = tv%Rml(i,j,k) - rho_ref ; raR = tv%Rml(i+1,j,k) - rho_ref
          intx_dpa(I,j) = C1_6 * (dz(i,j)*(2.0*raL + raR) + &
                                  dz(i+1,j)*(2.0*raR + raL))
        enddo ; enddo
        do J=Jsq,Jeq ; do i=is,ie
          raL = tv%Rml(i,j,k) - rho_ref ; raR = tv%Rml(i,j+1,k) - rho_ref
          inty_dpa(i,j) = C1_6 * (dz(i,j)*(2.0*raL + raR) + &
                                  dz(i,j+1)*(2.0*raR + raL))
        enddo ; enddo
      else
        do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
          Rho_anom_here = max(G%Rlay(k), tv%Rml(i,j,nkmb)) - rho_ref
          dpa(i,j) = Rho_anom_here*dz(i,j)
          intz_dpa(i,j) = 0.5*Rho_anom_here*dz(i,j)*h(i,j,k)
          rho_star(i,j,k) = G_Rho0*max(G%Rlay(k), tv%Rml(i,j,nkmb))
        enddo ; enddo
        do j=js,je ; do I=Isq,Ieq
          raL = max(G%Rlay(k), tv%Rml(i,j,nkmb)) - rho_ref
          raR = max(G%Rlay(k), tv%Rml(i+1,j,nkmb)) - rho_ref
          intx_dpa(I,j) = C1_6 * (dz(i,j)*(2.0*raL + raR) + &
                                  dz(i+1,j)*(2.0*raR + raL))
        enddo ; enddo
        do J=Jsq,Jeq ; do i=is,ie
          raL = max(G%Rlay(k), tv%Rml(i,j,nkmb)) - rho_ref
          raR = max(G%Rlay(k), tv%Rml(i,j+1,nkmb)) - rho_ref
          inty_dpa(i,j) = C1_6 * (dz(i,j)*(2.0*raL + raR) + &
                                  dz(i,j+1)*(2.0*raR + raL))
        enddo ; enddo
      endif
  ...
  if (present(pbce)) then
    if (use_EOS .or. .not.CS%bulkmixedlayer) then
      call set_pbce_Bouss(e, tv_tmp, G, G%g_Earth, CS%Rho0, CS%GFS_scale, pbce)
    else
      call set_pbce_Bouss(e, tv_tmp, G, G%g_Earth, CS%Rho0, CS%GFS_scale, pbce, rho_star)
    endif
  endif
...
!  (in)      tides_CSp - a pointer to the control structure of the tide module.
  character(len=128) :: version = '$Id: GOLD_PressureForce_analytic_FV.F90,v 1.1.2.3.2.19 2011/05/12 22:08:05 Robert.Hallberg Exp $'
  ...
  call log_param(param_file, mod, "BULKMIXEDLAYER", CS%bulkmixedlayer, &
                 "If true, use a Kraus-Turner-like refined bulk mixed \n"//&
                 "layer with transitional buffer layers.  Layers 1 \n"//&
                 "through NKML+NKBL have variable densities. There must \n"//&
                 "be at least NKML+NKBL+1 layers if BULKMIXEDLAYER is true.", &
                 default=.false.)
