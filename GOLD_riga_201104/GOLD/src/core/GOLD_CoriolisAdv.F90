...
!                 for this module
  ...
  character(len=128) :: version = '$Id: GOLD_CoriolisAdv.F90,v 13.0.2.4.2.25 2011/09/25 08:20:02 Robert.Hallberg Exp $'
  ...
  CS%Coriolis_En_Dis = .false. ; debug_truncations = .false.

  call read_param(param_file,"NOSLIP",CS%no_slip)
  call read_param(param_file,"BOUND_CORIOLIS",CS%bound_Coriolis)
  call read_param(param_file,"CORIOLIS_EN_DIS",CS%Coriolis_En_Dis)
  call read_param(param_file,"DEBUG_TRUNCATIONS",debug_truncations)

  ! Set %Coriolis_Scheme
  ! (Select the baseline discretization for the Coriolis term)
  tmpstr = SADOURNY75_ENERGY_STRING
  call read_param(param_file, "CORIOLIS_SCHEME", tmpstr, .true.)
  ...
  if (CS%Coriolis_Scheme == AL_BLEND) then
    CS%wt_lin_blend = 0.125 ; CS%F_eff_max_blend = 4.0
    call read_param(param_file, "CORIOLIS_BLEND_WT_LIN", CS%wt_lin_blend)
    call read_param(param_file, "CORIOLIS_BLEND_F_EFF_MAX", CS%F_eff_max_blend)
    CS%wt_lin_blend = min(1.0, max(CS%wt_lin_blend,1e-16))
    if (CS%F_eff_max_blend <= 2.0) call GOLD_error(FATAL, "CoriolisAdv_init: "//&
           "CORIOLIS_BLEND_F_EFF_MAX must be greater than 2.")
  endif

  ! Set KE_Scheme (selects discretization of KE)
  tmpstr = KE_ARAKAWA_STRING; call read_param(param_file,"KE_SCHEME",tmpstr)
  tmpstr = uppercase(tmpstr)
  select case (tmpstr)
  ...
  tmpstr = PV_ADV_CENTERED_STRING
  call read_param(param_file,"PV_ADV_SCHEME",tmpstr); tmpstr = uppercase(tmpstr)
  select case (tmpstr)
...
! These are hard-wired diagnostics.
  ...
  if (CS%Coriolis_En_Dis .and. (CS%Coriolis_Scheme == SADOURNY75_ENERGY)) then
    call log_param(param_file, mod, "BOUND_CORIOLIS", CS%bound_Coriolis, &
  ...
  call log_param(param_file, mod, "CORIOLIS_EN_DIS", CS%Coriolis_En_Dis, &
                 ...
                 default=.false.)
  tmpstr = " "
  if (CS%Coriolis_Scheme == SADOURNY75_ENERGY) tmpstr = SADOURNY75_ENERGY_STRING
  if (CS%Coriolis_Scheme == ARAKAWA_HSU90) tmpstr = ARAKAWA_HSU_STRING
  if (CS%Coriolis_Scheme == ROBUST_ENSTRO) tmpstr = ROBUST_ENSTRO_STRING
  if (CS%Coriolis_Scheme == SADOURNY75_ENSTRO) tmpstr = SADOURNY75_ENSTRO_STRING
  if (CS%Coriolis_Scheme == ARAKAWA_LAMB81) tmpstr = ARAKAWA_LAMB_STRING
  if (CS%Coriolis_Scheme == AL_BLEND) tmpstr = AL_BLEND_STRING
  call log_param(param_file, mod, "CORIOLIS_SCHEME", tmpstr, &
  ...
  tmpstr = " "
  if (CS%KE_Scheme == KE_ARAKAWA) tmpstr = KE_ARAKAWA_STRING
  if (CS%KE_Scheme == KE_SIMPLE_GUDONOV) tmpstr = KE_SIMPLE_GUDONOV_STRING
  if (CS%KE_Scheme == KE_GUDONOV) tmpstr = KE_GUDONOV_STRING
  call log_param(param_file, mod, "KE_SCHEME", tmpstr, &
  ...
  tmpstr = " "
  if (CS%PV_Adv_Scheme == PV_ADV_CENTERED) tmpstr = PV_ADV_CENTERED_STRING
  if (CS%PV_Adv_Scheme == PV_ADV_UPWIND1) tmpstr = PV_ADV_UPWIND1_STRING
  call log_param(param_file, mod, "PV_ADV_SCHEME", tmpstr, &
