...
use GOLD_variables, only : ocean_OBC_type, BT_cont_type, OBC_SIMPLE
...
type, public :: continuity_PPM_CS ; private
  ...
  logical :: Riga_bugs       ! If true, use code that retains bugs that were in
                             ! the Riga version of the code.  False by default.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
  ...
  if (.not.associated(CS)) call GOLD_error(FATAL, &
         "GOLD_continuity_PPM: Module must be initialized before it is used.")
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
  ...
  if (present(visc_rem_u) /= present(visc_rem_v)) call GOLD_error(FATAL, &
      "GOLD_continuity_PPM: Either both visc_rem_u and visc_rem_v or neither"// &
      " one must be present in call to continuity_PPM.")

!    First, advect zonally.
  LB%ish = G%isc ; LB%ieh = G%iec ; LB%jsh = G%jsc-3 ; LB%jeh = G%jec+3
...
!                      areas when estimating the CFL number.
  ...
  real :: h_marg ! The marginal thickess of a flux, in H.
  integer :: i
...
!GOMP(              visc_rem_max, I_vrm, dv_lim, dy_N, dy_S ))
  do J=jsh-1,jeh
    ...
    if (present(vhbt) .or. do_aux .or. set_BT_cont) then
      ...
      if (use_visc_rem .or. CS%aggress_adjust .or. (.not.CS%Riga_bugs)) then
...
!                       areas when estimating the CFL number.
  ...
  real :: curv_3 ! A measure of the thickness curvature over a grid length,
                 ! with the same units as h_in.
  real :: h_marg ! The marginal thickess of a flux, in m.
  integer :: i
...
subroutine PPM_reconstruction_x(h_in, h_l, h_r, G, LB, h_min, monotonic)
...
! Local variables with useful mnemonic names.
  ...
  integer :: i, j, isl, iel, jsl, jel

  use_CW84 = .false. ; if (present(monotonic)) use_CW84 = monotonic
  isl = LB%ish-1 ; iel = LB%ieh+1 ; jsl = LB%jsh ; jel = LB%jeh

  if ((isl < G%isd+2) .or. (iel > G%ied-2)) then
    write(mesg,'("In GOLD_continuity_PPM, PPM_reconstruction_x called with a ", &
               & "x-halo that needs to be increased by ",i2,".")') &
               2 + max(G%isd-isl,iel-G%ied)
...
subroutine PPM_reconstruction_y(h_in, h_l, h_r, G, LB, h_min, monotonic)
  real, dimension(NXMEM_,NYMEM_), intent(in)  :: h_in
  real, dimension(NXMEM_,NYMEM_), intent(out) :: h_l, h_r
  type(ocean_grid_type),          intent(in)  :: G
  type(loop_bounds_type),         intent(in)  :: LB
...
! Local variables with useful mnemonic names.
  ...
  if ((isl < G%isd) .or. (iel > G%ied)) then
    write(mesg,'("In GOLD_continuity_PPM, PPM_reconstruction_y called with a ", &
               & "x-halo that needs to be increased by ",i2,".")') &
               max(G%isd-isl,iel-G%ied)
    call GOLD_error(FATAL,mesg)
  endif
  if ((jsl < G%jsd+2) .or. (jel > G%jed-2)) then
    write(mesg,'("In GOLD_continuity_PPM, PPM_reconstruction_y called with a ", &
                 & "y-halo that needs to be increased by ",i2,".")') &
                 2 + max(G%jsd-jsl,jel-G%jed)
    call GOLD_error(FATAL,mesg)
  endif
...
!                 for this module
  character(len=128) :: version = '$Id: GOLD_continuity_PPM.F90,v 1.1.2.6.2.33 2011/09/29 19:58:55 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_continuity_PPM" ! This module's name.

  if (associated(CS)) then
    call GOLD_error(WARNING, "continuity_PPM_init called with associated control structure.")
    return
  endif
  allocate(CS)

  CS%monotonic = .false.
  call read_param(param_file, "MONOTONIC_CONTINUITY", CS%monotonic)
  CS%tol_eta = 0.5*G%ke*G%Angstrom_z
  call read_param(param_file, "ETA_TOLERANCE", CS%tol_eta)
  CS%tol_eta_aux = CS%tol_eta
  call read_param(param_file, "ETA_TOLERANCE_AUX", CS%tol_eta_aux)
  CS%tol_vel = 3.0e8 ! Use the speed of light as the default.
  call read_param(param_file, "VELOCITY_TOLERANCE", CS%tol_vel)
  CS%CFL_limit_adjust = 1.0 ! This is for preservation of answers - it should be 0.5.
  call read_param(param_file, "CONTINUITY_CFL_LIMIT", CS%CFL_limit_adjust)
  CS%aggress_adjust = .false.
  call read_param(param_file, "CONT_PPM_AGGRESS_ADJUST", CS%aggress_adjust)
  CS%vol_CFL = CS%aggress_adjust
  if (.not.CS%aggress_adjust) &
    call read_param(param_file, "CONT_PPM_VOLUME_BASED_CFL", CS%vol_CFL)
  CS%better_iter = .false. ; CS%use_visc_rem_max = .false.
  call read_param(param_file, "CONT_PPM_BETTER_ITER", CS%better_iter)
  call read_param(param_file, "CONT_PPM_USE_VISC_REM_MAX", CS%use_visc_rem_max)
  CS%Riga_bugs = .false.
  call read_param(param_file, "CONT_PPM_RIGA_BUGS", CS%Riga_bugs)
  CS%diag => diag
  ...
  call log_param(param_file, mod, "MONOTONIC_CONTINUITY", CS%monotonic, &
  ...
  call log_param(param_file, mod, "ETA_TOLERANCE", CS%tol_eta, &
  ...
  call log_param(param_file, mod, "ETA_TOLERANCE_AUX", CS%tol_eta_aux, &
                 ...
                 units="m")
  call log_param(param_file, mod, "VELOCITY_TOLERANCE", CS%tol_vel, &
                 "The tolerance for barotropic velocity discrepancies \n"//&
                 "between the barotropic solution and  the sum of the \n"//&
                 "layer thicknesses.", units="m s-1", default=3.0e8)
  call log_param(param_file, mod, "CONT_PPM_AGGRESS_ADJUST", CS%aggress_adjust,&
                 "If true, allow the adjusted velocities to have a \n"//&
                 "relative CFL change up to 0.5.", default=.false.)
  call log_param(param_file, mod, "CONT_PPM_VOLUME_BASED_CFL", CS%vol_CFL, &
                 ...
                 default=CS%aggress_adjust)
  call log_param(param_file, mod, "CONTINUITY_CFL_LIMIT", CS%CFL_limit_adjust, &
                 "The maximum CFL of the adjusted velocities.  The \n"//&
                 "default should be changed to 0.5.", units="nondim", &
                 default=1.0)
  call log_param(param_file, mod, "CONT_PPM_BETTER_ITER", CS%better_iter, &
                 "If true, stop corrective iterations using a velocity \n"//&
                 "based criterion and only stop if the iteration is \n"//&
                 "better than all predecessors.", default=.false.)
  call log_param(param_file, mod, "CONT_PPM_USE_VISC_REM_MAX", &
                                 CS%use_visc_rem_max, &
                 "If true, use more appropriate limiting bounds for \n"//&
                 "corrections in strongly viscous columns.", default=.false.)
  call log_param(param_file, mod, "CONT_PPM_RIGA_BUGS", CS%Riga_bugs, &
                 "If true, use code that retains a bug in intializing \n"//&
                 "some of the iterative solvers that was in the Riga \n"//&
                 "version of the code.", default=.false.)
...
end module GOLD_continuity_PPM
