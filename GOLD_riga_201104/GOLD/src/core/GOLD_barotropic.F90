...
use GOLD_diag_mediator, only : post_data, query_averaging_enabled, register_diag_field
use GOLD_diag_mediator, only : safe_alloc_ptr, diag_ptrs, enable_averaging
use GOLD_domains, only : pass_var, pass_vector, min_across_PEs, GOLD_domains_init
use GOLD_domains, only : pass_var_start, pass_var_complete
use GOLD_domains, only : pass_vector_start, pass_vector_complete
use GOLD_domains, only : To_All, Scalar_Pair, AGRID, CORNER, GOLD_domain_type
use GOLD_domains, only : pass_var_start, pass_var_complete
use GOLD_domains, only : pass_vector_start, pass_vector_complete
...
use GOLD_tidal_forcing, only : tidal_forcing_sensitivity, tidal_forcing_CS
use GOLD_time_manager, only : time_type, set_time, operator(+), operator(-)
use GOLD_variables, only : forcing, BT_cont_type, ocean_OBC_type, OBC_SIMPLE
use GOLD_variables, only : OBC_FLATHER_E, OBC_FLATHER_W, OBC_NONE
...
#    define BT_HALO 0
#  endif
#  define WHALO_X MAX(BT_HALO-NX_HALO,0)
#  define WHALO_Y MAX(BT_HALO-NY_HALO,0)
#  define NXMEMW_   1-WHALO_X:NXMEM+WHALO_X
#  define NYMEMW_   1-WHALO_Y:NYMEM+WHALO_Y
#  define NXMEMQW_  -WHALO_X:NXMEM+WHALO_X
#  define NYMEMQW_  -WHALO_Y:NYMEM+WHALO_Y
#  define SZIW_(G)  NXMEMW_
#  define SZJW_(G)  NYMEMW_
#  define SZIQW_(G) NXMEMQW_
#  define SZJQW_(G) NYMEMQW_
#else
#  define NXMEMW_   :
#  define NYMEMW_   :
#  define NXMEMQW_  :
...
#  define SZIQW_(G) G%isdw-1:G%iedw
#  define SZJQW_(G) G%jsdw-1:G%jedw
#endif
...
type, public :: barotropic_CS ; private
  ...
  real PTR_, dimension(NXMEM_,NYMEM_) :: &
    ...
    eta_cor_slow TO_NULL_, & ! A more slowly varying corrective pseudo mass flux
                       ! into the barotropic equations in H. With this term, the
                       ! steady solution has baroclinic and barotropic heights
  ...
  real PTR_, dimension(NXMEMW_,NYMEMW_) :: &
  ...
  real PTR_, dimension(NXMEMQW_,NYMEMW_) :: &
  ...
  real PTR_, dimension(NXMEMW_,NYMEMQW_) :: &
  ...
  real PTR_, dimension(NXMEMQW_,NYMEMQW_) :: &
  ...
  real    :: BT_frac_cor     ! The fraction of the SSH anomaly to be corrected
                             ! by pseudo mass fluxes within a timestep.  The
                             ! default (for historical reasons) is 0.25, but the
                             ! more appropriate value with the iterative solver
                             ! might be 1.0 if BT_cor_slow_rate = 0.
  real    :: BT_cor_slow_rate ! The rate per timestep at which the slowly
                             ! evolving pseudo mass fluxes are adjusted.  The
                             ! default value is 0.125, but with the iterative
                             ! solver 0 might be more appropriate.
  ...
  logical :: arith_hvel      ! If true use the arithmetic mean layer thicknesses
                             ! to estimate the barotropic velocity. The default
                             ! is to use harmonic mean thicknesses.
  logical :: hybrid_hvel     ! If true, the thicknesses used to estimate the
                             ! barotropic velocity vary with depth relative to
                             ! the shallowest neighboring bottom, using an
                             ! arithmetic mean for layers above the shallowest
                             ! bottom, the harmonic mean for layers below, and
                             ! a weighted average for layers that straddle that
                             ! depth. The default is to use harmonic mean
  ...
  type(GOLD_domain_type), pointer :: BT_Domain => NULL()
...
subroutine btstep(use_fluxes, U_in, V_in, eta_in, dt, bc_accel_u, bc_accel_v, &
  ...
  real, dimension(NXMEMQ_,NYMEM_),     intent(out), optional :: sum_u_dhdt
  real, dimension(NXMEM_,NYMEMQ_),     intent(out), optional :: sum_v_dhdt
  real, dimension(NXMEM_,NYMEM_),      intent(out), optional :: etaav
  real, dimension(NXMEMQ_,NYMEM_),     intent(out), optional :: uhbt_out
  real, dimension(NXMEM_,NYMEMQ_),     intent(out), optional :: vhbt_out
...
!  from input velocites or volume (Boussinesq) or mass (non-Boussinesq) fluxes.
  ...
  real :: ubt_Cor(SZIQ_(G),SZJ_(G)) ! The barotropic velocities that had been
  real :: vbt_Cor(SZI_(G),SZJQ_(G)) ! use to calculate the input Coriolis
                                    ! terms, in m s-1.
  real :: wt_u(SZIQ_(G),SZJ_(G),SZK_(G)) ! wt_u and wt_v are the
  real :: wt_v(SZI_(G),SZJQ_(G),SZK_(G)) ! normalized weights to
  ...
  real :: q(SZIQW_(CS),SZJQW_(CS))  ! A pseudo potential vorticity in s-1 m-1.
  real, dimension(SZIQW_(CS),SZJW_(CS)) :: &
    ...
    Datu          ! Basin depth at u-velocity grid points times the y-grid
                  ! spacing, in H m.
  real, dimension(SZIW_(CS),SZJQW_(CS)) :: &
  ...
  type(local_BT_cont_u_type), dimension(SZIQW_(CS),SZJW_(CS)) :: &
    BTCL_u        ! A repackaged version of the u-point information in BT_cont.
  type(local_BT_cont_v_type), dimension(SZIW_(CS),SZJQW_(CS)) :: &
  ...
  integer :: pid_eta_PF_1, pid_d_eta_PF
  ...
  if (.not.associated(CS)) call GOLD_error(FATAL, &
      "btstep: Module GOLD_barotropic must be initialized before it is used.")
  ...
  if ((use_visc_rem) /= present(visc_rem_v)) call GOLD_error(FATAL, &
  ...
  if (present(OBC)) then ; if (associated(OBC)) then
    ...
    if (apply_OBC_flather .and. .not.G%Boussinesq) call GOLD_error(FATAL, &
...
!# endif
  ...
  ! Set the mass source, perhaps first initializing the halos to 0.
  do j=jsvf-1,jevf+1; do i=isvf-1,ievf+1 ; eta_src(i,j) = 0.0 ; enddo ; enddo
  !   These bounds must extend 1 past the computational domain to work
  ! with the open boundary conditions.
  if (CS%bound_BT_corr) then ; do j=js-1,je+1 ; do i=is-1,ie+1
  ...
  if (nonblock_setup) then
    ...
    ! The following halo update is not needed without wide halos.  RWH
    if ((use_visc_rem) .and. (CS%drag_amp > 0.0) .and. (ievf > ie)) &
...
! Calculate the initial barotropic velocities from the layer's velocities.
  do j=jsvf-1,jevf+1 ; do I=isvf-2,ievf+1 ; ubt(I,j) = 0.0 ; uhbt(I,j) = 0.0 ; u_accel_bt(I,j) = 0.0 ; enddo ; enddo
  do J=jsvf-2,jevf+1 ; do i=isvf-1,ievf+1 ; vbt(i,J) = 0.0 ; vhbt(i,J) = 0.0 ; v_accel_bt(i,J) = 0.0 ; enddo ; enddo
  if (use_fluxes) then
 ...
 !GOMP(parallel do default(shared) private(i, j))
      do j=jsv-1,jev+1 ; do I=isv-2,iev+1
        uhbt(I,j) = find_uhbt(ubt(I,j),BTCL_u(I,j))
      enddo ; enddo
      do J=jsv-2,jev+1 ; do i=isv-1,iev+1
        vhbt(i,J) = find_vhbt(vbt(i,J),BTCL_v(i,J))
      enddo ; enddo
...
!GOMP(parallel do default(shared) private(i, j))
    ...
    if (find_etaav) then ; do j=js,je ; do i=is,ie
      eta_sum(i,j) = eta_sum(i,j) + eta_pred(i,j)
    enddo ; enddo ; endif
...
!GOMP(parallel do default(shared) private(i, j, Cor, gradP, vel_prev, vel_trans ))
      do J=jsv-1,jev ; do i=isv-1,iev+1
        ...
        gradP = ((eta_pred(i,j)-eta_PF(i,j))*gtot_N(i,j) - &
                 (eta_pred(i,j+1)-eta_PF(i,j+1))*gtot_S(i,j+1)) * &
                dgeo_de * CS%IDYv(i,J)
        if (CS%dynamic_psurf) &
          gradP = gradP + (p_surf_dyn(i,j) - p_surf_dyn(i,j+1)) * CS%IDYv(i,J)
        v_accel_bt(I,j) = v_accel_bt(I,j) + Instep*(Cor + gradP)
        if (find_PF)  PFv_bt_sum(i,J)  = PFv_bt_sum(i,J) + gradP
        if (find_Cor) Corv_bt_sum(i,J) = Corv_bt_sum(i,J) + Cor
        ...
        vbt_sum(i,J) = vbt_sum(i,J) + vel_trans
        if (use_BT_cont) then
          vhbt(i,J) = find_vhbt(vel_trans,BTCL_v(i,J))
        else
          vhbt(i,J) = Datv(i,J)*vel_trans
        endif
        vhbt_sum(i,J) = vhbt_sum(i,J) + vhbt(i,J)
      enddo ; enddo
...
!GOMP(parallel do default(shared) private(i, j, Cor, gradP, vel_prev, vel_trans ))
      do j=jsv,jev ; do I=isv-1,iev
        ...
        gradP = ((eta_pred(i,j)-eta_PF(i,j))*gtot_E(i,j) - &
                 (eta_pred(i+1,j)-eta_PF(i+1,j))*gtot_W(i+1,j)) * &
                dgeo_de * CS%IDXu(I,j)
        if (CS%dynamic_psurf) &
          gradP = gradP + (p_surf_dyn(i,j) - p_surf_dyn(i+1,j)) * CS%IDXu(I,j)
        u_accel_bt(I,j) = u_accel_bt(I,j) + Instep*(Cor + gradP)
        if (find_PF)  PFu_bt_sum(I,j)  = PFu_bt_sum(I,j) + gradP
        if (find_Cor) Coru_bt_sum(I,j) = Coru_bt_sum(I,j) + Cor
        ...
        ubt_sum(I,j) = ubt_sum(I,j) + vel_trans
        if (use_BT_cont) then
          uhbt(I,j) = find_uhbt(vel_trans, BTCL_u(I,j))
        else
          uhbt(I,j) = Datu(I,j)*vel_trans
        endif
        uhbt_sum(I,j) = uhbt_sum(I,j) + uhbt(I,j)
      enddo ; enddo
...
!GOMP(parallel do default(shared) private(i, j, Cor, gradP, vel_prev, vel_trans ))
    ...
    if (apply_OBCs) call apply_velocity_OBCs(OBC, ubt, vbt, uhbt, vhbt, &
           ubt_sum, vbt_sum, uhbt_sum, vhbt_sum, eta, ubt_old, vbt_old, BT_OBC, &
           G, MS, iev-ie, dtbt, bebt, use_BT_cont, Datu, Datv, BTCL_u, BTCL_v)

    if (CS%debug) then
      call uchksum(uhbt, "BT uhbt just after OBC",CS%debug_BT_G,haloshift=0)
      call vchksum(vhbt, "BT vhbt just after OBC",CS%debug_BT_G,haloshift=0)
    endif
...
!GOMP(parallel do default(shared) private(i, j))
    ...
    if (CS%debug_bt) then
      ...
      call uchksum(ubt, mesg//" ubt",G,haloshift=1)
      call vchksum(vbt, mesg//" vbt",G,haloshift=1)
      call hchksum(G%H_to_kg_m2*eta, mesg//" eta",G,haloshift=1)
    endif
  ...
  do j=js,je ; do I=is-1,ie
    CS%ubtav(I,j) = ubt_sum(I,j) * Instep
    uhbtav(I,j) = uhbt_sum(I,j) * Instep
  enddo ; enddo

  do J=js-1,je ; do i=is,ie
    CS%vbtav(i,J) = vbt_sum(i,J) * Instep
    vhbtav(i,J) = vhbt_sum(i,J) * Instep
  enddo ; enddo

  if (present(uhbt_out)) then
    uhbt_out(:,:) = 0.0
    if (use_BT_cont) then ; do j=js,je ; do I=is-1,ie
      uhbt_out(I,j) = find_uhbt(ubt(I,j), BTCL_u(I,j))
    enddo ; enddo ; else ; do j=js,je ; do I=is-1,ie
      uhbt_out(I,j) = ubt(I,j) * Datu(I,j)
    enddo ; enddo ; endif

    vhbt_out(:,:) = 0.0
    if (use_BT_cont) then ; do J=js-1,je ; do i=is,ie
      vhbt_out(i,J) = find_vhbt(vbt(i,J), BTCL_v(i,J))
    enddo ; enddo ; else ; do J=js-1,je ; do i=is,ie
      vhbt_out(i,J) = vbt(i,J) * Datv(i,J)
    enddo ; enddo ; endif
  endif
...
!GOMP(parallel do default(shared) private(i, j, k))
  ...
  if (query_averaging_enabled(CS%diag)) then
    ...
    do j=js,je ; do I=is-1,ie ; CS%ubt_IC(I,j) = ubt(I,j) ; enddo ; enddo
    do J=js-1,je ; do i=is,ie ; CS%vbt_IC(i,J) = vbt(i,J) ; enddo ; enddo
    
    if (ASSOCIATED(CS%diag%PFu_bt)) then ; do j=js,je ; do I=is-1,ie
      CS%diag%PFu_bt(I,j) = PFu_bt_sum(I,j) * Instep
    enddo ; enddo ; endif
    if (ASSOCIATED(CS%diag%Coru_bt)) then ; do j=js,je ; do I=is-1,ie
      CS%diag%Coru_bt(I,j) = Coru_bt_sum(I,j) * Instep
    enddo ; enddo ; endif
    ...
    if (ASSOCIATED(CS%diag%PFv_bt)) then ; do J=js-1,je ; do i=is,ie
      CS%diag%PFv_bt(i,J) = PFv_bt_sum(i,J) * Instep
    enddo ; enddo ; endif
    if (ASSOCIATED(CS%diag%Corv_bt)) then ; do J=js-1,je ; do i=is,ie
      CS%diag%Corv_bt(i,J) = Corv_bt_sum(i,J) * Instep
    enddo ; enddo ; endif
...
!  Offer various barotropic terms for averaging.
    ...
    if (CS%id_ubt > 0) call post_data(CS%id_ubt, ubt(Isdq:Iedq,jsd:jed), CS%diag)
    if (CS%id_vbt > 0) call post_data(CS%id_vbt, vbt(isd:ied,Jsdq:Jedq), CS%diag)
...
!                      error when calculating the external wave speed, in m.
  ...
  real, dimension(SZI_(G),SZJ_(G)) :: &
    ...
    gtot_S        ! equations half a grid-point in the X-direction (X is N, S,
                  ! E, or W) from the thickness point. gtot_X has units of m2 H-1 s-2.
                  ! (See Hallberg, J Comp Phys 1997 for a discussion.)
  real, dimension(SZIQS_(G),SZJ_(G)) :: &
    Datu          ! Basin depth at u-velocity grid points times the y-grid
                  ! spacing, in m2.
  real, dimension(SZI_(G),SZJQS_(G)) :: &
    Datv          ! Basin depth at v-velocity grid points times the x-grid
                  ! spacing, in m2.
  real :: det_de  ! The partial derivative due to self-attraction and loading
                  ! of the reference geopotential with the sea surface height.
  ...
  if (.not.associated(CS)) call GOLD_error(FATAL, &
      "set_dtbt: Module GOLD_barotropic must be initialized before it is used.")
  if (.not.CS%split) return
  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec ; nz = G%ke
  MS%isdw = G%isd ; MS%iedw = G%ied ; MS%jsdw = G%jsd ; MS%jedw = G%jed

  if (.not.(present(pbce) .or. present(gtot_est))) call GOLD_error(FATAL, &
...
subroutine apply_velocity_OBCs(OBC, ubt, vbt, uhbt, vhbt, ubt_sum, vbt_sum, &
                               uhbt_sum, vhbt_sum, eta, ubt_old, vbt_old, BT_OBC, &
                               G, MS, halo, dtbt, bebt, use_BT_cont, Datu, Datv, &
                               BTCL_u, BTCL_v)
  type(ocean_OBC_type),              pointer       :: OBC
  type(ocean_grid_type),             intent(inout) :: G
  type(memory_size_type),            intent(in)    :: MS
  real, dimension(SZIQW_(MS),SZJW_(MS)), intent(inout) :: ubt, uhbt, ubt_sum, uhbt_sum
  real, dimension(SZIW_(MS),SZJQW_(MS)), intent(inout) :: vbt, vhbt, vbt_sum, vhbt_sum
  real, dimension(SZIW_(MS),SZJW_(MS)),   intent(in)    :: eta
  real, dimension(SZIQW_(MS),SZJW_(MS)),  intent(in)    :: ubt_old
  real, dimension(SZIW_(MS),SZJQW_(MS)),  intent(in)    :: vbt_old
  type(BT_OBC_type),                 intent(in)    :: BT_OBC
  integer,                           intent(in)    :: halo
  real,                              intent(in)    :: dtbt, bebt
  logical,                           intent(in)    :: use_BT_cont
  real, dimension(SZIQW_(MS),SZJW_(MS)),  intent(in)    :: Datu
  real, dimension(SZIW_(MS),SZJQW_(MS)),  intent(in)    :: Datv
  type(local_BT_cont_u_type), dimension(SZIQW_(MS),SZJW_(MS)), intent(in) :: BTCL_u
  type(local_BT_cont_v_type), dimension(SZIW_(MS),SZJQW_(MS)), intent(in) :: BTCL_v
...
!  (inout)   ubt_sum - the sum over timesteps of the zonal barotropic
!                      velocity, in m s-1.
!  (inout)   vbt_sum - the sum over timesteps of the meridional barotropic
!                      velocity, in m s-1.
!  (inout)   uhbt_sum - the sum over timesteps of the zonal barotropic
!                       transport, in H m2 s-1.
!  (inout)   vhbt_sum - the sum over timesteps of the meridional barotropic
!                       transport, H m2 s-1.
...
subroutine apply_eta_OBCs(OBC, eta, ubt, vbt, BT_OBC, G, MS, halo, dtbt)
  ...
  real, dimension(SZIQW_(MS),SZJW_(MS)), intent(in)    :: ubt
  real, dimension(SZIW_(MS),SZJQW_(MS)), intent(in)    :: vbt
  type(BT_OBC_type),                     intent(in)    :: BT_OBC
  type(ocean_grid_type),                 intent(inout) :: G
  integer,                               intent(in)    :: halo
  real,                                  intent(in)    :: dtbt
...
!  (in)      BCTL_v - of the face areas at u- and v- points.
  ...
  if ((isdw < isd) .or. (jsdw < jsd)) then
    call GOLD_error(FATAL, "set_up_BT_OBC: Open boundary conditions are not "//&
  ...
  OBC_used = .false.
  do j=js,je ; do I=is-1,ie ; if (BT_OBC%OBC_mask_u(I,j)) OBC_used = .true. ; enddo ; enddo
  if (.not. OBC_used) deallocate(BT_OBC%OBC_mask_u)

  OBC_used = .false.
  do J=js-1,je ; do i=is,ie ; if (BT_OBC%OBC_mask_v(i,J)) OBC_used = .true. ; enddo ; enddo
...
subroutine btcalc(h, G, CS, h_u, h_v)
...
! All of these variables are in the same units as h - usually m or kg m-2.
  real :: hatutot(SZI_(G))     ! The sum of the layer thicknesses
  real :: hatvtot(SZIQ_(G))    ! interpolated to the u & v grid points.
  real :: Ihatutot(SZI_(G))    ! Ihatutot and Ihatvtot are the inverses
  real :: Ihatvtot(SZIQ_(G))   ! of hatutot and hatvtot, both in H-1.
  ...
  real :: e_u(SZI_(G),SZK_(G)+1) !   The interface heights at u-velocity and
  real :: e_v(SZIQ_(G),SZK_(G)+1)! v-velocity points in H.
  real :: D_shallow_u(SZI_(G)) ! The shallower of the adjacent depths in H.
  real :: D_shallow_v(SZIQ_(G))! The shallower of the adjacent depths in H.
...
! second order accurate estimate h = 2*(h+ * h-)/(h+ + h-).
  if (.not.associated(CS)) call GOLD_error(FATAL, &
      "btcalc: Module GOLD_barotropic must be initialized before it is used.")
  if (.not.CS%split) return
...
!GOMP(parallel do default(shared) private(i, j, k, hatutot, h_harm, Rh))
    do j=js-1,je+1
...
!GOMP(parallel do default(shared) private(i, j, k, hatvtot, h_harm, Rh))
    do J=js-2,je+1
...
subroutine set_local_BT_cont_types(BT_cont, BTCL_u, BTCL_v, G, MS, BT_Domain, halo)
  ...
  type(memory_size_type),                                intent(in)    :: MS
  type(local_BT_cont_u_type), dimension(SZIQW_(MS),SZJW_(MS)), intent(out) :: BTCL_u
  type(local_BT_cont_v_type), dimension(SZIW_(MS),SZJQW_(MS)), intent(out) :: BTCL_v
  type(ocean_grid_type),                                 intent(inout) :: G
  type(GOLD_domain_type),                                intent(inout) :: BT_Domain
...
!  (in)      halo - The extra halo size to use here.
  ...
  real, dimension(SZIQW_(MS),SZJW_(MS)) :: &
    u_polarity, uBT_EE, uBT_WW, FA_u_EE, FA_u_E0, FA_u_W0, FA_u_WW
  real, dimension(SZIW_(MS),SZJQW_(MS)) :: &
...
!  (in)      CS - The control structure returned by a previous call to
!                 barotropic_init.
  real :: h_tot(SZ1_(h))      ! The sum of the layer thicknesses, in H.
  real :: eta_h(SZ1_(h))      ! The free surface height determined from
  ...
  if (.not.associated(CS)) call GOLD_error(FATAL, "bt_mass_source: "// &
        "Module GOLD_barotropic must be initialized before it is used.")
  if (.not.CS%split) return

  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec ; nz = G%ke

  ! The loop bounds in this routine must extend 1 past the computational domain
  ! to work with the open boundary conditions.
!GOMP(parallel do default(shared) private(i,j,k,eta_h))
  do j=js-1,je+1
    do i=is-1,ie+1 ; h_tot(i) = h(i,j,1) ; enddo
    if (G%Boussinesq) then
      do i=is-1,ie+1 ; eta_h(i) = h(i,j,1) - G%D(i,j) ; enddo
    else
      do i=is-1,ie+1 ; eta_h(i) = h(i,j,1) ; enddo
    endif
    do k=2,nz ; do i=is-1,ie+1
    ...
    if (CS%BT_cor_slow_rate > 0.0) then
      if (set_cor) then
        do i=is-1,ie+1
          d_eta = eta_h(i) - (eta(i,j) - dt_since_therm*CS%eta_source(i,j))
          CS%eta_cor_slow(i,j) = CS%eta_cor_slow(i,j) + CS%BT_cor_slow_rate * &
                                 CS%BT_frac_cor * d_eta
          CS%eta_cor(i,j) = CS%eta_cor_slow(i,j) + CS%BT_frac_cor * d_eta
        enddo
      else
        do i=is-1,ie+1
          d_eta = eta_h(i) - (eta(i,j) - dt_since_therm*CS%eta_source(i,j))
          CS%eta_cor(i,j) = 0.5*((CS%eta_cor(i,j) + CS%eta_cor_slow(i,j)) + &
                                 CS%BT_frac_cor*d_eta)
        enddo
      endif
    else
      if (set_cor) then
        do i=is-1,ie+1
          d_eta = eta_h(i) - (eta(i,j) - dt_since_therm*CS%eta_source(i,j))
          CS%eta_cor(i,j) = CS%BT_frac_cor*d_eta
        enddo
      else
        do i=is-1,ie+1
          d_eta = eta_h(i) - (eta(i,j) - dt_since_therm*CS%eta_source(i,j))
          CS%eta_cor(i,j) = CS%eta_cor(i,j) + CS%BT_frac_cor*d_eta
        enddo
      endif
    endif
  enddo
...
!  (in)      tides_CSp - a pointer to the control structure of the tide module.
  character(len=128) :: version = '$Id: GOLD_barotropic.F90,v 13.0.2.12.2.74 2011/09/27 21:13:39 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_barotropic"  ! This module's name.
  real :: Datu(SZIQS_(G),SZJ_(G)), Datv(SZI_(G),SZJQS_(G))
  ...
  if (CS%module_is_initialized) then
    call GOLD_error(WARNING, "barotropic_init called with a control structure "// &
                            "that has already been initialized.")
    return
  endif
  CS%module_is_initialized = .true.

! SPLIT must be found in the parameter file - it has no default value.
  ...
  CS%bound_BT_corr = .false. ; CS%tides = .false. ; CS%Sadourny = .true.
  CS%Nonlinear_continuity = .false. ; CS%rescale_D_bt = .false.
  CS%clip_velocity = .false. ; apply_bt_drag = .true.
  CS%arith_hvel = .false. ; CS%hybrid_hvel = .false. ; CS%strong_drag = .false.
  CS%BT_frac_cor = 0.25 ; CS%BT_cor_slow_rate = 0.125
  CS%Nonlin_cont_update_period = 1 ; CS%eta_source_limit = 0.0
  CS%dynamic_psurf = .false. ; CS%use_wide_halos = .true.
  CS%Dmin_dyn_psurf = 1.0e-6 ! A minimum thickness, in m.
  CS%const_dyn_psurf = 0.9 ; CS%ice_strength_length = 1.0e4
  CS%linearized_BT_PV = .true.

  call read_param(param_file,"BOUND_BT_CORRECTION",CS%bound_BT_corr)
  call read_param(param_file,"BT_COR_FRAC", CS%BT_frac_cor)
  call read_param(param_file,"BT_COR_SLOW_RATE", CS%BT_cor_slow_rate)
  call read_param(param_file,"BT_USE_WIDE_HALOS", CS%use_wide_halos)
  call read_param(param_file,"NONLINEAR_BT_CONTINUITY",CS%Nonlinear_continuity)
  if (CS%Nonlinear_continuity) &
    call read_param(param_file,"NONLIN_BT_CONT_UPDATE_PERIOD", CS%Nonlin_cont_update_period)
  call read_param(param_file,"RESCALE_BT_FACE_AREAS",CS%rescale_D_bt)
  call read_param(param_file,"BT_MASS_SOURCE_LIMIT",CS%eta_source_limit)
  call read_param(param_file,"DYNAMIC_SURFACE_PRESSURE", CS%dynamic_psurf)
  if (CS%dynamic_psurf) then
    call read_param(param_file,"DEPTH_MIN_DYN_PSURF", CS%Dmin_dyn_psurf)
    call read_param(param_file,"CONST_DYN_PSURF", CS%const_dyn_psurf)
    call read_param(param_file,"ICE_LENGTH_DYN_PSURF", CS%ice_strength_length)
  endif
  call read_param(param_file,"TIDES",CS%tides)
  call read_param(param_file,"SADOURNY",CS%Sadourny)
  call read_param(param_file,"ARITHMETIC_BT_THICK",CS%arith_hvel)
  call read_param(param_file,"HYBRID_BT_THICK",CS%hybrid_hvel)
  call read_param(param_file,"BT_STRONG_DRAG",CS%strong_drag)
  call read_param(param_file,"DTBT",CS%dtbt,.true.)
  call read_param(param_file,"BEBT",CS%bebt,.true.)
  CS%G_extra = 0.0 ; call read_param(param_file,"G_BT_EXTRA",CS%G_extra)
  max_depth = 1.0e6 ; call read_param(param_file,"MAXIMUM_DEPTH",max_depth)
  SSH_extra = min(10.0,0.05*max_depth)   ! Dimensions of m.
  call read_param(param_file,"SSH_EXTRA",SSH_extra)
  call read_param(param_file,"APPLY_BT_DRAG",apply_bt_drag)
  call read_param(param_file,"CLIP_BT_VELOCITY",CS%clip_velocity)
  CS%debug = .false. ; call read_param(param_file,"DEBUG",CS%debug)
  CS%debug_bt = .false. ; call read_param(param_file,"DEBUG_BT",CS%debug_bt)
  CS%maxvel = 1.0e6 ; call read_param(param_file,"MAXVEL",CS%maxvel)
  CS%maxCFL_BT_cont = 0.1 ; call read_param(param_file,"MAXCFL_BT_CONT",CS%maxCFL_BT_cont)
  bt_halo_sz = 0 ; call read_param(param_file, "BTHALO", bt_halo_sz)
#ifdef STATIC_MEMORY
  if ((bt_halo_sz > 0) .and. (bt_halo_sz /= BT_HALO)) call GOLD_error(FATAL, &
      "barotropic_init: Run-time values of BTHALO must agree with the \n"//&
      "macro BT_HALO with STATIC_MEMORY.")
  wd_halos(1) = WHALO_X+NX_HALO ; wd_halos(2) = WHALO_Y+NY_HALO
#else
  wd_halos(1) = bt_halo_sz; wd_halos(2) =  bt_halo_sz
#endif
  if (apply_bt_drag) then ; CS%drag_amp = 1.0 ; else ; CS%drag_amp = 0.0 ; endif

  ! Initialize a version of the GOLD domain that is specific to the barotropic solver.
  call GOLD_domains_init(CS%BT_Domain, param_file, min_halo=wd_halos, symmetric=.true.)
#ifdef STATIC_MEMORY
  if (wd_halos(1) /= WHALO_X+NX_HALO) call GOLD_error(FATAL, "barotropic_init: "//&
          "Barotropic x-halo sizes are incorrectly resized with STATIC_MEMORY.")
  if (wd_halos(2) /= WHALO_Y+NY_HALO) call GOLD_error(FATAL, "barotropic_init: "//&
          "Barotropic y-halo sizes are incorrectly resized with STATIC_MEMORY.")
#else
  if (bt_halo_sz > 0) then
    if (wd_halos(1) > bt_halo_sz) &
      call GOLD_mesg("barotropic_init: barotropic x-halo size increased.", 3)
    if (wd_halos(2) > bt_halo_sz) &
      call GOLD_mesg("barotropic_init: barotropic y-halo size increased.", 3)
  endif
#endif
  ...
  ALLOC(CS%frhatu(Isdq:Iedq,jsd:jed,nz)) ; ALLOC(CS%frhatv(isd:ied,Jsdq:Jedq,nz))
  ALLOC(CS%eta_source(isd:ied,jsd:jed)) ; ALLOC(CS%eta_cor(isd:ied,jsd:jed))
  if (CS%bound_BT_corr) then
    ALLOC(CS%eta_cor_bound(isd:ied,jsd:jed)) ; CS%eta_cor_bound(:,:) = 0.0
  endif
  ALLOC(CS%IDatu(Isdq:Iedq,jsd:jed)) ; ALLOC(CS%IDatv(isd:ied,Jsdq:Jedq))

  ALLOC(CS%Datu_res(isdw-1:iedw,jsdw:jedw))
  ALLOC(CS%Datv_res(isdw:iedw,jsdw-1:jedw))
  ALLOC(CS%ua_polarity(isdw:iedw,jsdw:jedw))
  ALLOC(CS%va_polarity(isdw:iedw,jsdw:jedw))
  ...
  ALLOC(CS%Idxdyh(CS%isdw:CS%iedw,CS%jsdw:CS%jedw)) ; CS%Idxdyh(:,:) = 0.0
  ALLOC(CS%D(CS%isdw:CS%iedw,CS%jsdw:CS%jedw)) ; CS%D(:,:) = G%Angstrom_z
  ALLOC(CS%Idxu(CS%isdw-1:CS%iedw,CS%jsdw:CS%jedw)) ; CS%Idxu(:,:) = 0.0
  ALLOC(CS%Idyv(CS%isdw:CS%iedw,CS%jsdw-1:CS%jedw)) ; CS%Idyv(:,:) = 0.0
  ALLOC(CS%dy_u(CS%isdw-1:CS%iedw,CS%jsdw:CS%jedw)) ; CS%dy_u(:,:) = 0.0
  ALLOC(CS%dx_v(CS%isdw:CS%iedw,CS%jsdw-1:CS%jedw)) ; CS%dx_v(:,:) = 0.0
  do j=G%jsd,G%jed ; do i=G%isd,G%ied
  ...
  if (CS%linearized_BT_PV) then
    ALLOC(CS%q_D(CS%isdw-1:CS%iedw,CS%jsdw-1:CS%jedw))
    ALLOC(CS%D_u_Cor(CS%isdw-1:CS%iedw,CS%jsdw:CS%jedw))
    ALLOC(CS%D_v_Cor(CS%isdw:CS%iedw,CS%jsdw-1:CS%jedw))
  ...
  call set_dtbt(G, CS, gtot_est = gtot_estimate, SSH_add = SSH_extra)
  if (dtbt_input > 0.0) CS%dtbt = dtbt_input

  ! ubtav, vbtav, and eta_cor_slow are allocated and initialized in
  ...
  if (CS%id_PFv_bt > 0)    call safe_alloc_ptr(diag%PFu_bt, Isdq,Iedq,jsd,jed)
  ...
  if (CS%BT_cor_slow_rate > 0.0) then
    if (.NOT. query_initialized(CS%eta_cor_slow,"eta_cor",restart_CS)) then
      do j=js-1,je+1 ; do i=is-1,ie+1
        CS%eta_cor_slow(i,j) = 0.0
      enddo ; enddo
    endif
  endif
...
! endif
  ...
  call log_param(param_file, mod, "SPLIT", CS%split, &
                 "Use the split time stepping if true.")
  call log_param(param_file, mod, "BOUND_BT_CORRECTION", CS%bound_BT_corr, &
  ...
  call log_param(param_file, mod, "BT_COR_FRAC", CS%BT_frac_cor, &
                 "The fraction of the discrepancy between the barotropic \n"//&
                 "SSH and that based on the sum of the layers' thicknesses \n"//&
                 "to be corrected by pseudo mass fluxes within a timestep. \n"//&
                 "The default (for historical reasons) is 0.25, but the \n"//&
                 "more appropriate value with the iterative solver is 1.0 \n"//&
                 "if BT_cor_slow_rate = 0.", units="nondim", default=0.25)
  call log_param(param_file, mod, "BT_COR_SLOW_RATE", CS%BT_cor_slow_rate, &
                 "The rate per timestep at which the slowly evolving \n"//&
                 "pseudo mass fluxes are adjusted.  The default value is \n"//&
                 "0.125, but with the flux-form coupling and iterative \n"//&
                 "continuity solver, 0.0 would be more appropriate.", &
                 units="nondim", default=0.125)
  call log_param(param_file, mod, "BT_USE_WIDE_HALOS", CS%use_wide_halos, &
                 "If true, use wide halos and march in during the \n"//&
                 "barotropic time stepping for efficiency.", default=.true.)
  call log_param(param_file, mod, "BTHALO", bt_halo_sz, &
  ...
  call log_param(param_file, mod, "NONLINEAR_BT_CONTINUITY", &
  ...
  if (CS%Nonlinear_continuity) &
    call log_param(param_file, mod, "NONLIN_BT_CONT_UPDATE_PERIOD", &
  ...
  call log_param(param_file, mod, "RESCALE_BT_FACE_AREAS", CS%rescale_D_bt, &
  ...
  call log_param(param_file, mod, "BT_MASS_SOURCE_LIMIT", CS%eta_source_limit, &
  ...
  call log_param(param_file, mod, "DYNAMIC_SURFACE_PRESSURE", CS%dynamic_psurf, &
                 "If true, add a dynamic pressure due to a viscous ice \n"//&
                 "shelf, for instance.", default=.false.)
  if (CS%dynamic_psurf) then
    call log_param(param_file, mod, "ICE_LENGTH_DYN_PSURF", CS%ice_strength_length, &
    ...
    call log_param(param_file, mod, "DEPTH_MIN_DYN_PSURF", CS%Dmin_dyn_psurf, &
                  ...
                  default=1.0e-6)
    call log_param(param_file, mod, "CONST_DYN_PSURF", CS%const_dyn_psurf, &
  ...
  call log_param(param_file, mod, "SADOURNY", CS%Sadourny, &
  ...
  call log_param(param_file, mod, "ARITHMETIC_BT_THICK", CS%arith_hvel, &
                 "If true use the arithmetic mean layer thicknesses to \n"//&
                 "estimate the barotropic velocity. The default (false) \n"//&
                 "is to use harmonic mean thicknesses. This does not \n"//&
                 "apply if USE_BT_CONT_TYPE is true.", default=.false.)
  call log_param(param_file, mod, "HYBRID_BT_THICK", CS%hybrid_hvel, &
                 "If true, the thicknesses used to calculate the \n"//&
                 "barotropic transports vary with depth relative to the \n"//&
                 "shallowest neighboring bottom, using an arithmetic mean \n"//&
                 "for layers above the shallowest bottom, the harmonic \n"//&
                 "mean for layers below, and a weighted average for \n"//&
                 "layers that straddle that depth. The default (false) is \n"//&
                 "to use harmonic mean  thicknesses everywhere.  This \n"//&
                 "does not apply if USE_BT_CONT_TYPE is true.", default=.false.)
  call log_param(param_file, mod, "BT_STRONG_DRAG", CS%strong_drag, &
  ...
  call log_param(param_file, mod, "DTBT", dtbt_input, &
  ...
  call log_param(param_file, mod, "BEBT", CS%bebt, &
                 ...
                 units="nondim")
  call log_param(param_file, mod, "G_BT_EXTRA", CS%G_extra, &
                 "A nondimensional factor by which gtot is enhanced.", &
                 units="nondim", default=0.0)
  call log_param(param_file, mod, "SSH_EXTRA", SSH_extra, &
  ...
  call log_param(param_file, mod, "APPLY_BT_DRAG", apply_bt_drag, &
                 "If defined, bottom drag is applied within the \n"//&
                 "barotropic solver.", default=.true.)
  call log_param(param_file, mod, "CLIP_BT_VELOCITY", CS%clip_velocity, &
  ...
  if (CS%clip_velocity) &
    call log_param(param_file, mod, "MAXVEL", CS%maxvel, &
                 ...
                 "components are truncated.", units="m s-1", default=1.0e6)
  call log_param(param_file, mod, "MAXCFL_BT_CONT", CS%maxCFL_BT_cont, &
                 "The maximum permitted CFL number associated with the \n"//&
                 "barotropic accelerations from the summed velocities \n"//&
                 "times the time-derivatives of thicknesses.", units="nondim", &
                 default=0.1)
  if (CS%debug) call log_param(param_file, mod, "DEBUG", CS%debug, &
                 "If true, write out verbose debugging data.", default=.false.)
  if (CS%debug_bt) call log_param(param_file, mod, "DEBUG_BT", CS%debug_bt, &
                 "If true, write out verbose debugging data within the \n"//&
                 "barotropic time-stepping loop. The data volume can be \n"//&
                 "quite large if this is true.", default=.false.)
...
subroutine barotropic_end(CS)
  type(barotropic_CS), pointer :: CS
  DEALLOC(CS%frhatu) ; DEALLOC(CS%frhatv)
  DEALLOC(CS%IDatu) ; DEALLOC(CS%IDatv)
  DEALLOC(CS%Datu_res) ; DEALLOC(CS%Datv_res)
  DEALLOC(CS%ubtav) ; DEALLOC(CS%vbtav)
  DEALLOC(CS%eta_cor) ; DEALLOC(CS%eta_source)
  DEALLOC(CS%ua_polarity) ; DEALLOC(CS%va_polarity)
  if (CS%bound_BT_corr) then
    DEALLOC(CS%eta_cor_bound)
  endif
  if (CS%BT_cor_slow_rate > 0.0) then
    DEALLOC(CS%eta_cor_slow)
  endif

  deallocate(CS)
end subroutine barotropic_end

subroutine register_barotropic_restarts(G, param_file, CS, restart_CS)
  type(ocean_grid_type), intent(in) :: G
  type(param_file_type), intent(in) :: param_file
  type(barotropic_CS),   pointer    :: CS
  type(GOLD_restart_CS),  pointer    :: restart_CS
! This subroutine is used to register any fields from GOLD_barotropic.F90
...
!  (in)      restart_CS - A pointer to the restart control structure.
  ...
  if (associated(CS)) then
    call GOLD_error(WARNING, "register_barotropic_restarts called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)

  ALLOC(CS%ubtav(Isdq:Iedq,jsd:jed))      ; CS%ubtav(:,:) = 0.0
  ALLOC(CS%vbtav(isd:ied,Jsdq:Jedq))      ; CS%vbtav(:,:) = 0.0
  ALLOC(CS%ubt_IC(Isdq:Iedq,jsd:jed))     ; CS%ubt_IC(:,:) = 0.0
  ALLOC(CS%vbt_IC(isd:ied,Jsdq:Jedq))     ; CS%vbt_IC(:,:) = 0.0
  ...
  slow_rate = 0.125 ; call read_param(param_file,"BT_COR_SLOW_RATE", slow_rate)
  if (slow_rate > 0.0) then
    ALLOC(CS%eta_cor_slow(isd:ied,jsd:jed)) ; CS%eta_cor_slow(:,:) = 0.0
    if (G%Boussinesq) then
      vd(1) = vardesc("eta_cor","Slowly varying correction to barotropic height equation", &
                      'h','1','s', "meter second-1", 'd')
    else
      vd(1) = vardesc("eta_cor","Slowly varying correction to barotropic height equation", &
                      'h','1','s', "kilogram meter-2 second-1", 'd')
    endif
    call register_restart_field(CS%eta_cor_slow, CS%eta_cor_slow, vd(1), &
                                .false., restart_CS)
  endif

end subroutine register_barotropic_restarts

end module GOLD_barotropic
