...
public GOLD_grid_init, GOLD_grid_end
public get_flux_units, get_thickness_units, get_tr_flux_units

type, public :: ocean_grid_type
  type(GOLD_domain_type), pointer :: Domain => NULL()
  type(GOLD_domain_type), pointer :: Domain_aux => NULL()
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!    
  ...
  real, pointer, dimension(:) :: &
    gridlath => NULL(), gridlatq => NULL() ! The latitude of h or q points for purposes of output.
                        ! On many grids these are the same as geolath & geolatq.
  real, pointer, dimension(:) :: &
    gridlonh => NULL(), gridlonq => NULL() ! The longitude of h or q points for purposes of output.
...
subroutine GOLD_grid_init(grid, param_file)
  type(ocean_grid_type), intent(inout) :: grid
  type(param_file_type), intent(in)    :: param_file
! Arguments: grid - The ocean's grid structure.
!  (in)      param_file - A structure indicating the open file to parse for
!                         model parameter values.
  character(len=128) :: version = '$Id: GOLD_grid.F90,v 1.1.2.12 2011/01/28 16:20:03 Robert.Hallberg Exp $'
...
#ifdef STATIC_MEMORY
  nz = NZ ! This is transferring the macro NZ to the variable nz.
  call read_param(param_file,"NZ",nz)
  if (nz /= NZ) call GOLD_error(FATAL, "GOLD_grid_init: " // &
       "Mismatched number of layers NZ between GOLD_memory.h and param_file")
#else
  call read_param(param_file, "NZ", nz, .true.)
#endif
  ...
  call read_param(param_file,"G_EARTH", grid%g_Earth, .true.)
  call read_param(param_file,"RHO_0",grid%Rho0,.true.)

  grid%Boussinesq = .true.
  call read_param(param_file, "BOUSSINESQ", grid%Boussinesq)

  isd = grid%isd ; ied = grid%ied ; jsd = grid%jsd ; jed = grid%jed
  Isdq = grid%Isdq ; Iedq = grid%Iedq ; Jsdq = grid%Jsdq ; Jedq = grid%Jedq
  grid%Angstrom_z = EPSILON
  call read_param(param_file, "ANGSTROM", grid%Angstrom_z)
  ...
  grid%bathymetry_at_vel = .false.
  call read_param(param_file, "BATHYMETRY_AT_VEL", grid%bathymetry_at_vel)
...
!    grid%H_to_m = 1.0 / Rho0
    grid%H_to_kg_m2 = 1.0
    call read_param(param_file,"H_TO_KG_M2",grid%H_to_kg_m2)
  ...
  call log_version(param_file, "GOLD_grid", version, tagname, &
                   "Parameters providing information about the vertical grid.")
  call log_param(param_file, "GOLD", "G_EARTH", grid%g_Earth, &
                 "The gravitational acceleration of the Earth.", units="m s-2")
  call log_param(param_file, "GOLD", "RHO_0", grid%Rho0, &
                 "The mean ocean density used with BOUSSINESQ true to \n"//&
                 "calculate accelerations and the mass for conservation \n"//&
                 "properties, or with BOUSSINSEQ false to convert some \n"//&
                 "parameters from vertical units of m to kg m-2.", &
                 units="kg m-3")
  call log_param(param_file, "GOLD_grid", "BOUSSINESQ", grid%Boussinesq, &
                 "If true, make the Boussinesq approximation.", default=.true.)
  call log_param(param_file, "GOLD_grid", "ANGSTROM", grid%Angstrom_z, &
                 "The minumum layer thickness, usually one-Angstrom.", &
                 units="m", default=EPSILON)
  if (.not.grid%Boussinesq) &
    call log_param(param_file, "GOLD_grid", "H_TO_KG_M2", grid%H_to_kg_m2,&
                 "A constant that translates thicknesses from the model's \n"//&
                 "internal units of thickness to kg m-2.", units="kg m-2 H-1", &
                 default=1.0)
  call log_param(param_file, "GOLD_grid", "M to THICKNESS", grid%m_to_H)
  call log_param(param_file, "GOLD_grid", "BATHYMETRY_AT_VEL", grid%bathymetry_at_vel, &
                 "If true, there are separate values for the basin depths \n"//&
                 "at velocity points.  Otherwise the effects of of \n"//&
                 "topography are entirely determined from thickness points.", &
                 default=.false.)
  call log_param(param_file, "GOLD_grid", "NZ", nz, &
                 "The number of model layers.", units="nondim")

end subroutine GOLD_grid_init
...
!  (ret)     get_tr_flux_units - The model's flux units for a tracer.
  ...
  if (cnt == 0) call GOLD_error(FATAL, "get_tr_flux_units: One of the three "//&
    "arguments tr_units, tr_vol_conc_units, or tr_mass_conc_units "//&
    "must be present.")
  if (cnt > 1) call GOLD_error(FATAL, "get_tr_flux_units: Only one of "//&
...
end subroutine GOLD_grid_end

end module GOLD_grid
