...
!                        defined.
  ...
  character(len=128) :: version = '$Id: GOLD_domains.F90,v 13.0.2.10.2.32 2011/05/11 19:26:40 Robert.Hallberg Exp $'
  ...
  call read_param(param_file,"REENTRANT_X",reentrant_x,.true.)
  call read_param(param_file,"REENTRANT_Y",reentrant_y)
  call read_param(param_file,"TRIPOLAR_N",tripolar_N)
  
  GOLD_dom%nonblocking_updates = .false.
  call read_param(param_file,"NONBLOCKING_UPDATES",GOLD_dom%nonblocking_updates)
#ifdef FORBID_NONBLOCKING_UPDATES
  ...
  GOLD_dom%nx_halo = NX_HALO ; call read_param(param_file,"NXHALO",GOLD_dom%nx_halo)
  GOLD_dom%ny_halo = NY_HALO ; call read_param(param_file,"NYHALO",GOLD_dom%ny_halo)
...
#ifdef STATIC_MEMORY
  GOLD_dom%nxtot = NXTOT ; GOLD_dom%nytot = NYTOT
  call read_param(param_file,"NXTOT",GOLD_dom%nxtot)
  call read_param(param_file,"NYTOT",GOLD_dom%nytot)
...
#else
  call read_param(param_file,"NXTOT",GOLD_dom%nxtot,.true.)
  call read_param(param_file,"NYTOT",GOLD_dom%nytot,.true.)
#endif
...
!  call GOLD_error(FATAL,"GOLD_domains: layout in meridional axis has too few processors.")
  ...
  io_layout(:) = (/ 0, 0 /)
  call read_param(param_file, "NXPROC_IO", io_layout(1))
  call read_param(param_file, "NYPROC_IO", io_layout(2))
...
! is used.
  ...
  call log_param(param_file, mod, "REENTRANT_X", reentrant_x, &
                 "If true, the domain is zonally reentrant.")
  call log_param(param_file, mod, "REENTRANT_Y", reentrant_y, &
                 "If true, the domain is meridionally reentrant.", &
                 default=.false.)
  call log_param(param_file, mod, "TRIPOLAR_N", tripolar_N, &
  ...
  call log_param(param_file, mod, "NONBLOCKING_UPDATES", GOLD_dom%nonblocking_updates, &
  ...
  if (present(min_halo)) then
    call log_param(param_file, mod, "NXHALO min_halo", GOLD_dom%nx_halo, &
                 "The number of halo points in the x-direction in a call \n"//&
                 "with a minimum halo size.", default=NX_HALO)
    call log_param(param_file, mod, "NYHALO min_halo", GOLD_dom%nx_halo, &
                 "The number of halo points in the y-direction in a call \n"//&
                 "with a minimum halo size.", default=NY_HALO)
  else
    call log_param(param_file, mod, "NXHALO", GOLD_dom%nx_halo, &
    ...
    call log_param(param_file, mod, "NYHALO", GOLD_dom%ny_halo, &
                 "The number of halo points on each side in the \n"//&
                 "y-direction.  With STATIC_MEMORY this is set as NY_HALO \n"//&
                 "in GOLD_memory.h at compile time; without STATIC_MEMORY \n"//&
                 "the default is NY_HALO in GOLD_memory.h.", default=NY_HALO)
  endif
  call log_param(param_file, mod, "NXTOT", GOLD_dom%nxtot, &
                 "The total number of thickness grid points in the \n"//&
                 "x-direction in the physical domain. With STATIC_MEMORY \n"//&
                 "this is set in GOLD_memory.h at compile time.")
  call log_param(param_file, mod, "NYTOT", GOLD_dom%nytot, &
                 "The total number of thickness grid points in the \n"//&
                 "x-direction in the physical domain. With STATIC_MEMORY \n"//&
                 "this is set in GOLD_memory.h at compile time.")
  call log_param(param_file, mod, "NXPROC", GOLD_dom%layout(1), &
                 "The number of processors in the x-direction. With \n"//&
                 "STATIC_MEMORY this is set in GOLD_memory.h at compile time.")
  call log_param(param_file, mod, "NYPROC", GOLD_dom%layout(2), &
                 "The number of processors in the x-direction. With \n"//&
                 "STATIC_MEMORY this is set in GOLD_memory.h at compile time.")
  call log_param(param_file, mod, "NXPROC_IO", GOLD_dom%io_layout(1), &
                 "The number of processors used for I/O in the \n"//&
                 "x-direction, or 0 to equal NXPROC.", default=0)
  call log_param(param_file, mod, "NYPROC_IO", GOLD_dom%io_layout(2), &
                 "The number of processors used for I/O in the \n"//&
