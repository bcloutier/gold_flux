...
use GOLD_domains, only : sum_across_PEs, pe_here, num_pes
...
!                 for this module.
  integer :: new_cputime   ! The CPU time returned by SYSTEM_CLOCK
  character(len=128) :: version = '$Id: GOLD_write_cputime.F90,v 1.1.2.1.2.4 2010/08/13 21:55:03 rwh Exp $'
  ...
  CS%maxcpu = -1.0 ; CS%CPUfile = "CPU_stats"
  call read_param(param_file,"MAXCPU",CS%maxcpu)
  call read_param(param_file,"CPU_TIME_FILE",CS%CPUfile)
  ...
  call log_param(param_file, mod, "MAXCPU", CS%maxcpu, &
  ...
  call log_param(param_file, mod, "CPU_TIME_FILE", CS%CPUfile, &
