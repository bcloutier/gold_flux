...
!  (in)      restart_CS - A pointer to the restart control structure.
  ...
  character(len=128) :: version = '$Id: GOLD_OCMIP2_CFC.F90,v 1.1.2.2.2.17 2011/02/17 23:17:56 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_OCMIP2_CFC" ! This module's name.
  ...
  if (associated(CS)) then
    call GOLD_error(WARNING, "register_OCMIP2_CFC called with an "// &
  ...
  if ((CS%ind_cfc_11_flux < 0) .or. (CS%ind_cfc_11_flux < 0)) then
    ...
    call GOLD_ERROR(WARNING, "CFCs are currently only set up to be run in " // &
  ...
  CS%diag => diag
  CS%IC_file = ""  ; call read_param(param_file,"CFC_IC_FILE",CS%IC_file)
  IC_file = CS%IC_file
  CS%Z_IC_file = .false. ; call read_param(param_file,"CFC_IC_FILE_IS_Z",CS%Z_IC_file)
...
!-----------------------------------------------------------------------
  ...
  CS%a1_11 = a11_dflt(1) ; call read_param(param_file, "CFC11_A1", CS%a1_11)
  CS%a2_11 = a11_dflt(2) ; call read_param(param_file, "CFC11_A2", CS%a2_11)
  CS%a3_11 = a11_dflt(3) ; call read_param(param_file, "CFC11_A3", CS%a3_11)
  CS%a4_11 = a11_dflt(4) ; call read_param(param_file, "CFC11_A4", CS%a4_11)

  CS%a1_12 = a12_dflt(1) ; call read_param(param_file, "CFC12_A1", CS%a1_12)
  CS%a2_12 = a12_dflt(2) ; call read_param(param_file, "CFC12_A2", CS%a2_12)
  CS%a3_12 = a12_dflt(3) ; call read_param(param_file, "CFC12_A3", CS%a3_12)
  CS%a4_12 = a12_dflt(4) ; call read_param(param_file, "CFC12_A4", CS%a4_12)
...
!-----------------------------------------------------------------------
  ...
  CS%d1_11 = d11_dflt(1) ; call read_param(param_file, "CFC11_D1", CS%d1_11)
  CS%d2_11 = d11_dflt(2) ; call read_param(param_file, "CFC11_D2", CS%d2_11)
  CS%d3_11 = d11_dflt(3) ; call read_param(param_file, "CFC11_D3", CS%d3_11)
  CS%d4_11 = d11_dflt(4) ; call read_param(param_file, "CFC11_D4", CS%d4_11)
  CS%e1_11 = e11_dflt(1) ; call read_param(param_file, "CFC11_E1", CS%e1_11)
  CS%e2_11 = e11_dflt(2) ; call read_param(param_file, "CFC11_E2", CS%e2_11)
  CS%e3_11 = e11_dflt(3) ; call read_param(param_file, "CFC11_E3", CS%e3_11)

  CS%d1_12 = d12_dflt(1) ; call read_param(param_file, "CFC12_D1", CS%d1_12)
  CS%d2_12 = d12_dflt(2) ; call read_param(param_file, "CFC12_D2", CS%d2_12)
  CS%d3_12 = d12_dflt(3) ; call read_param(param_file, "CFC12_D3", CS%d3_12)
  CS%d4_12 = d12_dflt(4) ; call read_param(param_file, "CFC12_D4", CS%d4_12)
  CS%e1_12 = e12_dflt(1) ; call read_param(param_file, "CFC12_E1", CS%e1_12)
  CS%e2_12 = e12_dflt(2) ; call read_param(param_file, "CFC12_E2", CS%e2_12)
  CS%e3_12 = e12_dflt(3) ; call read_param(param_file, "CFC12_E3", CS%e3_12)

  call log_version(param_file, mod, version, tagname, "")
  call log_param(param_file, mod, "INPUTDIR/CFC_IC_FILE", CS%IC_file)
  call log_param(param_file, mod, "CFC_IC_FILE", IC_file, &
                 "The file in which the CFC initial values can be \n"//&
                 "found, or an empty string for internal initialization.", &
                 default=" ")
  call log_param(param_file, mod, "CFC_IC_FILE_IS_Z", CS%Z_IC_file, &
  ...
  call log_param(param_file, mod, "CFC11_A1", CS%a1_11, &
                 "A coefficient in the Schmidt number of CFC11.", &
                 units="nondim", default=a11_dflt(1))
  call log_param(param_file, mod, "CFC11_A2", CS%a2_11, &
                 "A coefficient in the Schmidt number of CFC11.", &
                 units="degC-1", default=a11_dflt(2))
  call log_param(param_file, mod, "CFC11_A3", CS%a3_11, &
                 "A coefficient in the Schmidt number of CFC11.", &
                 units="degC-2", default=a11_dflt(3))
  call log_param(param_file, mod, "CFC11_A4", CS%a4_11, &
                 "A coefficient in the Schmidt number of CFC11.", &
                 units="degC-3", default=a11_dflt(4))
  call log_param(param_file, mod, "CFC12_A1", CS%a1_12, &
                 "A coefficient in the Schmidt number of CFC12.", &
                 units="nondim", default=a12_dflt(1))
  call log_param(param_file, mod, "CFC12_A2", CS%a2_12, &
                 "A coefficient in the Schmidt number of CFC12.", &
                 units="degC-1", default=a12_dflt(2))
  call log_param(param_file, mod, "CFC12_A3", CS%a3_12, &
                 "A coefficient in the Schmidt number of CFC12.", &
                 units="degC-2", default=a12_dflt(3))
  call log_param(param_file, mod, "CFC12_A4", CS%a4_12, &
                 "A coefficient in the Schmidt number of CFC12.", &
                 units="degC-3", default=a12_dflt(4))

  call log_param(param_file, mod, "CFC11_D1", CS%d1_11, &
                 "A coefficient in the solubility of CFC11.", &
                 units="none", default=d11_dflt(1))
  call log_param(param_file, mod, "CFC11_D2", CS%d2_11, &
                 "A coefficient in the solubility of CFC11.", &
                 units="hK", default=d11_dflt(2))
  call log_param(param_file, mod, "CFC11_D3", CS%d3_11, &
                 "A coefficient in the solubility of CFC11.", &
                 units="none", default=d11_dflt(3))
  call log_param(param_file, mod, "CFC11_D4", CS%d4_11, &
                 "A coefficient in the solubility of CFC11.", &
                 units="hK-2", default=d11_dflt(4))
  call log_param(param_file, mod, "CFC11_E1", CS%e1_11, &
                 "A coefficient in the solubility of CFC11.", &
                 units="PSU-1", default=e11_dflt(1))
  call log_param(param_file, mod, "CFC11_E2", CS%e2_11, &
                 "A coefficient in the solubility of CFC11.", &
                 units="PSU-1 hK-1", default=e11_dflt(2))
  call log_param(param_file, mod, "CFC11_E3", CS%e3_11, &
                 "A coefficient in the solubility of CFC11.", &
                 units="PSU-1 hK-2", default=e11_dflt(3))

  call log_param(param_file, mod, "CFC12_D1", CS%d1_12, &
                 "A coefficient in the solubility of CFC12.", &
                 units="none", default=d12_dflt(1))
  call log_param(param_file, mod, "CFC12_D2", CS%d2_12, &
                 "A coefficient in the solubility of CFC12.", &
                 units="hK", default=d12_dflt(2))
  call log_param(param_file, mod, "CFC12_D3", CS%d3_12, &
                 "A coefficient in the solubility of CFC12.", &
                 units="none", default=d12_dflt(3))
  call log_param(param_file, mod, "CFC12_D4", CS%d4_12, &
                 "A coefficient in the solubility of CFC12.", &
                 units="hK-2", default=d12_dflt(4))
  call log_param(param_file, mod, "CFC12_E1", CS%e1_12, &
                 "A coefficient in the solubility of CFC12.", &
                 units="PSU-1", default=e12_dflt(1))
  call log_param(param_file, mod, "CFC12_E2", CS%e2_12, &
                 "A coefficient in the solubility of CFC12.", &
                 units="PSU-1 hK-1", default=e12_dflt(2))
  call log_param(param_file, mod, "CFC12_E3", CS%e3_12, &
                 "A coefficient in the solubility of CFC12.", &
...
!                            in depth space.
  ...
  CS%id_CFC11 = register_diag_field("ocean_model", trim(name), G%axeshl, &
  ...
  CS%id_CFC12 = register_diag_field("ocean_model", trim(name), G%axeshl, &
      day, trim(longname) , trim(units))
  ...
  do m=1,NTR
    ...
    elseif (m==2) then
      ...
      call GOLD_error(FATAL,"initialize_OCMIP2_CFC is only set up to work"//&
                           "with NTR <= 2.")
    endif
...
subroutine init_tracer_CFC(h, tr, tr_desc, land_val, IC_val, G, CS)
  ...
  if (len_trim(CS%IC_file) > 0) then
    ...
    if (.not.file_exists(CS%IC_file, G%Domain)) &
      call GOLD_error(FATAL, "initialize_OCMIP2_CFC: Unable to open "//CS%IC_file)
    if (CS%Z_IC_file) then
      OK = tracer_Z_init(tr, h, CS%IC_file, tr_desc%name, G)
      if (.not.OK) then
        OK = tracer_Z_init(tr, h, CS%IC_file, trim(tr_desc%name)//"_z", G)
        if (.not.OK) call GOLD_error(FATAL,"initialize_OCMIP2_CFC: "//&
...
!     h_new[k] = h_old[k] + ea[k] - eb[k-1] + eb[k] - ea[k+1]
  ...
  call tracer_vertdiff(h_old, ea, eb, dt, CFC11, G, sfc_flux=CFC11_flux)
  call tracer_vertdiff(h_old, ea, eb, dt, CFC12, G, sfc_flux=CFC12_flux)
