...
function register_advection_test_tracer(G, param_file, CS, diag, tr_adv_CSp, &
                                      restart_CS)
...
!  (in)      restart_CS - A pointer to the restart control structure.
  ...
  character(len=128) :: version = '$Id: advection_test_tracer.F90,v 1.1.2.1 2011/10/12 14:18:40 Alistair.Adcroft Exp $'
  ...
  if (associated(CS)) then
    call GOLD_error(WARNING, "register_advection_test_tracer called with an "// &
  ...
  call read_param(param_file,"ADVECTION_TEST_TRACER_IC_FILE",tracer_IC_file)
  CS%x_origin = 0.; call read_param(param_file,"ADVECTION_TEST_X_ORIGIN",CS%x_origin)
  CS%y_origin = 0.; call read_param(param_file,"ADVECTION_TEST_Y_ORIGIN",CS%y_origin)
  CS%x_width = 0.; call read_param(param_file,"ADVECTION_TEST_X_WIDTH",CS%x_width)
  CS%y_width = 0.; call read_param(param_file,"ADVECTION_TEST_Y_WIDTH",CS%y_width)
  ...
  call log_param(param_file, mod, "ADVECTION_TEST_X_ORIGIN", CS%x_origin, &
        "The x-coorindate of the center of the test-functions.\n", default=0.)
  call log_param(param_file, mod, "ADVECTION_TEST_Y_ORIGIN", CS%y_origin, &
        "The y-coorindate of the center of the test-functions.\n", default=0.)
  call log_param(param_file, mod, "ADVECTION_TEST_X_WIDTH", CS%x_width, &
        "The x-width of the test-functions.\n", default=0.)
  call log_param(param_file, mod, "ADVECTION_TEST_Y_WIDTH", CS%y_width, &
        "The y-width of the test-functions.\n", default=0.)
  call log_param(param_file, mod, "ADVECTION_TEST_TRACER_IC_FILE", tracer_IC_file, &
                 "The name of a file from which to read the initial \n"//&
                 "conditions for the tracers, or blank to initialize \n"//&
                 "them internally.", default=" ")
  if (len_trim(tracer_IC_file) >= 1) &
    call log_param(param_file, mod, "ADVECTION_TEST_TRACER_IC_FILE", &
                   CS%tracer_IC_file)
