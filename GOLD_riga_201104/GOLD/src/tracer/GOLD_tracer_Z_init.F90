...
!*  GOLD vertical grid from a depth- (or z*-) space file.              *
!*                                                                     *
!*                                                                     *
...
use GOLD_diag_to_Z, only : find_overlap, find_limited_slope
...
!  (in,opt)  land_val - The value to use to fill in land points.
  ...
  character(len=128) :: version = '$Id: GOLD_tracer_Z_init.F90,v 1.1.2.2 2009/12/04 22:42:55 rwh Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_tracer_Z_init" ! This module's name.
    ...
    do j=js,je ; do i=is,ie
      ...
      elseif (abs(tr_in(i,j,1) - missing_val) <= 1e-6*abs(missing_val)) then
        ...
        if (zero_surface) then
          call GOLD_error(WARNING, "tracer_Z_init: Missing value of "// &
                trim(tr_name)//" found in an ocean point at "//trim(loc_msg)// &
                " in "//trim(filename) )
          tr_in(i,j,1) = 0.0
        else
          call GOLD_error(FATAL, "tracer_Z_init: Missing value of "// &
                trim(tr_name)//" found in an ocean point at "//trim(loc_msg)// &
                " in "//trim(filename) )
        endif
      endif
...
!  (inout)   missing - The missing value, if one has been found.
  ...
  mod = "GOLD_tracer_Z_init read_Z_edges: "
  tr_msg = trim(tr_name)//" in "//trim(filename)

  status = NF90_OPEN(filename, NF90_NOWRITE, ncid);
  if (status /= NF90_NOERR) then
    call GOLD_error(WARNING,mod//" Difficulties opening "//trim(filename)//&
  ...
  if (status /= NF90_NOERR) then
    call GOLD_error(WARNING,mod//" Difficulties finding variable "//&
  ...
  if (status /= NF90_NOERR) then
    call GOLD_ERROR(WARNING,mod//" cannot inquire about "//trim(tr_msg))
  elseif ((ndim < 3) .or. (ndim > 4)) then
    call GOLD_ERROR(WARNING,mod//" "//trim(tr_msg)//&
  ...
  if (status /= NF90_NOERR) then
    call GOLD_ERROR(WARNING,mod//" cannot inquire about dimension(3) of "//&
                    trim(tr_msg))
  endif
  ...
  if (status /= NF90_NOERR) then
    call GOLD_mesg(mod//" "//trim(dim_msg)//&
    ...
    if (status /= NF90_NOERR) then
      call GOLD_error(WARNING,mod//" Difficulties finding edge variable "//&
  ...
  if (has_edges) then
    ...
    if (status /= NF90_NOERR) then
      call GOLD_error(WARNING,mod//" Difficulties reading variable "//&
          trim(dim_msg)//" - "//trim(NF90_STRERROR(status)))
      nz_out = -1 ; status = NF90_CLOSE(ncid) ; return
    endif
  else
  ...
  if (.not.monotonic) &
    call GOLD_error(WARNING,mod//" "//trim(dim_msg)//" is not monotonic.")

end subroutine read_Z_edges


end module GOLD_tracer_Z_init
