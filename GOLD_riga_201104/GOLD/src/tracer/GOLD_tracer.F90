...
use GOLD_cpu_clock, only : CLOCK_MODULE, CLOCK_ROUTINE
...
use GOLD_domains, only : pass_var, pass_vector, sum_across_PEs, max_across_PEs
...
use GOLD_variables, only : ocean_OBC_type, thermo_var_ptrs, OBC_FLATHER_E
use GOLD_variables, only : OBC_FLATHER_W, OBC_FLATHER_N, OBC_FLATHER_S
...
type, public :: advect_tracer_CS ; private
  ...
  logical :: Riga_diffuse_bugs  ! If true, use code with bugs from the Riga
                            ! release.  These bugs are only tickled when a
                            ! mixed or buffer layer have _exactly_ the same
                            ! density as a neighboring mixed or buffer layer or
                            ! an interior layer, about a 1 in 10^16 chance.
  integer :: ntr = 0        ! The number of registered tracers.
  integer :: nkml           ! The number of sublayer in the bulk mixed layer.
...
!                       tracer cells, in the same units as tr (CONC).
  ...
  if (CS%ntr>=MAX_FIELDS) then
    write(mesg,'("Increase MAX_FIELDS in GOLD_memory.h to at least ",I3," to allow for &
        &all the tracers being registered via register_tracer.")') CS%ntr+1
    call GOLD_error(FATAL,"GOLD register_tracer: "//mesg)
  endif
  CS%ntr = CS%ntr + 1
  ntr = CS%ntr
...
!  (in)      OBC_in_v - The value of the tracer at inflows through v-faces of
  ...
  if (.not. associated(CS)) call GOLD_error(FATAL, "add_tracer_OBC_values :"// &
  ...
  if (m <= CS%ntr) then
    ...
    call GOLD_error(FATAL, "GOLD_tracer: register_tracer must be called for "//&
...
!                   in units of CONC m3 s-1.
  ...
  if (.not. associated(CS)) call GOLD_error(FATAL, "add_tracer_diagnostics: "// &
  ...
  if (CS%Diffuse_ML_interior) then
    ...
    if (write_warning .and. is_root_pe()) call GOLD_error(WARNING, &
...
!                 advect_tracer_init.
  ...
  if (.not. associated(CS)) call GOLD_error(FATAL, "GOLD_tracer: "// &
...
!                 mixed layer and the interior.
  ...
  if (CS%debug) call GOLD_tracer_chksum("Before tracer diffusion ", Tr, ntr, G)
  ...
  do itt=1,num_itts
    ...
    do k=1,nz
      ...
      if (CS%Diffuse_ML_interior) then
        ...
        if ((k>CS%nkml) .and. (k<=tv%nk_Rml)) cycle
      endif
  ...
  if (CS%Diffuse_ML_interior) then
    if (CS%debug) call GOLD_tracer_chksum("Before epipycnal diff ", Tr, ntr, G)
  ...
  if (CS%debug) call GOLD_tracer_chksum("After tracer diffusion ", Tr, ntr, G)
...
!  (in)      num_itts - The number of iterations to use, usually 1.
  ...
  real, dimension(SZI_(G), SZJ_(G), max(1,tv%nk_RML)) :: &
  ...
  if (associated(tv%eqn_of_state)) then
    ...
    do k=1,nkmb ; do j=js-2,je+2 ; do i=is-2,ie+2
      rho_coord(i,j,k) = tv%Rml(i,j,k)
    enddo ; enddo ; enddo
  endif
    ...
    do k=1,nPu(I,j)
      ...
      if (left_set(k)) then ! Add the contributing thicknesses on the right.
        if (deep_wt_Ru(j)%p(I,k) < 1.0) then
          ...
          if (CS%Riga_diffuse_bugs) then
            hP_Ru(j)%p(I,k) = 0.5*h_srt(i,kL,j) * h_supply_frac_R(k)
          else
      ...
      if (right_set(k)) then ! Add the contributing thicknesses on the left.
        if (deep_wt_Lu(j)%p(I,k) < 1.0) then
          ...
          if (CS%Riga_diffuse_bugs) then
            hP_Lu(j)%p(I,k) = 0.5*h_srt(i,kL,j) * h_supply_frac_L(k)
          else
    ...
    do k=1,nPv(i,J)
      ...
      if (left_set(k)) then ! Add the contributing thicknesses on the right.
        if (deep_wt_Rv(J)%p(i,k) < 1.0) then
          ...
          if (CS%Riga_diffuse_bugs) then
            hP_Rv(J)%p(i,k) = 0.5*h_srt(i,kL,j) * h_supply_frac_R(k)
          else
      ...
      if (right_set(k)) then ! Add the contributing thicknesses on the left.
        if (deep_wt_Lv(J)%p(i,k) < 1.0) then
          ...
          if (CS%Riga_diffuse_bugs) then
            hP_Lv(J)%p(i,k) = 0.5*h_srt(i,kL,j) * h_supply_frac_L(k)
          else
...
subroutine GOLD_tracer_chksum(mesg, Tr, ntr, G)
...
!  (in)      G - The ocean's grid structure.
  ...
  enddo
end subroutine GOLD_tracer_chksum
...
!                 for this module
  ...
  character(len=128) :: version = '$Id: GOLD_tracer.F90,v 13.0.2.4.2.49 2011/07/27 21:32:15 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_tracer" ! This module's name.
  character(len=256) :: mesg    ! Message for error messages.

  if (.not.associated(CS)) then ; allocate(CS)
  else ; return ; endif

  CS%KhTr = 0.0; call read_param(param_file,"KHTR",CS%KhTr)
  CS%KhTr_Slope_Cff = 0.; call read_param(param_file,"KHTR_SLOPE_CFF",CS%KhTr_Slope_Cff)
  CS%KhTr_Min = 0.0 ; call read_param(param_file,"KHTR_MIN",CS%KhTr_Min)
  CS%KhTr_Max = 0.0 ; call read_param(param_file,"KHTR_MAX",CS%KhTr_Max)
  CS%KhTr_passivity_min = 0.5; call read_param(param_file,"KHTR_PASSIVITY_MIN",CS%KhTr_passivity_min)
  CS%KhTr_passivity_coeff = 0.; call read_param(param_file,"KHTR_PASSIVITY_COEFF",CS%KhTr_passivity_coeff)
  call read_param(param_file,"DT",CS%dt,.true.)
  CS%Diffuse_ML_interior = .false. ; CS%check_diffusive_CFL = .false.
  CS%ML_KhTR_scale = 1.0
  call read_param(param_file,"DIFFUSE_ML_TO_INTERIOR",CS%Diffuse_ML_interior)
  call read_param(param_file,"ML_KHTR_SCALE",CS%ML_KhTR_scale)
  call read_param(param_file,"CHECK_DIFFUSIVE_CFL",CS%check_diffusive_CFL)
  CS%nkml = 1 ; call read_param(param_file,"NKML",CS%nkml)
  CS%Riga_diffuse_bugs = .false.
  call read_param(param_file,"RIGA_TRACER_DIFFUSE_BUGS",CS%Riga_diffuse_bugs)
  ...
  if (init_calls == 1) then
    ...
    call log_param(param_file, mod, "KHTR", CS%KhTr, &
                 "The background along-isopycnal tracer diffusivity.", &
                 units="m2 s-1", default=0.0)
    call log_param(param_file, mod, "KHTR_SLOPE_CFF", CS%KhTr_Slope_Cff, &
    ...
    call log_param(param_file, mod, "KHTR_MIN", CS%KhTr_Min, &
                 "The minimum along-isopycnal tracer diffusivity.", &
                 units="m2 s-1", default=0.0)
    call log_param(param_file, mod, "KHTR_MAX", CS%KhTr_Max, &
                 "The maximum along-isopycnal tracer diffusivity.", &
                 units="m2 s-1", default=0.0)
    call log_param(param_file, mod, "KHTR_PASSIVITY_COEFF", CS%KhTr_passivity_coeff, &
                 ...
                 units="nondim", default=0.0)
    call log_param(param_file, mod, "KHTR_PASSIVITY_MIN", CS%KhTr_passivity_min, &
    ...
    call log_param(param_file, mod, "DIFFUSE_ML_TO_INTERIOR", CS%Diffuse_ML_interior, &
                 "If true, enable epipycnal mixing between the surface \n"//&
                 "boundary layer and the interior.", default=.false.)
    call log_param(param_file, mod, "CHECK_DIFFUSIVE_CFL", CS%check_diffusive_CFL, &
    ...
    if (CS%Diffuse_ML_interior) then
      call log_param(param_file, mod, "ML_KHTR_SCALE", CS%ML_KhTR_scale, &
                 "With Diffuse_ML_interior, the ratio of the truly \n"//&
                 "horizontal diffusivity in the mixed layer to the \n"//&
                 "epipycnal diffusivity.  The valid range is 0 to 1.", &
                 units="nondim", default=1.0)
      call log_param(param_file, mod, "RIGA_TRACER_DIFFUSE_BUGS", CS%Riga_diffuse_bugs, &
                 "If true, use tracer diffusion code with known indexing \n"//&
                 "bugs that arise when a mixed or buffer layer has \n"//&
                 "exactly the same density as a neighboring layer (about \n"//&
                 "1 time in 10^16 grid-point time-steps.", default=.false.)
    endif
    ...
    if (is_root_pe()) call GOLD_error(WARNING,"GOLD_tracer"//mesg)
  endif
