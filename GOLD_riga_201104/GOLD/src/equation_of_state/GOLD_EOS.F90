...
use GOLD_EOS_Wright
use GOLD_EOS_UNESCO
use GOLD_TFreeze, only : calculate_TFreeze_linear, calculate_TFreeze_Millero
use GOLD_error_handler, only : GOLD_error, FATAL, GOLD_mesg
...
public calculate_compress, calculate_density
...
! *====================================================================*
  ...
  if (.not.associated(EOS)) call GOLD_error(FATAL, &
...
! *====================================================================*
  character(len=128) :: version = '$Id: GOLD_EOS.F90,v 13.0.2.4.2.12 2011/09/19 16:10:35 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_EOS" ! This module's name.
  real :: Rho_T0_S0_def, dRho_dT_def, dRho_dS_def ! Default values.
  real :: TFr_S0_P0_def, dTFr_dS_def, dTFr_dp_def ! Default values.
  character*(24) :: tmpstr

  if (.not.associated(EOS)) allocate(EOS)

  tmpstr = EOS_DEFAULT
  call read_param(param_file,"EQN_OF_STATE",tmpstr,.false.)
  ...
  select case (tmpstr)
    ...
    case default
      call GOLD_error(FATAL, "interpret_eos_selection: #DEFINE EQN_OF_STATE "//&
                              trim(tmpstr) // "in input file is invalid.")
  end select
  call GOLD_mesg('interpret_eos_selection: equation of state set to "' // &
                trim(tmpstr)//'"', 5)

  if (EOS%form_of_EOS == EOS_LINEAR) then
    Rho_T0_S0_def = 1000.0 ; dRho_dT_def = -0.2 ; dRho_dS_def = 0.8
    EOS%Rho_T0_S0 = 1000.0 ; EOS%dRho_dT = -0.2 ; EOS%dRho_dS = 0.8
    call read_param(param_file,"RHO_T0_S0",EOS%Rho_T0_S0)
    call read_param(param_file,"DRHO_DT",EOS%dRho_dT)
    call read_param(param_file,"DRHO_DS",EOS%dRho_dS)
  endif

  EOS%EOS_quadrature = .false.
  call read_param(param_file,"EOS_QUADRATURE",EOS%EOS_quadrature)

  tmpstr = TFREEZE_DEFAULT
  call read_param(param_file,"TFREEZE_FORM",tmpstr,.false.)
  tmpstr = uppercase(tmpstr)
  select case (tmpstr)
    ...
    case default
      call GOLD_error(FATAL, "interpret_eos_selection: #DEFINE TFREEZE_FORM "//&
                              trim(tmpstr) // "in input file is invalid.")
  end select
  if (EOS%form_of_TFreeze == TFREEZE_LINEAR) then
    ! Set typical default values.
    TFr_S0_P0_def = 0.0 ; dTFr_dS_def = -0.054 ; dTFr_dp_def = 0.0
    EOS%TFr_S0_P0 = TFr_S0_P0_def ; EOS%dTFr_dS = dTFr_dS_def
    EOS%dTFr_dp = dTFr_dp_def
    call read_param(param_file, "TFREEZE_S0_P0", EOS%TFr_S0_P0)
    call read_param(param_file, "DTFREEZE_DS", EOS%dTFr_dS)
    call read_param(param_file, "DTFREEZE_DP", EOS%dTFr_dP)
  endif
  ...
  if (write_log) then
    ...
    select case (EOS%form_of_EOS)
      case (EOS_LINEAR) ; tmpstr = EOS_LINEAR_STRING
      case (EOS_UNESCO) ; tmpstr = EOS_UNESCO_STRING
      case (EOS_WRIGHT) ; tmpstr = EOS_WRIGHT_STRING
      case default      ; tmpstr = "Unrecognized"
    end select
    call log_param(param_file, mod, "EQN_OF_STATE", tmpstr, &
    ...
    if (EOS%form_of_EOS == EOS_LINEAR) then
      call log_param(param_file, mod, "RHO_T0_S0", EOS%Rho_T0_S0, &
                   "When EQN_OF_STATE="//trim(EOS_LINEAR_STRING)//", \n"//&
                   "this is the density at T=0, S=0.", units="kg m-3", &
                   default=Rho_T0_S0_def)
      call log_param(param_file, mod, "DRHO_DT", EOS%dRho_dT, &
                   "When EQN_OF_STATE="//trim(EOS_LINEAR_STRING)//", \n"//&
                   "this is the partial derivative of density with \n"//&
                   "temperature.", units="kg m-3 K-1", default=dRho_dT_def)
      call log_param(param_file, mod, "DRHO_DS", EOS%dRho_dS, &
                   "When EQN_OF_STATE="//trim(EOS_LINEAR_STRING)//", \n"//&
                   "this is the partial derivative of density with \n"//&
                   "salinity.", units="kg m-3 PSU-1", default=dRho_dS_def)
    endif
    call log_param(param_file, mod, "EOS_QUADRATURE", EOS%EOS_quadrature, &
                   "If true, always use the generic (quadrature) code \n"//&
                   "code for the integrals of density.", default=.false.) 

    select case (EOS%form_of_TFreeze)
      case (TFREEZE_LINEAR)  ; tmpstr = TFREEZE_LINEAR_STRING
      case (TFREEZE_MILLERO) ; tmpstr = TFREEZE_MILLERO_STRING
      case default           ; tmpstr = "Unrecognized"
    end select
    call log_param(param_file, mod, "TFREEZE_FORM", tmpstr, &
    ...
    if (EOS%form_of_TFreeze == TFREEZE_LINEAR) then
      call log_param(param_file, mod, "TFREEZE_S0_P0",EOS%TFr_S0_P0, &
                   "When TFREEZE_FORM="//trim(TFREEZE_LINEAR_STRING)//", \n"//&
                   "this is the freezing potential temperature at \n"//&
                   "S=0, P=0.", units="deg C", default=TFr_S0_P0_def)
      call log_param(param_file, mod, "DTFREEZE_DS",EOS%dTFr_dS, &
                   "When TFREEZE_FORM="//trim(TFREEZE_LINEAR_STRING)//", \n"//&
                   "this is the derivative of the freezing potential \n"//&
                   "temperature with salinity.", &
                   units="deg C PSU-1", default=dTFr_dS_def)
      call log_param(param_file, mod, "DTFREEZE_DP",EOS%dTFr_dP, &
                   "When TFREEZE_FORM="//trim(TFREEZE_LINEAR_STRING)//", \n"//&
                   "this is the derivative of the freezing potential \n"//&
                   "temperature with pressure.", &
                   units="deg C Pa-1", default=dTFr_dP_def)
    endif
