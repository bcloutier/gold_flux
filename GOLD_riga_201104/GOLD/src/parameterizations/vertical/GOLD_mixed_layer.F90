...
use GOLD_domains, only : pass_var
...
type, public :: mixedlayer_CS ; private
  ...
  real    :: Hmix            ! The minimum mixed layer thickness in m.
  ...
  logical :: use_local_Pref  ! Use rough approximation to local pressure rather
                             ! than 0 or 1e5 Pa (ie. use local pot. dens.)
                             ! The error in the original implementation of
                             ! use_local_Pref has been fixed.
  ...
  real    :: Allowed_T_chg = 0.5  ! The amount by which temperature is allowed
                             ! to exceed previous values during detrainment, K.
  real    :: Allowed_S_chg = 0.1  ! The amount by which salinity is allowed
...
!                        The default is .true.
  ...
  real, dimension(SZI_(G)) :: &
    ...
    R0_ml, &    !   The potential density referenced to the surface of the mixed
                ! layer, in units of kg m-3.
    Rcv_ml, &   !   The coordinate value potential density of the mixed layer,
                ! in kg m-3.
  ...
  logical :: do_T   ! If true, T & S are state variables.
  ...
  if (.not. associated(CS)) call GOLD_error(FATAL, "GOLD_mixed_layer: "//&
         "Module must be initialized before it is used.")
  if (.not.CS%bulkmixedlayer) return
  ...
  if (.NOT. ASSOCIATED(fluxes%ustar)) call GOLD_error(FATAL, &
      "GOLD_mixed_layer: No surface TKE fluxes (ustar) defined in mixedlayer!")

  nsw = 0
  if ((do_T) .and. ASSOCIATED(optics)) nsw = optics%nbands
  ...
  do j=js,je
    ...
    do k=1,nz ; do i=is,ie
      ...
      if (do_T) then
        ...
        R0(i,k) = G%Rlay(k) ; if (k <= nkmb) R0(i,k) = tv%Rml(i,j,k)
      endif
    enddo ; enddo
    ...
    if (do_T) then
      if (CS%use_local_pref) then
        ...
        do k=1,nz
          call calculate_2_densities(T(:,k),S(:,k),0.0,tv%P_Ref, &
                                     R0(:,k),Rcv(:,k),is,ie-is+1,tv%eqn_of_state)
        enddo
      endif
    endif
    ...
    if (CS%ML_resort) then
      ...
      if (CS%ML_presort_nz_conv_adj > 0) &
        call convective_adjustment(h, u, v, R0, Rcv, T, S, d_eb, dKE_CA, cTKE, &
                                   j, G, CS, do_T, CS%ML_presort_nz_conv_adj)
    ...
    call cpu_clock_begin(id_clock_EOS)
    if ((do_T) .and. (.not.CS%use_local_pref)) then
      do k=1,nkmb
       !### This call may be unnecessary, but removing it will change answers
       !### in the global test case.
        call calculate_2_densities(T(:,k),S(:,k),0.0,tv%P_Ref, &
                                   R0(:,k),Rcv(:,k),is,ie-is+1,tv%eqn_of_state)
      enddo

      call calculate_density_derivs(T(:,1), S(:,1), p_ref, &
                                    dR0_dT, dR0_dS,is,ie-is+1,tv%eqn_of_state)
      call calculate_density_derivs(T(:,1), S(:,1), p_ref_cv, &
                                    dRcv_dT, dRcv_dS,is,ie-is+1,tv%eqn_of_state)
    endif
    call cpu_clock_end(id_clock_EOS)
    ...
    call extract_fluxes(fluxes, optics, Net_H, Net_heat, Net_salt, nsw, &
                        Pen_SW_bnd, Bforce, dt, j, G, CS, do_T)
...
! Here the mechanically driven entrainment occurs.
    ...
    if (do_T) call absorb_remaining_SW(h, htot, T, Ttot, nsw, Pen_SW_bnd, &
                                       j, ksort, G, CS)
    ...
    if (CS%limit_det .or. (CS%id_Hsfc_max > 0) .or. (CS%id_Hsfc_min > 0)) then
      ...
      if (CS%limit_det .or. (CS%id_Hsfc_min > 0)) then
        ...
        do i=is,ie
          ...
          Hsfc_min(i,j) = G%H_to_m*max(htot(i), min(Hsfc(i), H_nbr))
...
! treated in conjuction.
    ...
    if (CS%nkbl == 1) then
      call mixedlayer_detrain_1(h, R0, htot, R0_ml, dt, dt__diag, d_ea, d_eb, &
               j, G, CS, do_T, T, S, Rcv, T_ml, S_ml, Rcv_ml, dRcv_dT, dRcv_dS, max_BL_det)
    elseif (CS%nkbl == 2) then
      call mixedlayer_detrain_2(h, R0, htot, R0_ml, dt, dt__diag, d_ea, j, G, &
                                CS, do_T, T, S, Rcv, T_ml, S_ml, Rcv_ml, &
                                dRcv_dT, dRcv_dS, dR0_dT, dR0_dS, max_BL_det)
    else ! CS%nkbl not = 1 or 2
      ! This code only works with 1 or 2 buffer layers.
      call GOLD_error(FATAL, "GOLD_mixed_layer: CS%nkbl must be 1 or 2 for now.")
    endif
    call cpu_clock_end(id_clock_detrain)


    if (CS%id_Hsfc_used > 0) then
      do i=is,ie ; Hsfc_used(i,j) = htot(i)*G%H_to_m ; enddo
...
! 3-d variables.
    if (CS%Resolve_Ekman .and. (CS%nkml>1)) then
      ...
      do i=is,ie
        h_3d(i,j,1) = htot(i) * Inkml
      enddo
      do k=2,CS%nkml ; do i=is,ie
        h_3d(i,j,k) = htot(i) * Inkml
        d_ea(i,k) = d_ea(i,k) + h_3d(i,j,k)
        d_ea(i,1) = d_ea(i,1) - h_3d(i,j,k)
      enddo ; enddo
    endif
    do k=1,CS%nkml ; do i=is,ie
      if (do_T) then
        tv%T(i,j,k) = T_ml(i) ; tv%S(i,j,k) = S_ml(i)
      else
        tv%Rml(i,j,k) = R0_ml(i)
      endif
    enddo ; enddo
...
! eaml(i,nz) is derived from  h(i,nz) - h_orig(i,nz) = eaml(i,nz) - ebml(i,nz-1)
    ...
    if (do_T) then
      ...
      do k=CS%nkml+1,nz ; do i=is,ie ; h_3d(i,j,k) = h(i,k) ; enddo ; enddo
      do k=CS%nkml+1,nkmb ; do i=is,ie ; tv%Rml(i,j,k) = R0(i,k) ; enddo ; enddo
    endif
  ...
  if (do_T) then
    call cpu_clock_begin(id_clock_EOS)
    do k=1,nkmb ; do j=js,je
      call calculate_density(tv%T(:,j,k),tv%S(:,j,k),p_ref_cv,tv%Rml(:,j,k), &
                             is,ie-is+1,tv%eqn_of_state)
    enddo ; enddo
    call cpu_clock_end(id_clock_EOS)
  endif
  ...
  subroutine convective_adjustment(h, u, v, R0, Rcv, T, S, d_eb, &
                                   dKE_CA, cTKE, j, G, CS, do_T, nz_conv)
  ...
  !  (in)      do_T - If true, T & S are state variables.
...
! layer above.
  ...
  subroutine extract_fluxes(fluxes, optics, Net_H, Net_heat, Net_salt, nsw, &
    ...
    type(forcing),               intent(in)  :: fluxes
    type(optics_type),           pointer     :: optics
    real, dimension(NXMEM_),     intent(out) :: Net_H, Net_heat, Net_salt
    integer,                     intent(in)  :: nsw
    real, dimension(:,:),        intent(out) :: Pen_SW_bnd
    real, dimension(NXMEM_),     intent(out) :: Bforce
    real,                        intent(in)  :: dt
    integer,                     intent(in)  :: j
    type(ocean_grid_type),       intent(in)  :: G
    type(mixedlayer_CS),         pointer     :: CS
  ...
  !                         in each penetrating band, in K H, size nsw x NXMEM_.
  ...
  !  (in)      do_T - If true, T & S are state variables.
    ...
    Irho0 = 1.0 / G%Rho0 ; I_Cp = 1.0 / CS%C_p

    if (nsw > 0) then ; if (nsw /= optics%nbands) call GOLD_error(WARNING, &
      "mismatch in the number of bands of shortwave radiation in mixed_layer extract_fluxes.")
    endif

    if (do_T) then
      if (.not.ASSOCIATED(fluxes%sw)) call GOLD_error(FATAL, &
        "GOLD_mixed_layer: fluxes%sw is not associated.")
      if (.not.ASSOCIATED(fluxes%lw)) call GOLD_error(FATAL, &
        "GOLD_mixed_layer: fluxes%lw is not associated.")
      if (.not.ASSOCIATED(fluxes%latent)) call GOLD_error(FATAL, &
        "GOLD_mixed_layer: fluxes%latent is not associated.")
      if (.not.ASSOCIATED(fluxes%sens)) call GOLD_error(FATAL, &
        "GOLD_mixed_layer: fluxes%sens is not associated.")

      if (.not.ASSOCIATED(fluxes%evap)) call GOLD_error(FATAL, &
        "GOLD_mixed_layer: No evaporation defined in mixedlayer.")
      if (.not.ASSOCIATED(fluxes%virt_precip)) call GOLD_error(FATAL, &
        "GOLD_mixed_layer: fluxes%virt_precip not defined in mixedlayer.")

      if ((.not.ASSOCIATED(fluxes%liq_precip)) .or. &
          (.not.ASSOCIATED(fluxes%froz_precip))) call GOLD_error(FATAL, &
        "GOLD_mixed_layer: No precipitation defined in mixedlayer.")
    else
      if (.not.ASSOCIATED(fluxes%buoy)) call GOLD_error(FATAL, &
          "GOLD_mixed_layer: No buoyancy fluxes defined in mixedlayer.")
    endif
...
!  Convert the penetrating shortwave forcing to K m.
        ...
        if (CS%use_river_heat_content) then
          if (.not.ASSOCIATED(fluxes%runoff_hflx)) call GOLD_error(FATAL, &
        ...
        if (CS%use_calving_heat_content) then
          if (.not.ASSOCIATED(fluxes%calving_hflx)) call GOLD_error(FATAL, &
      ...
      do i=is,ie
        ...
        if (htot(i)*Ih_limit < 1.0) scale = htot(i)*Ih_limit
        Net_H(i) = 0.0
        Bforce(i) = (dt * scale * fluxes%buoy(i,j)) * G%m_to_H
        Pen_SW_bnd(1,i) = 0.0
        Net_salt(i) = 0.0
      enddo
    endif
...
!                    dRcv_dT(i)*precip*(T_precip - T(i,1)) + &      
      ...
      do i=is,ie ; if (ksort(i,1) > 0) then
        ...
        htot(i) = max(min(Angstrom,h(i,k)-eps(i,k)),0.0)
        h(i,k) = h(i,k) - htot(i)
        d_eb(i,k) = d_eb(i,k) - htot(i)
        uhtot(i) = u(i,k)*htot(i) ; vhtot(i) = v(i,k)*htot(i)
        R0_tot(i) = htot(i)*R0(i,k) - (G%Rho0/G%g_Earth)*Bforce(i)
        Conv_En(i) = 0.0 ; dKE_FC(i) = 0.0
      endif ; enddo
    endif
...
! salt that is left behind goes into Stot.
        if ((Evap_rem(i) > 0.0) .and. (h(i,k) > eps(i,k)) .and. (do_T)) then
...
!   Without penetrative shortwave radiation, the entire layer is
! either above or below the depth of free convection.
            if (dr > 0.0) then
              h_ent = h_avail
              Conv_En(i) = Conv_En(i) + g_H2_2Rho0 * h_ent * &
                            (R0_tot(i) - R0(i,k)*htot(i))
              R0_tot(i) = R0_tot(i) + h_ent * R0(i,k)
            endif ! dr0 > 0.0
          endif
  ...
  subroutine find_starting_TKE(htot, h_CA, fluxes, Conv_En, cTKE, dKE_FC, dKE_CA, &
                               TKE, TKE_river, Idecay_len_TKE, cMKE1, cMKE2, dt, &
                               j, ksort, G, CS)
    real, dimension(NXMEM_),        intent(in)    :: htot, h_CA
    type(forcing),                  intent(in)    :: fluxes
...
! Add additional TKE at river mouths
  ...
  !  (in)      do_T - If true, T & S are state variables.
    ...
    real :: E_HxHpE   ! Entrainment divided by the product of the new and old
                      ! thicknesses, in H-1.
    real :: Hmix      ! The minimum mixed layer depth in H.
    real :: opacity
    ...
    do ks=1,nz
      ...
      do i=is,ie ; if (ksort(i,ks) > 0) then
        ...
        h_avail = h(i,k) - eps(i,k)
        if ((h_avail > 0.0) .and. ((TKE(i) > 0.0) .or. (htot(i) < Hmix))) then
...
! Find the TKE that would remain if the entire layer were entrained.
          ...
          Pen_En_Contrib = 0.0
          if (do_T) then ; do n=1,nsw ; if (Pen_SW_bnd(n,i) > 0.0) then
            opacity = opacity_band(n,i,k)
...
! numbers.  They are, of course, mathematically identical.
          ...
          if ((TKE_full_ent > 0.0) .or. (h_avail+htot(i) <= Hmix)) then
...
! numbers.  They are, of course, mathematically identical.
              ...
              endif ; enddo ; endif ! (Pen_SW_bnd(n,i) > 0.0)
...
! Bisect if the Newton's method prediction is outside of the bounded range.
            ...
            if (h_ent < Hmix-htot(i)) h_ent = Hmix - htot(i)
  ...
  !                         the water column, in K H, size nsw x NXMEM_.
  !  (in)      j - The j-index to work on.
  !  (in)      ksort - The density-sorted k-indicies.
  !  (in)      G - The ocean's grid structure.
  !  (in)      CS - The control structure for this module.
...
subroutine resort_ML(h, eps, d_ea, d_eb, R0, Rcv, T, S, ksort, G, CS, do_T, &
                     dRcv_dT, dRcv_dS, dR0_dT, dR0_dS, Rcv_ml, R0_ml)
...
!  (in/out)  Rcv_ml - The coordinate defining potential density of the mixed
!                     layer, in kg m-3.
!  (in)      R0_ml - Mixed layer potential density referenced to surface
!                    pressure, in kg m-3.
...
!   Otherwise, if there are more than nkbl+1 remaining massive layers
  ...
  integer :: ks_deep(SZI_(G)), k_count(SZI_(G)), ks2_reverse(SZI_(G), SZK_(G))
  integer :: ks2(SZK_(G))
...
! Find out how many massive layers are above the deepest buffer or mixed layer.
  ...
  do i=is,ie ; if (k_count(i) > 1) then
    ...
    if (num_interior >= 1) then
      ...
      do k=nkmb+1,nz ; if (Rcv_ml(i) < G%Rlay(k)) exit ; enddo
      ...
      do ks = nks,top_interior_ks,-1
        ...
        elseif (Rcv(i,k) < Rcv_int) then
          ...
          if (do_T) then
            ...
            if ((R0_up > R0_ml(i)) .or. (R0_dn > R0_ml(i))) &
              ! Avoid creating obviously unstable profiles.
              exit
          else
            R0_up = G%Rlay(k2-1) ; R0_dn = G%Rlay(k2)
          endif
...
!                of h are referred to as H below.
!  (in/out)  R0 - Potential density referenced to surface pressure, in kg m-3.
!  (in/out)  h_ml - New mixed layer thickness in m or kg m-2 (H).
!  (in)      R0_ml - Mixed layer potential density referenced to surface
!                    pressure, in kg m-3.
!  (in)      dt - Time increment, in s.
...
! The following 8 arguments are either all used or not, depending on do_T.
!  (in/out)  T - Potential temperature, in C.
!  (in/out)  S - Salinity, in psu.
!  (in/out)  Rcv - The coordinate defining potential density, in kg m-3.
!  (in/out)  T_ml - Mixed layer potential temperature, in C.
!  (in/out)  S_ml - Mixed layer salinity, in psu.
!  (in/out)  Rcv_ml - The coordinate defining potential density of the mixed
!                     layer, in kg m-3.
...
!  real :: dT_2dz, dS_2dz          ! S, in kg m-4, kg m-4, K m-1, and psu m-1.
  ...
  if (CS%nkbl /= 2) call GOLD_error(FATAL, "GOLD_mixed_layer"// &
  ...
  ! since it means that a number of variables can now be scalars, not arrays.
    ...
    h_min_bl = MIN(h_min_bl_thick,h_min_bl_frac_ml*h_ml(i))

    stable_Rcv = .true.
    if ((do_T) .and. ((R0(i,kb2)-R0(i,kb1)) * (Rcv(i,kb2)-Rcv(i,kb1)) <= 0.0)) &
      stable_Rcv = .false.
    ...
    if ((h1 > 0.0) .and. (h2 > 0.0) .and. (h_to_bl > 0.0) .and. &
      ...
      if (do_T) then
        ...
        do k1=kb2+1,nz ; if (G%Rlay(k1) >= R0(i,kb2)) exit ; enddo ; k0 = k1-1
        dR1 = G%Rlay(k0)-R0(i,kb1) ; dR2 = R0(i,kb2)-G%Rlay(k0)
      endif
      ...
      if ((k1<=nz) .and. (h2 > h_min_bl) .and. (h1_avail > 0.0) .and. &
        ...
        if (do_T) then
          ...
          R0(i,kb2) = ((h(i,kb2)*R0(i,kb2) - h2_to_k1*G%Rlay(k1)) + &
                       h1_to_h2*R0(i,kb1)) * Ih2f
          R0(i,k1) = (h(i,k1)*R0(i,k1) + h2_to_k1*G%Rlay(k1)) * Ihk1
        endif
        ...
        if (do_T) then
          ...
          if ((k1>kb2+1) .and. (G%Rlay(k1-1) >= R0(i,kb2))) then
            do k1=k1,kb2+1,-1 ; if (G%Rlay(k1-1) < R0(i,kb2)) exit ; enddo
          endif
        endif
      endif
      ...
      if ((k1<=nz).and.(.not.mergeable_bl)) then
        ...
        if (do_T) then
          ...
          dR2b = G%Rlay(k1)-R0(i,kb2) ; dR21 = R0(i,kb2) - R0(i,kb1)
        endif
...
!               call GOLD_error(WARNING, mesg)
            ...
            Ihk1 = 1.0 / (h(i,k1) + h2_to_k1)
            R0_det = G%Rlay(k1)
            R0(i,kb2) = R0(i,kb2) + h2_to_k1*(R0(i,kb1) - R0(i,kb2)) / (h1+h2)
            R0(i,k1) = (h(i,k1)*R0(i,k1) + h2_to_k1*G%Rlay(k1)) * Ihk1
          endif
    ...
    if (h_det_h2 > 0.0) then
      ...
      R0(i,kb2) = ((h2*R0(i,kb2) + h1*R0(i,kb1)) + &
                   (h_det_to_h2*R0_to_bl*Ihdet + h_ml_to_h2*R0_ml(i))) * Ih
      R0(i,kb1) = (h_det_to_h1*R0_to_bl*Ihdet + h_ml_to_h1*R0_ml(i)) * Ih1f
      if (do_T) then
        Rcv(i,kb2) = ((h2*Rcv(i,kb2) + h1*Rcv(i,kb1)) + &
                      (h_det_to_h2*Rcv_to_bl*Ihdet + h_ml_to_h2*Rcv_ml(i))) * Ih
        Rcv(i,kb1) = (h_det_to_h1*Rcv_to_bl*Ihdet + h_ml_to_h1*Rcv_ml(i)) * Ih1f

        T(i,kb2) = ((h2*T(i,kb2) + h1*T(i,kb1)) + &
                    (h_det_to_h2*T_to_bl*Ihdet + h_ml_to_h2*T_ml(i))) * Ih
        T(i,kb1) = (h_det_to_h1*T_to_bl*Ihdet + h_ml_to_h1*T_ml(i)) * Ih1f

        S(i,kb2) = ((h2*S(i,kb2) + h1*S(i,kb1)) + &
                    (h_det_to_h2*S_to_bl*Ihdet + h_ml_to_h2*S_ml(i))) * Ih
        S(i,kb1) = (h_det_to_h1*S_to_bl*Ihdet + h_ml_to_h1*S_ml(i)) * Ih1f
      endif
      ...
      if (ASSOCIATED(CS%diag%PE_detrain) .or. ASSOCIATED(CS%diag%PE_detrain2)) then
        ...
        s1en = G_2 * Idt_H2 * ( ((R0(i,kb2)-R0(i,kb1))*h1*h2 + &
            ...
            h_ml_to_h2*( (R0(i,kb2)-R0_ml(i))*h2 + (R0(i,kb1)-R0_ml(i))*h1 + &
    ...
    ! from the uppermost buffer layer into the next one down.
      ...
      if (h_from_ml > 0.0) then
        ...
        dPE_extrap = dPE_extrap - I2Rho0*h_from_ml*(R0_to_bl - R0_ml(i)*h_to_bl)
        R0_to_bl = R0_to_bl + h_from_ml*R0_ml(i)
        if (do_T) then
          Rcv_to_bl = Rcv_to_bl + h_from_ml*Rcv_ml(i)
          T_to_bl = T_to_bl + h_from_ml*T_ml(i)
          S_to_bl = S_to_bl + h_from_ml*S_ml(i)
        endif
        h_to_bl = h_to_bl + h_from_ml
        h_ml(i) = h_ml(i) - h_from_ml
  ...
  !         stays = s - sqrt(s2*s2 - s3sq) ! Note that s2 = h1-s & stays_min >= 0
      ...
      if (dPE_time_ratio*h_to_bl > h_to_bl+h_ml(i)) then
        dPE_ratio = (h_to_bl+h_ml(i)) / h_to_bl
      else
...
!                       dR0_2dz*(h1_to_k0 + h1_to_h2))) * Ih1f
        ...
        if (do_T) then
          ...
          if (h_to_bl > 0.0) then
            ...
            dSpice_lim = dS_dT_gauge*dR0_dS(i)*(T_ml(i)-T(i,kb1)) - &
                         dT_dS_gauge*dR0_dT(i)*(S_ml(i)-S(i,kb1))
          endif
...
subroutine mixedlayer_detrain_1(h, R0, h_ml, R0_ml, dt, dt_diag, d_ea, d_eb, &
                                j, G, CS, do_T, T, S, Rcv, T_ml, S_ml, Rcv_ml, &
...
!                of h are referred to as H below.
!  (in/out)  R0 - Potential density referenced to surface pressure, in kg m-3.
!  (in/out)  h_ml - New mixed layer thickness in m.
!  (in/out)  R0_ml - Mixed layer potential density referenced to surface
!                    pressure, in kg m-3.
!  (in)      dt - Time increment, in s.
!  (in/out)  d_ea - The upward increase across a layer in the entrainment from
!                   above, in m or kg m-2 (H).  Positive d_ea goes with layer
...
!                      density with salinity, in kg m-3 psu-1.
  ...
  is = G%isc ; ie = G%iec ; nz = G%ke
  nkmb = CS%nkml+CS%nkbl
  if (CS%nkbl /= 1) call GOLD_error(FATAL,"GOLD_mixed_layer: "// &
...
! graceful options are available.
  do i=is,ie ; if (h(i,nkmb) > 0.0) then
    if ((R0_ml(i)<R0(i,nz)) .and. (R0(i,nz)<R0(i,nkmb))) then
      if ((R0(i,nz)-R0_ml(i))*h_ml(i) > &
          (R0(i,nkmb)-R0(i,nz))*h(i,nkmb)) then
        detrain(i) = (R0(i,nkmb)-R0(i,nz))*h(i,nkmb) / (R0(i,nkmb)-R0_ml(i))
      else
        detrain(i) = (R0(i,nz)-R0_ml(i))*h_ml(i) / (R0(i,nkmb)-R0_ml(i))
      endif
      ...
      if (do_T) then
        x1 = Rcv_ml(i)
        Rcv_ml(i) = Rcv_ml(i) - detrain(i)*(Rcv_ml(i)-Rcv(i,nkmb)) / h_ml(i)
        Rcv(i,nkmb) = Rcv(i,nkmb) - detrain(i)*(Rcv(i,nkmb)-x1) / h(i,nkmb)
        x1 = T_ml(i)
        T_ml(i) = T_ml(i) - detrain(i)*(T_ml(i)-T(i,nkmb)) / h_ml(i)
        T(i,nkmb) = T(i,nkmb) - detrain(i)*(T(i,nkmb)-x1) / h(i,nkmb)
        x1 = S_ml(i)
        S_ml(i) = S_ml(i) - detrain(i)*(S_ml(i)-S(i,nkmb)) / h_ml(i)
...
!    dt_Time = dt/Timescale
  do k=nz-1,nkmb+1,-1 ; do i=is,ie
    if (splittable_BL(i)) then
      if ((do_T) .and. (G%Rlay(k)<=Rcv(i,nkmb))) then
...
! slightly arbitrary.
        ...
        if ((h(i,k+1) < 10.0*G%Angstrom) .or. &
            ((G%Rlay(k+1)-Rcv(i,nkmb)) >= 0.9*(Rcv(i,k1) - Rcv_ml(i)))) then
          if (k>=nz-1) then ; orthogonal_extrap = .true.
          elseif ((h(i,k+2) <= 10.0*G%Angstrom) .and. &
              ((G%Rlay(k+1)-Rcv(i,nkmb)) < 0.9*(Rcv(i,k+2)-Rcv_ml(i)))) then
            k1 = k+2
          else ; orthogonal_extrap = .true. ; endif
        endif

        if ((R0_ml(i) >= R0(i,k1)) .or. (Rcv_ml(i) >= Rcv(i,nkmb))) cycle
        ...
        if (orthogonal_extrap) then
          ...
          dT_dR = (T_ml(i) - T(i,k1)) / (Rcv_ml(i) - Rcv(i,k1))
          dS_dR = (S_ml(i) - S(i,k1)) / (Rcv_ml(i) - Rcv(i,k1))
        endif
        dRml = dt_Time * (R0(i,nkmb) - R0_ml(i)) * &
               (Rcv_ml(i) - Rcv(i,k1)) / (R0_ml(i) - R0(i,k1))
        ! Once again, there is an apparent density inversion in Rcv.
        if (dRml < 0.0) cycle
        dR0_dRcv = (R0_ml(i) - R0(i,k1)) / (Rcv_ml(i) - Rcv(i,k1))
...
!                           (h(i,nkmb) - detrain(i))
      ...
      elseif ( (.not. do_T) .and. (G%Rlay(k)<=R0(i,nkmb))) then
        splittable_BL(i) = .false.
        dRml = dt_Time * (R0(i,nkmb) - R0_ml(i))
        if ((R0(i,nkmb) - dRml < G%Rlay(k)) .and. (max_det_rem(i) > h(i,nkmb))) then
          ! In this case, the buffer layer is split into two isopycnal layers.
          detrain(i) = h(i,nkmb)*(R0(i,nkmb) - G%Rlay(k)) / &
                                    (G%Rlay(k+1) - G%Rlay(k))

          if (ASSOCIATED(CS%diag%PE_detrain)) CS%diag%PE_detrain(i,j) = &
            CS%diag%PE_detrain(i,j) - g_H2_2dt * detrain(i) * &
                 (h(i,nkmb)-detrain(i)) * (G%Rlay(k+1) - G%Rlay(k))
          ...
          ! Here only part of the buffer layer is moved into the interior.
          detrain(i) = h(i,nkmb) * dRml / (G%Rlay(k+1)-R0(i,nkmb)+dRml)
          if (detrain(i) > max_det_rem(i)) detrain(i) = max_det_rem(i)

          R0(i,nkmb) = R0(i,nkmb) - dRml
!            This is equivalent to ...
!            R0(i,nkmb) = (h(i,nkmb) * R0(i,nkmb) - detrain(i)*G%Rlay(k+1)) / &
!                           (h(i,nkmb) - detrain(i))
...
! and buffer layers is vague anyway, this seems hard to argue against.
  do i=is,ie
    if (h(i,nkmb) < 0.1*h_ml(i)) then
      h_ent =  0.1*h_ml(i) - h(i,nkmb)
      Ih = 10.0/h_ml(i)
      if (do_T) then
        T(i,nkmb) = (h(i,nkmb)*T(i,nkmb) + h_ent*T_ml(i)) * Ih
        S(i,nkmb) = (h(i,nkmb)*S(i,nkmb) + h_ent*S_ml(i)) * Ih
      else
        R0(i,nkmb) = (h(i,nkmb)*R0(i,nkmb) + h_ent*R0_ml(i)) * Ih
      endif
...
!                  for this module
  character(len=128) :: version = '$Id: GOLD_mixed_layer.F90,v 13.0.2.10.2.64 2011/01/31 15:10:00 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_mixed_layer"  ! This module's name.
  logical :: use_temperature
  integer :: isd, ied, jsd, jed
  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed
 
  if (associated(CS)) then
    call GOLD_error(WARNING, "mixedlayer_init called with an associated"// &
  ...
  CS%bulkmixedlayer = .false. ; CS%absorb_all_SW = .false.
  ...
  call read_param(param_file,"NSTAR",CS%nstar,.true.)
  call read_param(param_file,"BULK_RI_ML",CS%bulk_Ri_ML,.true.)
  CS%TKE_decay = 2.5 ; CS%nstar2 = CS%nstar ; CS%conv_decay = 0.5
  CS%Hmix = 0.0 ; CS%bulk_Ri_convective = CS%bulk_Ri_ML ; CS%use_omega = .false.
  CS%ML_resort = .false. ; CS%ML_presort_nz_conv_adj = 0
  call read_param(param_file,"ABSORB_ALL_SW",CS%absorb_all_sw)
  call read_param(param_file,"TKE_DECAY",CS%TKE_decay)
  call read_param(param_file,"CONV_DECAY",CS%conv_decay)
  call read_param(param_file,"NSTAR2",CS%nstar2)
  call read_param(param_file,"BULK_RI_CONVECTIVE",CS%bulk_Ri_convective)
  call read_param(param_file,"HMIX",CS%Hmix)
  call read_param(param_file,"HMIX_MIN",CS%Hmix)

  CS%limit_det = .false. ; CS%lim_det_dH_sfc = 0.5 ; CS%lim_det_dH_bathy = 0.2
  call read_param(param_file,"LIMIT_BUFFER_DETRAIN",CS%limit_det)
  if (CS%limit_det) then
    call read_param(param_file,"LIMIT_BUFFER_DET_DH_SFC",CS%lim_det_dH_sfc)
    call read_param(param_file,"LIMIT_BUFFER_DET_DH_BATHY",CS%lim_det_dH_bathy)
  endif
  call read_param(param_file,"ALLOWED_DETRAIN_TEMP_CHG",CS%Allowed_T_chg)
  call read_param(param_file,"ALLOWED_DETRAIN_SALT_CHG",CS%Allowed_S_chg)

  CS%H_limit_fluxes = 0.1*CS%Hmix
  call read_param(param_file,"DEPTH_LIMIT_FLUXES",CS%H_limit_fluxes)
  call read_param(param_file,"ML_USE_OMEGA",CS%use_omega)
  call read_param(param_file,"ML_RESORT",CS%ML_resort)
  call read_param(param_file,"ML_PRESORT_NZ_CONV_ADJ",CS%ML_presort_nz_conv_adj)
  if (CS%use_omega) then
    call read_param(param_file,"OMEGA",CS%omega,.true.)
  else
  ...
  CS%dT_dS_wt = 6.0
  call read_param(param_file,"ML_DT_DS_WEIGHT",CS%dT_dS_wt)

  CS%Resolve_Ekman = .false.
  call read_param(param_file,"RESOLVE_EKMAN",CS%Resolve_Ekman)
  CS%correct_absorption =  .false.
  call read_param(param_file,"CORRECT_ABSORPTION_DEPTH",CS%correct_absorption)

  call read_param(param_file,"TEMPERATURE",use_temperature,.true.)
  if (use_temperature) call read_param(param_file,"C_P",CS%C_p,.true.)

  CS%use_local_pref=.false. ; CS%do_rivermix=.false.
  call read_param(param_file,"USE_LOCAL_PREF",CS%use_local_pref)
  call read_param(param_file,"DO_RIVERMIX",CS%do_rivermix)
  if (CS%do_rivermix) &
    call read_param(param_file,"RIVERMIX_DEPTH",CS%rivermix_depth,.true.)
  CS%use_river_heat_content=.false.
  call read_param(param_file,"USE_RIVER_HEAT_CONTENT",CS%use_river_heat_content)
  CS%use_calving_heat_content=.false.
  call read_param(param_file,"USE_CALVING_HEAT_CONTENT",CS%use_calving_heat_content)
   
  CS%id_ML_depth = register_diag_field('ocean_model', 'h_ML', G%axesh1, &
      Time, 'Surface mixed layer depth', 'meter')
  CS%id_TKE_wind = register_diag_field('ocean_model', 'TKE_wind', G%axesh1, &
      Time, 'Wind-stirring source of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_RiBulk = register_diag_field('ocean_model', 'TKE_RiBulk', G%axesh1, &
      Time, 'Mean kinetic energy source of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_conv = register_diag_field('ocean_model', 'TKE_conv', G%axesh1, &
      Time, 'Convective source of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_pen_SW = register_diag_field('ocean_model', 'TKE_pen_SW', G%axesh1, &
      Time, 'TKE consumed by mixing penetrative shortwave radation through the mixed layer', 'meter3 second-3')
  CS%id_TKE_mixing = register_diag_field('ocean_model', 'TKE_mixing', G%axesh1, &
      Time, 'TKE consumed by mixing that deepens the mixed layer', 'meter3 second-3')
  CS%id_TKE_mech_decay = register_diag_field('ocean_model', 'TKE_mech_decay', G%axesh1, &
      Time, 'Mechanical energy decay sink of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_conv_decay = register_diag_field('ocean_model', 'TKE_conv_decay', G%axesh1, &
      Time, 'Convective energy decay sink of mixed layer TKE', 'meter3 second-3')
  CS%id_TKE_conv_s2 = register_diag_field('ocean_model', 'TKE_conv_s2', G%axesh1, &
      Time, 'Spurious source of mixed layer TKE from sigma2', 'meter3 second-3')
  CS%id_PE_detrain = register_diag_field('ocean_model', 'PE_detrain', G%axesh1, &
      Time, 'Spurious source of potential energy from mixed layer detrainment', 'Watt meter-2')
  CS%id_PE_detrain2 = register_diag_field('ocean_model', 'PE_detrain2', G%axesh1, &
      Time, 'Spurious source of potential energy from mixed layer only detrainment', 'Watt meter-2')
  CS%id_h_mismatch = register_diag_field('ocean_model', 'h_miss_ML', G%axesh1, &
      Time, 'Summed absolute mismatch in entrainment terms', 'meter')
  CS%id_Hsfc_used = register_diag_field('ocean_model', 'Hs_used', G%axesh1, &
      Time, 'Surface region thickness that is used', 'meter')
  CS%id_Hsfc_max = register_diag_field('ocean_model', 'Hs_max', G%axesh1, &
      Time, 'Maximum surface region thickness', 'meter')
  CS%id_Hsfc_min = register_diag_field('ocean_model', 'Hs_min', G%axesh1, &
      Time, 'Minimum surface region thickness', 'meter')
  if ((CS%id_Hsfc_min > 0) .and. .not.CS%limit_det) then
    call read_param(param_file,"LIMIT_BUFFER_DET_DH_SFC",CS%lim_det_dH_sfc)
    call read_param(param_file,"LIMIT_BUFFER_DET_DH_BATHY",CS%lim_det_dH_bathy)
  endif
  ...
  call log_param(param_file, mod, "NSTAR", CS%nstar, &
                 ...
                 units="nondim")
  call log_param(param_file, mod, "BULK_RI_ML", CS%bulk_Ri_ML, &
  ...
  call log_param(param_file, mod, "ABSORB_ALL_SW", CS%absorb_all_sw, &
  ...
  call log_param(param_file, mod, "CONV_DECAY", CS%conv_decay, &
                 ...
                 default=0.5)
  call log_param(param_file, mod, "NSTAR2", CS%nstar2, &
  ...
  call log_param(param_file, mod, "BULK_RI_CONVECTIVE", CS%bulk_Ri_convective, &
  ...
  call log_param(param_file, mod, "RESOLVE_EKMAN", CS%Resolve_Ekman, &
  ...
  call log_param(param_file, mod, "CORRECT_ABSORPTION_DEPTH", CS%correct_absorption, &
  ...
  call log_param(param_file, mod, "TEMPERATURE", use_temperature, &
                 ...
                 "variables.")
  if (use_temperature)  call log_param(param_file, mod, "C_P", CS%C_p, &
                 "The heat capacity of sea water, approximated as a constant.", &
                 units="J kg-1 K-1")
  call log_param(param_file, mod, "USE_LOCAL_PREF", CS%use_local_Pref, &
                 "If true, use the local potential density at the average \n"//&
                 "pressure of the previous mixed layer to calculate TKE \n"//&
                 "budgets, instead of 0 or 1e5Pa.  ###The default should be changed.", &
                 default=.false.)
  call log_param(param_file, mod, "DO_RIVERMIX", CS%do_rivermix, &
