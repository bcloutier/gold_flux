...
!*    j+1  > o > o >   At ^:  v                                        *
!*    j    x ^ x ^ x   At >:  u                                        *
!*    j    > o > o >   At o:  h, T, S, buoy, Rml, ea, eb, etc.         *
!*    j-1  x ^ x ^ x                                                   *
!*        i-1  i  i+1  At x & ^:                                       *
!*           i  i+1    At > & o:                                       *
!*                                                                     *
...
use GOLD_entrain_H2000, only : entrainment_H2000, entrain_H2000_init
use GOLD_entrain_H2000, only : entrain_H2000_end, entrain_H2000_CS
use GOLD_entrain_diffusive, only : entrainment_diffusive, entrain_diffusive_init
use GOLD_entrain_diffusive, only : entrain_diffusive_end, entrain_diffusive_CS
...
use GOLD_domains, only : pass_var, To_West, To_South
...
use GOLD_geothermal, only : geothermal, geothermal_init, geothermal_end, geothermal_CS
use GOLD_grid, only : ocean_grid_type
use GOLD_io, only : vardesc
use GOLD_kappa_shear, only : Calculate_kappa_shear, kappa_shear_init, Kappa_shear_CS
use GOLD_mixed_layer, only : mixedlayer, mixedlayer_init, mixedlayer_CS
use GOLD_opacity, only : opacity_init, set_opacity, opacity_end, opacity_CS
use GOLD_set_diffusivity, only : set_diffusivity, set_BBL_diffusivity
use GOLD_set_diffusivity, only : set_diffusivity_init, set_diffusivity_end
use GOLD_set_diffusivity, only : set_diffusivity_CS
use GOLD_sponge, only : apply_sponge, sponge_CS
use GOLD_tracer_flow_control, only : call_tracer_column_fns, tracer_flow_control_CS
use GOLD_variables, only : forcing, thermo_var_ptrs, vertvisc_type, optics_type
use GOLD_variables, only : GOLD_forcing_chksum, GOLD_thermovar_chksum, p3d
use GOLD_vert_remap, only : vert_remap, vert_remap_init, vert_remap_CS
use GOLD_EOS, only : calculate_density, calculate_2_densities, calculate_TFreeze
...
type, public :: diabatic_CS ; private
  ...
  logical :: use_H2000_entrainment ! If true, use the shear-driven
                             ! entrainment scheme from Hallberg (MWR, 2000).
                             ! Otherwise, all entrainment occurs because of
                             ! the specified diffusivities.
  ...
  real    :: C_p             !   The heat capacity, in J kg-1 K-1.
  ...
  type(entrain_H2000_CS), pointer :: entrain_H2000_CSp => NULL()
...
!                 diabatic_driver_init.
  ...
  real, dimension(SZ1_(h),SZ2_(h),SZ3_(h)) :: &
  ...
  real, dimension(SZ1_(h),SZ2_(h),SZ3_(h)), target :: &
  ...
  real, dimension(SZ1_(h),SZ2_(h),SZ3_(h)+1), target :: &
  ...
  integer :: kb(SZ1_(h),SZ2_(h)) ! The index of the lightest layer denser
                              ! than the buffer layer.  Nondimensional.
  real :: p_ref_cv(SZ1_(h))   ! Reference pressure for the potential
                              ! density which defines the coordinate
                              ! variable, set to P_Ref, in Pa.

  logical :: in_boundary(SZ1_(h)) ! True if there are no massive layers below,
  ...
  real :: htot(SZ1_(u))  ! The summed thickness from the bottom, in m.
  real :: b1(SZ1_(u)), d1(SZ1_(u)) ! b1, c1, and d1 are variables used by the
  real :: c1(SZ1_(u),SZ3_(u))      ! tridiagonal solver.
  ...
  if (.not. associated(CS)) call GOLD_error(FATAL, "GOLD_diabatic_driver: "// &
         "Module must be initialized before it is used.")

  if (CS%debug) then
    call GOLD_state_chksum("Start of diabatic ", u(:,:,:), v(:,:,:), h(:,:,:), G)
  endif
...
!  is actually present in the ocean.  
      ...
      if (CS%debug) call GOLD_state_chksum("After mixedlayer ", u, v, h, G)
    endif

!  Determine the coordinate densities of the mixed and buffer layers.
    if (ASSOCIATED(tv%eqn_of_state)) then
      do i=is,ie ; p_ref_cv(i) = tv%P_Ref ; enddo
      do k=1,nkmb ; do j=js,je
        call calculate_density(T(:,j,k), S(:,j,k), p_ref_cv, tv%Rml(:,j,k), &
...
! for diapycnal diffusion and advection.
  if (CS%debug) then
    call GOLD_state_chksum("before use_kappa_shear ", u(:,:,:), v(:,:,:), h(:,:,:), G)
  endif
  if (CS%use_kappa_shear) then
    ...
    if (CS%debug) then
      call GOLD_state_chksum("aft find_uv_at_h KS ", u(:,:,:), v(:,:,:), h(:,:,:), G)
      call GOLD_forcing_chksum("before calc_KS ", fluxes, G, haloshift=0)
      call GOLD_thermovar_chksum("before calc_KS ", tv, G)
      call uchksum(u_h, "before calc_KS u_h",G)
      call vchksum(v_h, "before calc_KS v_h",G)
    endif

    call cpu_clock_begin(id_clock_kappa_shear)
    call Calculate_kappa_shear(u_h, v_h, h, tv, fluxes, visc%Kd_turb, visc%TKE_turb, &
                               dt, G, CS%kappa_shear_CSp)
    call cpu_clock_end(id_clock_kappa_shear)
    if (CS%debug) then
      call GOLD_state_chksum("after Calc_KS ", u(:,:,:), v(:,:,:), h(:,:,:), G)
      call GOLD_forcing_chksum("after calc_KS ", fluxes, G, haloshift=0)
      call GOLD_thermovar_chksum("after calc_KS ", tv, G)
  ...
  call cpu_clock_end(id_clock_set_diffusivity)
  if (CS%debug) then
    call GOLD_state_chksum("after set_diffusivity ", u(:,:,:), v(:,:,:), h(:,:,:), G)
    call GOLD_forcing_chksum("after set_diffusivity ", fluxes, G, haloshift=0)
    call GOLD_thermovar_chksum("after set_diffusivity ", tv, G)
  ...
  call Calculate_Entrainment(u, v, h, tv, fluxes, dt, G, CS, ea, eb, kb, &
                             Kd_Lay=Kd, Kd_int=Kd_int)
  call cpu_clock_end(id_clock_entrain)
  if (CS%debug) then
    call GOLD_forcing_chksum("after calc_entrain ", fluxes, G, haloshift=0)
    call GOLD_thermovar_chksum("after calc_entrain ", tv, G)
    call GOLD_state_chksum("after calc_entrain ", u(:,:,:), v(:,:,:), h(:,:,:), G)
  ...
  enddo
  if (CS%debug) then
    call GOLD_state_chksum("after negative check ", u(:,:,:), v(:,:,:), h(:,:,:), G)
  endif
...
! that correspond to their prescribed densities.
  ...
  !    (5) Possibly splits the buffer layer into two isopycnal layers.
      ...
      if (CS%debug) call GOLD_state_chksum("find_uv_at_h1 ", u, v, h, G)
...
! Calculate the change in temperature & salinity due to entrainment.
  ...
  if (CS%use_sponge) then
    ...
    if (CS%debug) then
      call GOLD_state_chksum("apply_sponge ", u, v, h, G)
    endif
  endif
...
!  mixed layer turbulence is applied elsewhere.
  if (CS%bulkmixedlayer) then
    do k=2,CS%nkml ; do j=js,je ; do i=is,ie
...
!  or enters the ocean with the surface velocity.
  ...
  if (CS%debug) then
    call GOLD_state_chksum("before tridiag ", u, v, h, G)
  endif
  call cpu_clock_begin(id_clock_tridiag)
  do j=js,je
    do I=Isq,Ieq
  ...
  enddo
  if (CS%debug) then
    call GOLD_state_chksum("aft 1st loop tridiag ", u, v, h, G)
  endif
  do J=Jsq,Jeq
    do i=is,ie
  ...
  call cpu_clock_end(id_clock_tridiag)
  if (CS%debug) then
    call GOLD_state_chksum("after tridiag ", u, v, h, G)
  endif
...
subroutine Calculate_Entrainment(u, v, h, tv, fluxes, dt, G, CS, ea, eb, kb, &
                                 Kd_Lay, Kd_int)
...
!   This subroutine calls the appropriate subroutine to calculate ea and eb, the
! rates at which a layer entrains from the layers above and below.
...
!  (in)      CS - The control structure returned by a previous call to
!                 diabatic_init.
!  (out)     ea - The amount of fluid entrained from the layer above
!                 within this time step, in units of m or kg m-2.
!  (out)     eb - The amount of fluid entrained from the layer below
!                 within this time step, in units of m or kg m-2.
!  (out)     kb - The index of the lightest layer denser than the
...
!  (in,opt)  Kd_int - The diapycnal diffusivity of interfaces, in m2 s-1.
  ...
  if (CS%use_H2000_entrainment) then
    call Entrainment_H2000(u, v, h, tv, fluxes, dt, G, CS%entrain_H2000_CSp, &
...
end subroutine Calculate_Entrainment
...
!                 diabatic_driver_init.
  ...
  do j=js,je
    ...
    ! above freezing, cool the surface layer with the frazil heat deficit.
          hc = (CS%C_p*G%H_to_kg_m2) * h(i,j,1)
    ...
    do k=nz,1,-1
      ...
      do i=is,ie
        if ((G%hmask(i,j) > 0.0) .and. &
          ...
          hc = (CS%C_p*G%H_to_kg_m2) * h(i,j,k)
...
!  (in)      G - The ocean's grid structure.
  ...
  if (.not.associated(tv%T)) call GOLD_error(FATAL, &
      "double_diffuse_T_S: Called with an unassociated tv%T")
  if (.not.associated(tv%S)) call GOLD_error(FATAL, &
      "double_diffuse_T_S: Called with an unassociated tv%S")
  if (.not.associated(visc%Kd_extra_T)) call GOLD_error(FATAL, &
      "double_diffuse_T_S: Called with an unassociated visc%Kd_extra_T")
  if (.not.associated(visc%Kd_extra_S)) call GOLD_error(FATAL, &
...
!  (in)      CS - The control structure returned by a previous call to
!                 diabatic_driver_init.
  real :: salt_add_col(SZ1_(h),SZ2_(h)) ! The accumulated salt requirement
...
!  (in)      diag_to_Z_CSp - A pointer to the Z-diagnostics control structure.
  ...
  type(vardesc) :: vd
  character(len=128) :: version = '$Id: GOLD_diabatic_driver.F90,v 13.0.2.9.2.49 2011/09/27 21:10:24 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod  = "GOLD_diabatic_driver" ! This module's name.
  ...
  if (associated(CS)) then
    call GOLD_error(WARNING, "diabatic_driver_init called with an "// &
  ...
  CS%bulkmixedlayer = .false. ; CS%use_sponge = .false.
  CS%ML_mix_first = 0.0 ; CS%use_geothermal = .false.
  CS%reclaim_frazil = .true. ; CS%pressure_dependent_frazil = .false.
  CS%use_kappa_shear = .false. ; CS%use_H2000_entrainment = .false.
  CS%debug = .false. ;  CS%massless_match_targets = .true.
  double_diffusion = .false.
  call read_param(param_file, "BULKMIXEDLAYER", CS%bulkmixedlayer)
  call read_param(param_file, "SPONGE", CS%use_sponge)
  call read_param(param_file, "TEMPERATURE", use_temperature,.true.)
  call read_param(param_file, "DOUBLE_DIFFUSION", double_diffusion)
  call read_param(param_file, "USE_JACKSON_PARAM", CS%use_kappa_shear)
  call read_param(param_file, "USE_H2000_SHEAR_MIXING", CS%use_H2000_entrainment)
  if (CS%bulkmixedlayer) call read_param(param_file, "ML_MIX_FIRST", CS%ML_mix_first)
  if (use_temperature) call read_param(param_file, "DO_GEOTHERMAL", CS%use_geothermal)
  call read_param(param_file, "MASSLESS_MATCH_TARGETS", CS%massless_match_targets)
  call read_param(param_file, "DEBUG",CS%debug)
  call read_param(param_file, "RECLAIM_FRAZIL", CS%reclaim_frazil)
  call read_param(param_file, "PRESSURE_DEPENDENT_FRAZIL", CS%pressure_dependent_frazil)
  ...
  CS%mix_boundary_tracers = .true.
  call read_param(param_file,"MIX_BOUNDARY_TRACERS",CS%mix_boundary_tracers)
  if (CS%mix_boundary_tracers) then
    call read_param(param_file,"KD",Kd,.true.) ; CS%Kd_min_tr = 0.1 * Kd
    call read_param(param_file,"KD_MIN_TR",CS%Kd_min_tr)
    CS%Kd_BBL_tr = 0.0
    call read_param(param_file, "KD_BBL_TR", CS%Kd_BBL_tr)
  endif
  ...
  call log_param(param_file, mod, "TEMPERATURE", use_temperature, &
                 ...
                 "variables.")
  call log_param(param_file, mod, "USE_JACKSON_PARAM", CS%use_kappa_shear, &
                 "If true, use the Jackson-Hallberg-Legg (JPO 2008) \n"//& 
                 "shear mixing parameterization.", default=.false.)
  call log_param(param_file, mod, "USE_H2000_SHEAR_MIXING", CS%use_H2000_entrainment, &
                 "If true, use the older (obsolete?) shear mixing \n"//& 
                 "parameterization from Hallberg (MWR 2000).", default=.false.)
  if (CS%bulkmixedlayer) &
    call log_param(param_file, mod, "ML_MIX_FIRST", CS%ML_mix_first, &
                 "The fraction of the mixed layer mixing that is applied \n"//&
                 "before interior diapycnal mixing.  0 by default.", &
                 units="nondim", default=0.0)
  if (use_temperature) then
    call log_param(param_file, mod, "DO_GEOTHERMAL", CS%use_geothermal, &
                 "If true, apply geothermal heating.", default=.false.)
  endif
  call log_param(param_file, mod, "MASSLESS_MATCH_TARGETS", &
                 ...
                 default=.true.)
  call log_param(param_file, mod, "RECLAIM_FRAZIL", CS%reclaim_frazil, &
  ...
  call log_param(param_file, mod, "PRESSURE_DEPENDENT_FRAZIL", &
  ...
  if (use_temperature) call log_param(param_file, mod, "C_P", CS%C_p, &
                 ...
                 units="J kg-1 K-1")
  call log_param(param_file, mod, "MIX_BOUNDARY_TRACERS", CS%mix_boundary_tracers, &
  ...
  if (CS%mix_boundary_tracers) then
    call log_param(param_file, mod, "KD_MIN_TR", CS%Kd_min_tr, &
    ...
    call log_param(param_file, mod, "KD_BBL_TR", CS%Kd_BBL_tr, &
  ...
  if (CS%use_H2000_entrainment) then
    call entrain_H2000_init(Time, G, param_file, diag, CS%entrain_H2000_CSp)
  else
  ...
  if (use_temperature) then
    nbands = 1 ; call read_param(param_file,"PEN_SW_NBANDS",nbands)
    if (nbands > 0) then
      allocate(CS%optics)
...
subroutine diabatic_driver_end(CS)
  ...
  if (CS%use_H2000_entrainment) then
    call entrain_H2000_end(CS%entrain_H2000_CSp)
  else
...
!                 present or both absent.
  ...
  real :: h_neglect    ! A thickness that is so small it is usually lost
                       ! in roundoff and can be neglected, in m or kg m-2.
  real :: b1(SZ1_(h)), d1(SZ1_(h)), c1(SZ1_(h),SZ3_(h))
  ...
  if (present(ea) /= present(eb)) call GOLD_error(FATAL, &
...
subroutine GOLD_state_chksum(mesg, u, v, h, G)
...
end module GOLD_diabatic_driver
