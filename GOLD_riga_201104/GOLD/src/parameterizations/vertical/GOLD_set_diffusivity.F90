...
use GOLD_diag_mediator, only : safe_alloc_ptr, post_data, register_diag_field
use GOLD_diag_to_Z, only : diag_to_Z_CS, register_Zint_diag, calc_Zint_diags
...
use GOLD_intrinsic_functions, only : invcosh
use GOLD_io, only : slasher, vardesc
use GOLD_thickness_diffuse, only : vert_fill_TS
use GOLD_variables, only : forcing, thermo_var_ptrs, vertvisc_type, p3d
...
type, public :: set_diffusivity_CS ; private
  ...
  real :: I_zeta             ! Inverse decay scale for wave-generated TKE (m-1)
  ...
  real :: Mu_itides          ! Efficiency factor for conversion of dissipation
                             ! to potential energy
  real :: Gamma_itides       ! Fraction of local dissipation
  real :: min_zbot_itides    ! minimum ocean depth for internal tide conversion
  logical :: Int_tide_dissipation  ! Internal tide conversion.
  logical :: Lee_wave_dissipation  ! Enable Lee-wave driven mixing. (Nikurashin, 2010)
  logical :: limit_dissipation ! If enabled, dissipation is limited to be larger than:
  ...
  logical :: Riga_set_diffusivity ! If true, use the Riga version of the code.
  ...
  logical :: ML_use_omega    !   If true, use the absolute rotation rate instead
                             ...
                             ! mixed layer.
  logical :: double_diffusion         ! flag to enable double-diffusive mixing
  ...
  integer :: id_Kd_itidal = -1, id_Kd_Niku = -1
  integer :: id_N2_z = -1, id_Kd_itidal_z = -1, id_Kd_Niku_z = -1
  integer :: id_Kd_Work = -1, id_Kd_Itidal_Work = -1, id_Kd_Niku_Work = -1
  integer :: id_maxTKE = -1, id_TKE_to_Kd = -1
...
type diffusivity_diags
  real, pointer, dimension(:,:,:) :: &
    N2_3d => NULL(), &     ! The squared buoyancy frequency at interfaces, in s-2.
    Kd_itidal => NULL(), & ! The internal tide diffusivity at interfaces, in m2 s-1.
    Kd_Niku => NULL(), &   ! The lee-wave diffusivity at interfaces, in m2 s-1.
    ...
    Kd_Itidal_Work => NULL(), &  ! The work done by lee-wave driven mixing
                           ! integrated through layers, in W m-2.
    maxTKE => NULL(), &    ! The energy required to entrain to h_max, in m3 s-3.
    TKE_to_Kd => NULL(), & ! The conversion rate (~1.0 / (G_Earth + dRho_lay))
                           ! between TKE dissipated within a layer and Kd in
                           ! that layer, in m2 s-1 / m3 s-3 = s2 m-1.
    KT_extra => NULL(), &  ! Additional diffusivities due to double-diffusion of
    KS_extra => NULL()     ! temperature and salinity, in m2 s-1.
  real, pointer, dimension(:,:) :: &
    TKE_itidal_used => NULL() ! The internal tide TKE input at the bottom of
                           ! the ocean, in Watt meter-2.
end type diffusivity_diags

! integer :: id_clock_Itide
contains
...
!  (out,opt) Kd_int - The diapycnal diffusivity at each interface in m2 s-1.
  ...
  logical :: use_EOS    ! If true, density is calculated from T & S using an
                        ! equation of state.
  type(p3d) :: z_ptrs(5) ! Pointers to the diagnostics that are to be
  ...
  integer :: num_z_diags  ! The number of diagnostics that are to be
                       ! interpolated into depth space.
  integer :: z_ids(5)  ! The id numbers of the diagnostics that are to be
                       ! interpolated into depth space.
  ...
  if (.not.associated(CS)) call GOLD_error(FATAL,"set_diffusivity: "//&
  ...
  if ((CS%double_diffusion) .and. &
    ...
    call GOLD_error(FATAL, "set_diffusivity: visc%Kd_extra_T and "//&
...
! Set up arrays for diagnostics.
  ...
  if (CS%id_Kd_Itidal_work > 0) then
    allocate(dd%Kd_Itidal_work(isd:ied,jsd:jed,nz)) ; dd%Kd_Itidal_work(:,:,:) = 0.0
  endif
  if (CS%id_TKE_itidal > 0) then
    allocate(dd%TKE_Itidal_used(isd:ied,jsd:jed)) ; dd%TKE_Itidal_used(:,:) = 0.0
  endif
...
! another explicit parameterization of Kd.
  ...
  ! Add the input turbulent diffusivity.
    if (CS%Add_input_Kd) then
      ...
      enddo ; enddo
      do i=is,ie
        Kd(i,j,1) = Kd(i,j,1) + 0.5*(visc%Kd_turb(i,j,1) + visc%Kd_turb(i,j,2))
      enddo
    else
      if (present(Kd_int)) then
        do i=is,ie
    ...
    if (CS%Riga_set_diffusivity) then
      if (CS%Int_tide_dissipation .or. CS%ML_radiation .or. CS%Henyey_IGW_background_new &
         .or. CS%Lee_wave_dissipation) then
        call add_energy_diffusivity_orig(h, T_f, S_f, tv, fluxes, visc, j, &
                                         G, CS, dd, Kd, Kd_int)
      endif
    ...
    ! by the bottom drag.
      if (CS%bottomdraglaw .and. (CS%BBL_effic>0.0)) then
        call find_TKE_to_Kd(h, tv, dRho_int, N2_lay, j, dt, G, CS, TKE_to_Kd, maxTKE, kb)
      ...
      ! Add the Nikurashin + tidal bottom-driven mixing.
      ...
      if (CS%limit_dissipation) then
        ...
        !   3) dissipation corresponding to a (naarly) constant diffusivity.
      ...
      if (associated(dd%Kd_work)) then
        do k=1,nz ; do i=is,ie
          dd%Kd_Work(i,j,k)  = G%Rho0 * Kd(i,j,k) * N2_lay(i,k) * G%H_to_m*h(i,j,k)  ! Watt m-2 s or kg s-3
        enddo ; enddo
      endif
    endif
  enddo ! j-loop
  ...
  if (CS%Int_tide_dissipation .or. CS%Lee_wave_dissipation) then
    ...
    if (CS%id_Kd_Itidal_Work > 0) call post_data(CS%id_Kd_Itidal_Work, dd%Kd_Itidal_Work, CS%diag)
...
!        maxEnt(i,k) = ds_dsp1(i,k)*(maxEnt(i,k-1) + htot(i)) ! BITWISE CHG
  ...
  do k=kb_min,nz-1 ; do i=is,ie
    if (k<kb(i)) then
      ...
      if (CS%Riga_set_diffusivity) then
        dh_max = maxEnt(i,k) * (1.0 + gkp1_gk(k))
        maxTKE(i,k) = ( (I_dt*G%g_prime(k+1)) * (1.0/(1.0+gkp1_gk(k))) ) * &
                   ((G%H_to_m*h(i,j,k) + dh_max) * dh_max)
        TKE_to_Kd(i,k) = I_glay(k)
!       dh_max = maxEnt(i,k) * (1.0 + dsp1_ds(i,k))
!       maxTKE(i,k) = (I_dt * (G%g_prime(k+1) * maxEnt(i,k)) ) * &
!                      (G%H_to_m*h(i,j,k) + dh_max)
!       TKE_to_Kd(i,k) = 2.0 / (G%g_prime(k)+G%g_prime(k+1))
      else
...
subroutine find_N2(h, tv, T_f, S_f, fluxes, j, G, CS, dRho_int, N2_lay, N2_int, N2_bot)
  ...
  elseif (CS%bulkmixedlayer) then
    ! T & S are not state variables.  Set dRho_int from Rml and the layer properties.
    do k=2,tv%nk_Rml ; do i=is,ie
      dRho_int(i,k) = max(tv%Rml(i,j,k) - tv%Rml(i,j,k-1), 0.0)
    enddo ; enddo
    do k=tv%nk_Rml+1,nz ; do i=is,ie
      Rml_base = tv%Rml(i,j,tv%nk_Rml)
      dRho_int(i,k) = max(G%Rlay(k), Rml_base) - max(G%Rlay(k-1), Rml_base)
    enddo ; enddo
  else
    do k=2,nz ; do i=is,ie
...
! by the bottom drag.
  ...
  do k=2,nz-1
    Rint(k) = 0.5*(G%Rlay(k-1)+G%Rlay(k))
  enddo
  Rint(nz) = 0.5*(G%Rlay(nz-1)+G%Rlay(nz))
  ...
  ! The turbulence decay scale is 0.5*ustar/f from K&E & GOLD_vertvisc.F90
...
subroutine add_int_tide_diffusivity(h, N2_bot, j, TKE_to_Kd, max_TKE, G, CS, &
  ...
  real, dimension(SZI_(G)) :: &
    ...
    TKE_itidal_rem, & ! The remaining internal tide TKE.
    TKE_Niku_rem, &   ! The remaining lee-wave TKE.
  ...
  do k=nz-1,2,-1 ; do i=is,ie
    ...
    TKE_Niku_lay = TKE_Niku_rem(i) - TKE_Niku_bot(i)*TKE_frac_top(i)
    ...
    if (associated(dd%Kd_Itidal_work)) dd%Kd_itidal_work(i,j,k) = G%Rho0 * TKE_itide_lay
    if (associated(dd%Kd_Niku_work)) dd%Kd_itidal_work(i,j,k) = G%Rho0 * TKE_Niku_lay
  enddo ; enddo

end subroutine add_int_tide_diffusivity

subroutine add_energy_diffusivity_orig(h, T_f, S_f, tv, fluxes, visc, j, &
                                       G, CS, dd, Kd, Kd_int)
  real, dimension(NXMEM_,NYMEM_,NZ_),   intent(in)    :: h, T_f, S_f
  type(thermo_var_ptrs),                intent(in)    :: tv
  type(forcing),                        intent(in)    :: fluxes
  type(vertvisc_type),                  intent(in)    :: visc
  integer,                              intent(in)    :: j
  ...
  real, dimension(SZ3_(h)+1) :: &
       N2, N2_pos, I_dz_int, pressure, T_int, Sal_int, dbuoy_dT, dbuoy_dS, zint
  real, dimension(SZ1_(h),SZ3_(h)+1) :: &
       pressure_2d, Kd_2d, T_2d, S_2d, T_int_2d, Sal_int_2d, dz_2d,&
       zint_2d, N2_2d
  real, dimension(SZ3_(h)) :: &
       dz,Idz, T, S
  real, dimension(SZI_(G), SZJ_(G)) :: &
    TKE_itidal    ! Tidal dissipation energy (Watts m-2)
  real :: I_rho0        ! 1 / RHO0
  real :: Kd_Niku       ! Diffusivity calculated from TKE input of Lee wave
                        ! driven mixing. Reference: Nikurashin (2010)
  real :: N2_lay        ! The bouyancy frequency interpolated to layers.
  ! The following are temporary variables which still need to be documented.
  real :: z_bot, fz, I_Nsq_Om2, gR0, g_R0, hb, h1, dz_bot, C1_6
  real :: h_amp, z_lay, z_layp, Iz_bot, Izeta, h_ml, I_decay
  real :: z1          ! The thickness around an interface times the I_decay, ND.
  real :: dzL         ! The thickness around an interface, in m.
  real :: I_decay_len2_TKE  ! The squared inverse decay lengthscale for
                            ! TKE, as used in the mixed layer code, in m-2.

  ! The following are temporary variables which still need to be documented.
  real :: Nb, Kd_itide, f_sq, h_ml_sq, ustar_sq, TKE_ml_flux, Kd_mlr
  real :: Ifac, Omega2, Nb2, Nb2_pos
  real :: dissip      ! local variable for dissipation calculations, W m-3

  integer :: i, k, is, ie, nz, kml
  is = G%isc ; ie = G%iec ; nz = G%ke

! This adds several different energy-based diffusivities.
  if (.not.(CS%Int_tide_dissipation .or. CS%ML_radiation .or. CS%Henyey_IGW_background_new &
     .or. CS%Lee_wave_dissipation)) return

  I_Rho0 = 1.0/G%Rho0

  gR0 = G%Rho0*G%g_Earth
  g_R0 =  G%g_Earth/G%Rho0
  Omega2 = CS%Omega*CS%Omega
  C1_6 = 1.0 / 6.0

! Add the input turbulent diffusivity.
! due to internal tidal interactions with bottom slopes
! ala St. Laurent et al, GRL (2002) ; Simmons, Ocean Modelling (2004)

  do i=is,ie
    pressure_2d(i,1) = 0.0
    T_2d(i,1) = T_f(i,j,1)
    S_2d(i,1) = S_f(i,j,1)
    dz_2d(i,1) = G%H_to_m*h(i,j,1)
    zint_2d(i,1) = 0.0
    Kd_2d(i,1) = 0.0
    N2_2d(i,1) = Omega2
  enddo
  if (associated(dd%N2_3d)) then ; do i=is,ie
    dd%N2_3d(i,j,1) = 0.0
    dd%N2_3d(i,j,nz+1) = 0.0
  enddo ;  endif
  ...
  do k=2,nz;  do i=is,ie
    ...
    T_2d(i,k) = T_f(i,j,k)
    S_2d(i,k) = S_f(i,j,k)
    dz_2d(i,k) = G%H_to_m*h(i,j,k)
    zint_2d(i,k) = zint_2d(i,k-1) + dz_2d(i,k-1)
    T_int_2d(i,k) = 0.5*(T_2d(i,k-1) + T_2d(i,k))
    Sal_int_2d(i,k) = 0.5*(S_2d(i,k-1) + S_2d(i,k))
    pressure_2d(i,k) = gR0*zint_2d(i,k) ! at top of layer
  enddo; enddo

  do i=is, ie
    zint_2d(i,nz+1) = zint_2d(i,nz)+dz_2d(i,nz)
    pressure_2d(i,nz+1)=gR0*zint_2d(i,nz+1) ! bottom pressure
  enddo
  ...
  do i=is,ie
    do k=1,nz
      T_int(k) = T_int_2d(i,k)
      Sal_int(k) = Sal_int_2d(i,k)
      pressure(k) = pressure_2d(i,k)
      dz(k) = dz_2d(i,k)
      T(k) = T_2d(i,k)
      S(k) = S_2d(i,k)
      zint(k) = zint_2d(i,k)
    ...
    pressure(nz+1)=pressure_2d(i,nz+1)
    zint(nz+1)=zint_2d(i,nz+1)

    call calculate_density_derivs(T_int, Sal_int, pressure, dbuoy_dT, &
        dbuoy_dS, 2, nz-1, tv%eqn_of_state)
    ...
    do k=2,nz
      Idz(k) = 1.0/dz(k)
      I_dz_int(k) = 2.0/(dz(k)+dz(k-1))
      N2(k) = (dbuoy_dT(k) * (T(k-1) - T(k)) + &
               dbuoy_dS(k) * (S(k-1) - S(k))) * I_dz_int(k)
      N2_pos(k) = max(N2(k), 0.0)
      ! N2(k) is used in itides and Henyey. CS%N2_FLOOR_IOMEGA2
      ! was originally =1 but we now think it should be =0. -aja
      N2(k) = max(N2(k),CS%N2_FLOOR_IOMEGA2*Omega2)
    enddo

    hb=0.0 ; Nb2=0.0 ; Nb2_pos=0.0

    if (CS%Int_tide_dissipation .or. CS%Lee_wave_dissipation) then
      h_amp = sqrt(CS%h2(i,j)) ! for computing Nb
    else
      h_amp = 0.0
    endif

    do k=nz,2,-1 ! This only applies within bottom boundary layers.
      z_lay = 0.5*(zint(k)+zint(k-1)) ! middle of the layer above
      dz_bot = z_bot-z_lay ! distance from depth

      if (dz_bot > h_amp) then
        h1 = 0.5*(dz(k)+dz(k-1))
        hb = hb+h1
        Nb2 = Nb2 + N2(k)*h1
        Nb2_pos = Nb2_pos + N2_pos(k)*h1
        if (k>2) then
          h1 = 0.5*(dz(k-2)+dz(k-1))
          hb = hb+h1
          Nb2 = Nb2 + N2(k-1)*h1
          Nb2_pos = Nb2_pos + N2_pos(k-1)*h1
        endif
        exit ! the loop after the topmost layer in the BBL.
      else
        h1 = 0.5*(dz(k)+dz(k-1))
        Nb2 = Nb2 + N2(k)*h1 ;  Nb2_pos = Nb2_pos + N2_pos(k)*h1
        hb = hb+h1
      endif
    enddo

    Nb2 = Nb2 / hb ;  Nb2_pos = Nb2_pos / hb
    Nb = sqrt(Nb2)
    do k=nz,2,-1 ! This only applies within bottom boundary layers.
      z_lay = 0.5*(zint(k)+zint(k-1)) ! middle of the layer
      dz_bot = z_bot-z_lay ! distance from depth
      if (dz_bot <= h_amp) then
        N2(k) = Nb2 ; N2_pos(k) = Nb2_pos
      else
        N2(k) = Nb2 ; N2_pos(k) = Nb2_pos
        if (k>2) then
          N2(k-1) = Nb2 ; N2_pos(k-1) = Nb2_pos
        endif
        exit ! the loop after the topmost layer in the BBL.
      endif
    enddo

!       ! The above block/loop could be written as follows:
!       do k=nz+1,2,-1 ! This only applies within bottom boundary layers.
!         z_lay = 0.5*(zint(k)+zint(k-1)) ! middle of the layer
!         dz_bot = z_bot-z_lay ! distance from depth
!         if (dz_bot > hh_amp) exit   ! do not update N2(k) above the BBL
!         N2(k-1) = Nb2               ! set N2 at top of layer to Nb2
!       enddo
!
!       ! Or better yet, without an "exit":
!       k=nz+1
!       do while (k>2 .and. dz_bot <= hh_amp)  ! i.e. within the BBL
!         z_lay = 0.5*(zint(k)+zint(k-1))      ! middle of the layer
!         dz_bot = z_bot-z_lay                 ! distance from bottom
!         k=k-1
!         N2(k) = Nb2                          ! set N2 at top of layer to Nb2
!       enddo

    N2(nz+1) = N2(nz) ; N2_pos(nz+1) = N2_pos(nz)
    fz=0.0

    if (CS%Int_tide_dissipation .or. CS%Lee_wave_dissipation .or. CS%limit_dissipation) then
      CS%Nb(i,j) = Nb*CS%mask_itidal(i,j)
      TKE_itidal(i,j) = CS%TKE_itidal(i,j)*CS%Nb(i,j)
      TKE_itidal(i,j) = min(TKE_itidal(i,j),CS%TKE_itide_max)
      if (associated(dd%TKE_itidal_used)) dd%TKE_itidal_used(i,j) = TKE_itidal(i,j)
      Izeta = CS%I_zeta
      Ifac = 1.0 / (exp(Izeta*z_bot) - 1.0)

      do k = 2,nz
        I_Nsq_Om2 =  1.0/(N2(k)+Omega2)
        ! Analytically integrate eqn(4) from Simmons, 2004
        z_layp = 0.5*(zint(k+1)+zint(k))
        z_lay = 0.5*(zint(k)+zint(k-1))
        ! fz should be the average of the Simmons function over the layer.
          ! sum_k(fz*h) ~= 1.0
        fz = Ifac*(exp(Izeta*z_layp) - exp(Izeta*z_lay)) / &
             (0.5*(zint(k+1) - zint(k-1)))
        N2_2d(i,k) = N2(k)
        if (CS%Int_tide_dissipation) then
          Kd_itide = fz * I_Nsq_Om2 * TKE_itidal(i,j) * I_rho0 * &
                    CS%Mu_itides * CS%Gamma_itides
          Kd_2d(i,k) = Kd_itide+Kd_2d(i,k)
          if (associated(dd%Kd_itidal)) dd%Kd_itidal(i,j,k) = Kd_itide
        endif
        if (CS%Lee_wave_dissipation) then
          Kd_Niku = fz * I_Nsq_Om2 * CS%TKE_Niku(i,j) * I_rho0 * &
                    CS%Mu_itides * CS%Gamma_itides ! We are re-using the i-tide
                    ! parameters for lack of a better idea, until the the scheme
                    ! has been evaluated. -AJA
          Kd_2d(i,k) = Kd_Niku+Kd_2d(i,k)
          if (associated(dd%Kd_Niku)) dd%Kd_Niku(i,j,k) = Kd_Niku
        endif
      enddo

      Kd_2d(i,nz+1) = Kd_2d(i,nz)
      N2_2d(i,nz+1) = N2(nz+1)
      if (associated(dd%Kd_itidal)) then
        dd%Kd_itidal(i,j,1) = 0.0
        dd%Kd_itidal(i,j,nz+1) = dd%Kd_itidal(i,j,nz)
      endif
      if (CS%Lee_wave_dissipation .and. associated(dd%Kd_Niku)) then
        dd%Kd_Niku(i,j,1) = 0.0
        dd%Kd_Niku(i,j,nz+1) = dd%Kd_Niku(i,j,nz)
      endif
    ...
    if (CS%ML_radiation) then
      ...
      kml = CS%nkml
      h_ml=0.0 ; do k=1,kml ; h_ml = h_ml + dz_2d(i,k) ; enddo

      ustar_sq = max(fluxes%ustar(i,j), CS%ustar_min)**2

      TKE_ml_flux = (CS%mstar*CS%ML_rad_coeff)*(ustar_sq*fluxes%ustar(i,j))
      I_decay_len2_TKE = CS%TKE_decay**2 * (f_sq / ustar_sq)

      if (CS%ML_rad_TKE_decay) &
        TKE_ml_flux = TKE_ml_flux * exp(-h_ml * sqrt(I_decay_len2_TKE))

      ! Calculate the inverse decay scale
      h_ml_sq = CS%ML_rad_efold_coeff**2 * h_ml**2
      I_decay = sqrt((I_decay_len2_TKE * h_ml_sq + 1.0) / h_ml_sq)
      ! Average the dissipation over the top half of layer kml+1, using
      ! a more accurate Taylor series approximations for very thin layers.
      z1 = 0.5*dz_2d(i,kml+1)*I_decay
      if (z1 > 1e-5) then
        Kd_mlr = TKE_ml_flux * ((1.0 - exp(-z1)) / &
                                (0.5*dz_2d(i,kml+1) * (N2(kml+1) + Omega2)))
      else
        Kd_mlr = TKE_ml_flux * ((I_decay * (1.0 - z1 * (0.5 - C1_6*z1))) / &
                                (N2(kml+1) + Omega2))
      endif
      Kd_mlr = min(Kd_mlr,CS%ML_rad_kd_max)

      do k=kml+1,1,-1 ; Kd_2d(i,k) = Kd_2d(i,k) + Kd_mlr ; enddo
      TKE_ml_flux = TKE_ml_flux * exp(-z1)

      do k=kml+2,nz-1
        dzL = 0.5*(dz_2d(i,k-1) + dz_2d(i,k)) ;  z1 = dzL*I_decay
        if (z1 > 1e-5) then
          Kd_mlr = TKE_ml_flux * ((1.0 - exp(-z1)) / &
                                  (dzL  * (N2(k) + Omega2)))
        else
        ...
        Kd_mlr = min(Kd_mlr,CS%ML_rad_kd_max)
        Kd_2d(i,k) =  Kd_2d(i,k) + Kd_mlr

        TKE_ml_flux = TKE_ml_flux * exp(-z1)
        if (TKE_ml_flux * I_decay < 0.1*CS%Kd_min*Omega2) exit
      enddo
    endif
    ...
    if (CS%limit_dissipation) then
      ...
      !   3) dissipation corresponding to a (naarly) constant diffusivity.
        dissip = max( CS%dissip_min, &   ! Const. floor on dissip.
                      CS%dissip_N0 + CS%dissip_N1 * sqrt(N2_pos(k)), & ! Floor aka Gargett
                      CS%dissip_N2 * N2_pos(k) ) ! Floor of Kd_min*rho0/F_Ri
        Kd_2d(i,k) = max( Kd_2d(i,k) , &  ! Apply floor to Kd
           dissip * (CS%FluxRi_max / (G%Rho0 * (N2_pos(k) + Omega2))) )
      enddo
    endif

  enddo ! i-loop

  ! dont try to diagnose mixing work in the top layer.
  if (associated(dd%Kd_work)) dd%Kd_Work(:,j,1)=0.0
  if (associated(dd%Kd_Itidal_Work)) dd%Kd_Itidal_Work(:,j,1)=0.0
  if (associated(dd%Kd_Niku_Work)) dd%Kd_Niku_Work(:,j,1)=0.0
  do k=2,nz ; do i=is,ie
    if (present(Kd_int)) Kd_int(i,j,k) = Kd_int(i,j,k) + Kd_2d(i,k)
    Kd(i,j,k) = Kd(i,j,k) + 0.5*(Kd_2d(i,k)+Kd_2d(i,k+1))
    N2_lay = 0.5*(N2_2d(i,k) + N2_2d(i,k+1))  ! move the interface N2 to layers (This is a bit crazy.)
!    if (associated(dd%n2_3d)) dd%N2_3d(i,j,k) = N2_lay
    if (associated(dd%n2_3d)) dd%N2_3d(i,j,k) = N2_2d(i,k)
    if (associated(dd%Kd_work)) &
      dd%Kd_Work(i,j,k)  = G%Rho0 * Kd(i,j,k) * N2_lay * h(i,j,k)  ! Watt m-2 s or kg s-3
    if (associated(dd%Kd_Itidal_Work)) then
      Kd_itide = 0.5*(dd%Kd_itidal(i,j,k)+dd%Kd_itidal(i,j,k+1))
      dd%Kd_Itidal_Work(i,j,k)  = G%Rho0 * Kd_itide * N2_lay * h(i,j,k)  ! Watt m-2 s or kg s-3
    endif
    if (associated(dd%Kd_Niku_Work)) then
      Kd_Niku = 0.5*(dd%Kd_Niku(i,j,k)+dd%Kd_Niku(i,j,k+1))
      dd%Kd_Niku_Work(i,j,k)  = G%Rho0 * Kd_Niku * N2_lay * h(i,j,k)  ! Watt m-2 s or kg s-3
    endif
  enddo ; enddo

end subroutine add_energy_diffusivity_orig
...
subroutine set_BBL_diffusivity(u, v, h, fluxes, visc, G, CS)
  ...
  if (.not.associated(CS)) call GOLD_error(FATAL,"set_BBL_diffusivity: "//&
         "Module must be initialized before it is used.")
...
! interfaces above and below the buffer layer and the next denser layer.
        ...
        do k3=1,kmb
          a(k3+1) = (G%Rlay(k) - tv%Rml(i,j,k3)) * I_Drho
        enddo
...
!  (in)      diag_to_Z_CSp - A pointer to the Z-diagnostics control structure.
  ...
  character(len=128) :: version = '$Id: GOLD_set_diffusivity.F90,v 1.1.2.56 2011/04/05 18:39:05 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_set_diffusivity"  ! This module's name.
  ...
  CS%bottomdraglaw = .false. ; CS%Bryan_Lewis_diffusivity = .false.
  CS%Henyey_IGW_background = .false. ; CS%ML_radiation = .false.
  CS%Kd_tanh_lat_fn = .false. ; CS%Kd_tanh_lat_scale = 0.0
  CS%Henyey_IGW_background_new = .false.
  CS%BBL_mixing_as_max = .true.
  CS%Riga_set_diffusivity = .true.

  call read_param(param_file,"BULKMIXEDLAYER",CS%bulkmixedlayer)
  CS%nkml = 1 ; call read_param(param_file,"NKML",CS%nkml)
  call read_param(param_file,"BOTTOMDRAGLAW",CS%bottomdraglaw)
  call read_param(param_file,"RIGA_SET_DIFFUSIVITY",CS%Riga_set_diffusivity)
  call read_param(param_file,"BRYAN_LEWIS_DIFFUSIVITY",CS%Bryan_Lewis_diffusivity)
  call read_param(param_file,"HENYEY_IGW_BACKGROUND",CS%Henyey_IGW_background)
  call read_param(param_file,"KD_TANH_LAT_FN",CS%Kd_tanh_lat_fn)
  if (CS%Henyey_IGW_background .and. CS%Kd_tanh_lat_fn) call GOLD_error(FATAL, &
    "Set_diffusivity: KD_TANH_LAT_FN can not be used with HENYEY_IGW_BACKGROUND.")
  if (CS%Kd_tanh_lat_fn) &
    call read_param(param_file,"KD_TANH_LAT_SCALE",CS%Kd_tanh_lat_scale)

  call read_param(param_file,"ML_RADIATION",CS%ML_Radiation)
  CS%omega = 7.2921e-5 ; call read_param(param_file,"OMEGA",CS%omega)
  if (CS%ML_Radiation) then
    CS%ML_use_omega = .false.
    CS%ML_rad_coeff = 0.2; CS%ML_rad_kd_max = 1.e-3
    CS%ML_rad_efold_coeff = 0.2 ; CS%ML_rad_TKE_decay = .true. ; CS%TKE_decay = 2.5
     call read_param(param_file,"ML_RAD_EFOLD_COEFF",CS%ML_rad_efold_coeff)
    call read_param(param_file,"ML_RAD_KD_MAX",CS%ML_rad_kd_max)
    call read_param(param_file,"ML_RAD_COEFF",CS%ML_rad_coeff)
    call read_param(param_file,"ML_RAD_APPLY_TKE_DECAY", CS%ML_rad_TKE_decay)
    call read_param(param_file,"MSTAR",CS%mstar,.true.)
    call read_param(param_file,"TKE_DECAY",CS%TKE_decay)
    ...
    call read_param(param_file,"ML_USE_OMEGA",CS%ML_use_omega)
  endif

  CS%debug = .false. ; call read_param(param_file,"DEBUG",CS%debug)
  CS%N0_2Omega=20.0 ; call read_param(param_file,"HENYEY_N0_2OMEGA",CS%N0_2Omega)
  CS%N2_FLOOR_IOMEGA2=1.;call read_param(param_file, "N2_FLOOR_IOMEGA2", CS%N2_FLOOR_IOMEGA2)

  CS%Add_input_Kd = .false.
  call read_param(param_file,"USE_JACKSON_PARAM",CS%Add_input_Kd)

  CS%Int_tide_dissipation=.false.
  call read_param(param_file,"INT_TIDE_DISSIPATION",CS%Int_tide_dissipation)

  if (CS%Int_tide_dissipation .and. CS%Bryan_Lewis_diffusivity) then
    call GOLD_error(FATAL,"GOLD_Set_Diffusivity: "// &
         "BRYAN-LEWIS AND INTERNAL TIDAL DISSIPATION ARE BOTH ENABLED. CHOOSE ONE.")
  endif

  CS%Lee_wave_dissipation=.false.
  call read_param(param_file,"LEE_WAVE_DISSIPATION",CS%Lee_wave_dissipation)
  if (CS%Lee_wave_dissipation) then
    if (.not.CS%Int_tide_dissipation) call GOLD_error(FATAL,"GOLD_Set_Diffusivity: "// &
         "LEE WAVE DISSIPATION NEEDS INTERNAL TIDAL DISSIPATION TO BE ENABLED.")
    call safe_alloc_ptr(CS%TKE_Niku,is,ie,js,je); CS%TKE_Niku(:,:) = 0.0
    Niku_TKE_input_file=''
    call read_param(param_file,"NIKURASHIN_TKE_INPUT_FILE",Niku_TKE_input_file,.true.)
    ...
    Niku_scale=1.; call read_param(param_file,"NIKURASHIN_SCALE",Niku_scale)
    CS%TKE_Niku(:,:) = Niku_scale * CS%TKE_Niku(:,:)
  endif
  CS%limit_dissipation=.false.
  CS%dissip_min=0.; call read_param(param_file, "DISSIPATION_MIN", CS%dissip_min)
  if (CS%dissip_min>0.) CS%limit_dissipation=.true.
  CS%dissip_N1=0.; call read_param(param_file, "DISSIPATION_N1", CS%dissip_N1)
  if (CS%dissip_N1>0.) CS%limit_dissipation=.true.
  CS%dissip_N0=0.; call read_param(param_file, "DISSIPATION_N0", CS%dissip_N0)
  if (CS%dissip_N0>0.) CS%limit_dissipation=.true.
  CS%dissip_Kd_min=0.; call read_param(param_file, "DISSIPATION_KD_MIN", CS%dissip_Kd_min)
  if (CS%dissip_Kd_min>0.) CS%limit_dissipation=.true.
  CS%FluxRi_max = 0.2 ; call read_param(param_file, "FLUX_RI_MAX", CS%FluxRi_max)
  ...
  CS%Kd_min = 0.01*CS%Kd ; call read_param(param_file,"KD_MIN",CS%Kd_min)
  CS%Kd_add = 0.0 ; call read_param(param_file,"KD_ADD",CS%Kd_add)
  CS%Kd_max = -1.0 ; call read_param(param_file,"KD_MAX",CS%Kd_max)

  if (.not.CS%bulkmixedlayer) then
    call read_param(param_file,"KDML",CS%Kdml,.true.)
    call read_param(param_file,"HMIX",CS%Hmix,.true.)
  else
    CS%Kdml = CS%Kd ! This should not be used, but also cannot be a NaN.
  endif

  CS%cdrag = 0.0 ; CS%BBL_effic = 0.0 ; CS%IMax_decay = 1.0/200.0
  if (CS%bottomdraglaw) then
    call read_param(param_file,"CDRAG",CS%cdrag, .true.)
    call read_param(param_file,"BBL_EFFIC",CS%BBL_effic, .true.)
    decay_length = 0.0
    call read_param(param_file,"BBL_MIXING_MAX_DECAY", decay_length)
    if (decay_length > 0.0) CS%IMax_decay = 1.0/decay_length
    call read_param(param_file,"BBL_MIXING_AS_MAX", CS%BBL_mixing_as_max)
  endif

  if (CS%Bryan_Lewis_diffusivity) then
    call read_param(param_file,"KD_BRYAN_LEWIS_DEEP",CS%Kd_Bryan_Lewis_deep,.true.)
    call read_param(param_file,"KD_BRYAN_LEWIS_SURFACE",CS%Kd_Bryan_Lewis_surface,.true.)
    call read_param(param_file,"BRYAN_LEWIS_DEPTH_CENT",CS%Bryan_Lewis_depth_cent,.true.)
    call read_param(param_file,"BRYAN_LEWIS_WIDTH_TRANS",CS%Bryan_Lewis_width_trans,.true.)
  endif

  ! Write all relevant parameters to the model log.
  call log_version(param_file, mod, version, tagname)
  call log_param(param_file, mod, "BULKMIXEDLAYER", CS%bulkmixedlayer, &
                 "If defined, use a Kraus-Turner-like bulk mixed layer \n"//&
                 "with transitional buffer layers.  Layers 1 through \n"//&
                 "NKML+NKBL have variable densities. There must be at \n"//&
                 "least NKML+NKBL+1 layers if BULKMIXEDLAYER is true.", &
                 default=.false.)
  call log_param(param_file, mod, "RIGA_SET_DIFFUSIVITY", CS%Riga_set_diffusivity, &
                 "If true, use the Riga-style code that sets the \n"//&
                 "diffusivity, rather than the newer energy-conscience form.", &
                 default=.true.)
  call log_param(param_file, mod, "FLUX_RI_MAX", CS%FluxRi_max, &
  ...
  if (CS%bulkmixedlayer) then
    ...
    call log_param(param_file, mod, "KDML", CS%Kdml, &
                 "If BULKMIXEDLAYER is false, KDML is the elevated \n"//&
                 "diapycnal diffusivity in the topmost HMIX of fluid. \n"//&
                 "KDML is only used if BULKMIXEDLAYER is false.", units="m2 s-1")
    call log_param(param_file, mod, "HMIX", CS%Hmix, &
                 "If BULKMIXEDLAYER is false, HMIX is the depth over \n"//&
                 "which the diapycnal diffusivity is elevated.  HMIX \n"//&
                 "is only used directly if BULKMIXEDLAYER is false.", units="m")
  endif
  call log_param(param_file, mod, "ML_RADIATION", CS%ML_radiation, &
  ...
  if (CS%ML_radiation) then
    call log_param(param_file, mod, "ML_RAD_EFOLD_COEFF", CS%ML_rad_efold_coeff, &
                 ...
                 default=0.2)
    call log_param(param_file, mod, "ML_RAD_KD_MAX", CS%ML_rad_kd_max, &
                 ...
                 default=1.0e-3)
    call log_param(param_file, mod, "ML_RAD_COEFF", CS%ML_rad_coeff, &
    ...
    call log_param(param_file, mod, "ML_RAD_APPLY_TKE_DECAY", CS%ML_rad_TKE_decay, &
                 ...
                 default=.true.)
    call log_param(param_file, mod, "MSTAR", CS%mstar, &
                 "The ratio of the friction velocity cubed to the TKE \n"//&
                 "input to the mixed layer.", "units=nondim")
    call log_param(param_file, mod, "TKE_DECAY", CS%TKE_decay, &
                 "The ratio of the natural Ekman depth to the TKE decay scale.", &
                 units="nondim", default=2.5)
    call log_param(param_file, mod, "ML_USE_OMEGA", CS%ML_use_omega, &
  ...
  call log_param(param_file, mod, "BOTTOMDRAGLAW", CS%bottomdraglaw, &
  ...
  if  (CS%bottomdraglaw) then
    call log_param(param_file, mod, "CDRAG", CS%cdrag, &
                 "The drag coefficient relating the magnitude of the \n"//&
                 "velocity field to the bottom stress. CDRAG is only used \n"//&
                 "if BOTTOMDRAGLAW is true.", units="nondim", default=0.0)
    call log_param(param_file, mod, "BBL_EFFIC", CS%BBL_effic, &
                 "The efficiency with which the energy extracted by \n"//&
                 "bottom drag drives BBL diffusion.  This is only \n"//&
                 "used if BOTTOMDRAGLAW is true.", units="nondim", default=0.0)
    call log_param(param_file, mod, "BBL_MIXING_MAX_DECAY", decay_length, &
                 ...
                 default=200.0)
    call log_param(param_file, mod, "BBL_MIXING_AS_MAX", CS%BBL_mixing_as_max, &
  ...
  call log_param(param_file, mod, "BRYAN_LEWIS_DIFFUSIVITY", &
                 ...
                 default=.false.)
  if (CS%Bryan_Lewis_diffusivity) then
    call log_param(param_file, mod, "KD_BRYAN_LEWIS_DEEP", &
  ...
  if (CS%Henyey_IGW_background) &
    call log_param(param_file, mod, "HENYEY_N0_2OMEGA", CS%N0_2Omega, &
  ...
  call log_param(param_file, mod, "N2_FLOOR_IOMEGA2", CS%N2_FLOOR_IOMEGA2, &
                  "The floor applied to N2(k) scaled by Omega^2:\n"//&
                  "\tIf =0., N2(k) is simply positive definite.\n"//&
                  "\tIf =1., N2(k) > Omega^2 everywhere.", units="nondim", default=1.0)
  if (.not.CS%Henyey_IGW_background) &
    call log_param(param_file, mod, "KD_TANH_LAT_FN", &
                                  CS%Kd_tanh_lat_fn, &
  ...
  if (CS%Kd_tanh_lat_fn) &
    call log_param(param_file, mod, "KD_TANH_LAT_SCALE", &
  ...
  call log_param(param_file, mod, "USE_JACKSON_PARAM", CS%Add_input_Kd, &
  ...
  call log_param(param_file, mod, "KD", CS%Kd, &
                 ...
                 "may be used.", units="m2 s-1")
  call log_param(param_file, mod, "KD_MIN", CS%Kd_min, &
                 "The minimum diapycnal diffusivity.", &
                 units="m2 s-1", default=0.01*CS%Kd)
  call log_param(param_file, mod, "KD_MAX", CS%Kd_max, &
  ...
  call log_param(param_file, mod, "KD_ADD", CS%Kd_add, &
                 "A uniform diapycnal diffusivity that is added \n"//&
                 "everywhere without any filtering or scaling.", &
                 units="m2 s-1", default=0.0)
  call log_param(param_file, mod, "OMEGA", CS%omega, &
                 "The rotation rate of the earth.", units="s-1", &
                 default=7.2921e-5)
  call log_param(param_file, mod, "DEBUG", CS%debug, &
                 "If true, write out verbose debugging data.", default=.false.)

  call log_param(param_file, mod, "INT_TIDE_DISSIPATION", CS%Int_tide_dissipation, &
  ...
  call log_param(param_file, mod, "LEE_WAVE_DISSIPATION", CS%Lee_wave_dissipation, &
                 "If true, use an lee wave driven dissipation scheme to \n"//&
                 "drive diapycnal mixing, along the lines of Nikurashin \n"//&
                 "(2010)", default=.false.)

  call log_param(param_file, mod, "DISSIPATION_MIN", CS%dissip_min, &
                 "The minimum dissipation by which to determine a lower \n"//&
                 "bound of Kd (a floor).", default=0., units="W m-3")
  call log_param(param_file, mod, "DISSIPATION_N0", CS%dissip_N0, &
                 ...
                 units="W m-3", default=0.)
  call log_param(param_file, mod, "DISSIPATION_N1", CS%dissip_N1, &
                 ...
                 units="J m-3", default=0.)
  call log_param(param_file, mod, "DISSIPATION_KD_MIN", CS%dissip_Kd_min, &
                 "The minimum vertical diffusivity applied as a floor.\n"//&
                 "Units: m^2 s-1", default=0.)

  if (CS%Int_tide_dissipation) then
    call read_param(param_file,"I_ZETA",CS%I_Zeta,.true.)
    if (CS%I_Zeta /= 0.0) then
      CS%Int_tide_decay_scale = 1.0 / CS%I_Zeta
    else ; CS%Int_tide_decay_scale = 0.0 ; endif  ! Adcroft's rule reciprocal.
    call read_param(param_file, "INT_TIDE_DECAY_SCALE", CS%Int_tide_decay_scale)
    call read_param(param_file,"MU_ITIDES",CS%Mu_itides,.true.)
    call read_param(param_file,"GAMMA_ITIDES",CS%Gamma_itides,.true.)
    CS%min_zbot_itides=0.0
    call read_param(param_file,"MIN_ZBOT_ITIDES",CS%min_zbot_itides)
    ...
    CS%utide = 0.0 ; call read_param(param_file,"UTIDE",CS%utide)
    call safe_alloc_ptr(CS%tideamp,is,ie,js,je) ; CS%tideamp(:,:) = CS%utide

    call read_param(param_file,"H2_FILE",h2_file,.true.)
    filename = trim(CS%inputdir) // trim(h2_file)
    call read_data(filename, 'h2', CS%h2, domain=G%domain%mpp_domain, &
                   timelevel=1)

    CS%kappa_itides = 8.e-4*atan(1.0) ! Default is 2pi/10km (St.Laurent, 2002)
    call read_param(param_file,"KAPPA_ITIDES",CS%kappa_itides)
    CS%kappa_h2_factor = 1.0
    call read_param(param_file,"KAPPA_H2_FACTOR",CS%kappa_h2_factor)
    CS%TKE_itide_max = 1.e3
    call read_param(param_file,"TKE_ITIDE_MAX",CS%TKE_itide_max)

    read_tideamp = .false.
    call read_param(param_file,"READ_TIDEAMP",read_tideamp)
    if (read_tideamp) then
      tideamp_file = 'tideamp.nc'
      call read_param(param_file,"TIDEAMP_FILE",tideamp_file)
    ...
    do j=js, je; do i=is, ie
      if (G%D(i,j) .lt. CS%min_zbot_itides) CS%mask_itidal(i,j) = 0.0
    ...
    call log_param(param_file, mod, "I_ZETA", CS%I_Zeta, &
                 "The inverse decay scale away from the bottom for tidal \n"//&
                 "TKE when INT_TIDE_DISSIPATION is used. \n"//&
                 "###THIS NAME IS NOT DESCRIPTIVE AND SHOULD BE CHANGED!", &
                 units="m-1")
    call log_param(param_file, mod, "INT_TIDE_DECAY_SCALE", CS%Int_tide_decay_scale, &
                 "The decay scale away from the bottom for tidal TKE with \n"//&
                 "the new coding when INT_TIDE_DISSIPATION is used.", &
                 units="m")
    call log_param(param_file, mod, "MU_ITIDES", CS%Mu_itides, &
                 "A dimensionless turbulent mixing efficiency used with \n"//&
                 "INT_TIDE_DISSIPATION, often 0.2.", units="nondim")
    call log_param(param_file, mod, "GAMMA_ITIDES", CS%Gamma_itides, &
                 "The fraction of the internal tidal energy that is \n"//&
                 "dissipated locally with INT_TIDE_DISSIPATION.  \n"//&
                 "THIS NAME COULD BE BETTER.", &
                 units="nondim")
    call log_param(param_file, mod, "MIN_ZBOT_ITIDES", CS%min_zbot_itides, &
                 "Turn off internal tidal dissipation when the total \n"//&
                 "ocean depth is less than this value.", units="m", default=0.0)
    call log_param(param_file, mod, "KAPPA_ITIDES", CS%kappa_itides, &
    ...
    call log_param(param_file, mod, "UTIDE", CS%utide, &
                 "The constant tidal amplitude used with INT_TIDE_DISSIPATION.", &
                 units="m s-1", default=0.0)
    call log_param(param_file, mod, "KAPPA_H2_FACTOR", CS%kappa_h2_factor, &
                 "A scaling factor for the roughness amplitude with n"//&
                 "INT_TIDE_DISSIPATION.",  units="nondim", default=1.0)
    call log_param(param_file, mod, "TKE_ITIDE_MAX", CS%TKE_itide_max, &
                 "The maximum internal tide energy source availble to mix \n"//&
                 "above the bottom boundary layer with INT_TIDE_DISSIPATION.", units="W m-2", &
                 default=1.0e3)
    call log_param(param_file, mod, "READ_TIDEAMP", read_tideamp, &
                 "If true, read a file (given by TIDEAMP_FILE) containing \n"//&
                 "the tidal amplitude with INT_TIDE_DISSIPATION.", default=.false.)
    if (read_tideamp) call log_param(param_file, mod, "TIDEAMP_FILE", tideamp_file, &
                 "The path to the file containing the spatially varying \n"//&
                 "tidal amplitudes with INT_TIDE_DISSIPATION.", default="tideamp.nc")
    call log_param(param_file, mod, "H2_FILE", h2_file, &
                 "The path to the file containing the sub-grid-scale \n"//&
                 "topographic roughness amplitude with INT_TIDE_DISSIPATION.")
  endif

  if (CS%Lee_wave_dissipation) then
    call log_param(param_file, mod, "NIKURASHIN_TKE_INPUT_FILE",Niku_TKE_input_file, &
                 "The path to the file containing the TKE input \n"//&
                 "from Lee wave driven mixing. Used with LEE_WAVE_DISSIPATION.")
    call log_param(param_file, mod, "NIKURASHIN_SCALE",Niku_scale, &
                 "A non-dimensioanl factor by which to scale the Lee-wave driven \n"//&
                 "TKE input. Used with LEE_WAVE_DISSIPATION.", default=1.)
  ...
  if (associated(diag_to_Z_CSp)) then
    ...
    vd = vardesc("Kd_itidal_z","Internal Tide Driven Diffusiviity, interpolated to z",&
    ...
    vd = vardesc("Kd_Nikurashin_z","Lee Wave Driven Diffusivity, interpolated to z",&
                 'h','z','s',"meter2 second-1", 'd')
    if (CS%Lee_wave_dissipation) &
      CS%id_Kd_Niku_z = register_Zint_diag(vd, CS%diag_to_Z_CSp, Time)
  endif

  CS%double_diffusion = .false.
  call read_param(param_file,"DOUBLE_DIFFUSION",CS%double_diffusion)
  if (CS%double_diffusion) then
    CS%Max_Rrho_salt_fingers = 2.55  ! maximum density ratio for salt fingering (CCSM4.0 default; MOM4p1 is 1.9)
    call read_param(param_file,"MAX_RRHO_SALT_FINGERS",CS%Max_Rrho_salt_fingers)
    CS%Max_salt_diff_salt_fingers = 1.e-4  ! maximum diffusivity for salt fingering (CCSM4.0 and MOM4p1 defaults)
    call read_param(param_file,"MAX_SALT_DIFF_SALT_FINGERS",CS%Max_salt_diff_salt_fingers)
    CS%Kv_molecular = 1.5e-6  ! molecular viscosity (CCSM4.0 and MOM4p1 defaults)
    call read_param(param_file,"KV_MOLECULAR",CS%Kv_molecular)

    call log_param(param_file, mod, "DOUBLE_DIFFUSION", CS%double_diffusion, &
                 "If true, increase diffusivitives for temperature or salt \n"//&
                 "based on double-diffusive paramaterization from MOM4/KPP.", &
                 default=.false.)
    call log_param(param_file, mod, "MAX_RRHO_SALT_FINGERS", CS%Max_Rrho_salt_fingers, &
                 "Maximum density ratio for salt fingering regime. ", default=2.55)
    call log_param(param_file, mod, "MAX_SALT_DIFF_SALT_FINGERS", CS%Max_salt_diff_salt_fingers, &
                 "Maximum salt diffusivity for salt fingering regime. ",&
                 default=1.e-4)

    call log_param(param_file, mod, "KV_MOLECULAR", CS%Kv_molecular, &
                 "Molecular viscosity for calculation of fluxes under \n"//&
                 "double-diffusive convection. ", default=1.5e-6)
...
!    id_clock_Itide = cpu_clock_id('Ocean Set Diff ITides',grain=CLOCK_ROUTINE)
...
end module GOLD_set_diffusivity
