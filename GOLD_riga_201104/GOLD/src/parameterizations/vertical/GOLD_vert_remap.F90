...
!*  temperature and salinity in GOLD. Other tracers and the horizontal *
...
!                 vert_remap_init.
  ...
  if (.not. associated(CS)) call GOLD_error(FATAL, "GOLD_vert_remap: "//&
...
!                 vert_remap_init.
  ...
  real, dimension(SZI_(G),SZK_(G)) :: &
    ...
    Rcv, &      !   A 2-d version of tv%Rml, in kg m-3.
...
!  call cpu_clock_end(id_clock_EOS)
    ...
    do k=1,nz ; do i=is,ie
      ...
      if (do_T) then
        ...
        Rcv(i,k) = G%Rlay(k) ; if (k <= nkmb) Rcv(i,k) = tv%Rml(i,j,k)
      endif
    enddo ; enddo
        ...
        do i=is,ie ; if (more_ent_i(i)) then
          if (h_2d(i,k) - G%Angstrom > h_neglect) then
            ...
            if (do_T) then
              ...
              Rcv(i,nkmb) = (h_prev*Rcv(i,nkmb) + h_add*Rcv(i,k)) / h_2d(i,nkmb)
            endif
            ...
            if (h_add < h_2d(i,k1)) then
              ...
              if (h_add > 0.0) then
                ...
                if (do_T) then
                  ...
                  Rcv(i,k2) = (h_prev*Rcv(i,k2) + h_add*Rcv(i,k1)) / h_2d(i,k2)
                endif
                ...
                if (h_add < 0.0) &
                  call GOLD_error(FATAL, "h_add is negative.  Some logic is wrong.")
    ...
    do i=is,ie ; if (do_i(i)) then
        ...
        if (k2 > k1) then
          do k3=k1+1,k2
            ...
            if (do_T) then
              ...
              int_Rflux(k3) = int_Rflux(k3) + h_add*Rcv(i,k1)
            endif
          enddo
        elseif (k1 > k2) then
          do k3=k2,k1-1
            ...
            if (do_T) then
              ...
              int_Rflux(k3+1) = int_Rflux(k3+1) - h_add*Rcv(i,k1)
            endif
          enddo
        endif
      ...
      enddo
      if (k2 < nkmb) &
        call GOLD_error(FATAL, "Regularize_surface: Did not assign fluid to layer nkmb.")
      ...
      if (do_T) then
        ...
        Rcv(i,1) = (Rcv(i,1)*h_prev_1d(1) - int_Rflux(2)) / h_2d(i,1)
        do k=2,nkmb-1
          Rcv(i,k) = (Rcv(i,k)*h_prev_1d(k) + (int_Rflux(k) - int_Rflux(k+1))) / h_2d(i,k)
        enddo
        Rcv(i,nkmb) = (Rcv(i,nkmb)*h_prev_1d(nkmb) + int_Rflux(nkmb) ) / h_2d(i,nkmb)
      endif
    ...
    do k=1,nz ; do i=is,ie ; if (do_i(i)) then
      ...
      if (do_T) then
        ...
        if (k <= nkmb) tv%Rml(i,j,k) = Rcv(i,k)
      endif
    ...
    if (debug) then
      ...
      do k=1,nz ; do i=is,ie ; if (do_i(i)) then
        ...
        if (h(i,j,k) < 0.0) &
          call GOLD_error(FATAL,"regularize_surface: Negative thicknesses.")
        ...
        if (abs(h(i,j,k) - h_predicted) > MAX(1e-9*abs(h_predicted),G%Angstrom)) &
          call GOLD_error(FATAL, "regularize_surface: d_ea mismatch.")
      endif ; enddo ; enddo
      do i=is,ie ; if (do_i(i)) then
        fatal_error = .false.
        if (abs(h_tot1(i) - h_tot2(i)) > 1e-12*h_tot1(i)) then
          ...
          call GOLD_error(WARNING, "regularize_surface: Mass non-conservation."//&
                          trim(mesg), .true.)
          fatal_error = .true.
        endif
        if (do_T) then
          if (abs(Th_tot1(i) - Th_tot2(i)) > 1e-12*(Th_tot1(i)+10.0*h_tot1(i))) then
            ...
            call GOLD_error(WARNING, "regularize_surface: Heat non-conservation."//&
                            trim(mesg), .true.)
            fatal_error = .true.
          endif
          if (abs(Sh_tot1(i) - Sh_tot2(i)) > 1e-12*(Sh_tot1(i)+10.0*h_tot1(i))) then
            ...
            call GOLD_error(WARNING, "regularize_surface: Salinity non-conservation."//&
                            trim(mesg), .true.)
            fatal_error = .true.
          endif
        endif
        if (fatal_error) then
          ...
          call GOLD_error(FATAL, "regularize_surface: Terminating with fatal error.  "//&
                          trim(mesg))
        endif
      endif ; enddo
    endif
  ...
  if (do_T) then
    call cpu_clock_begin(id_clock_EOS)
    do k=1,nkmb ; do j=js,je ; if (do_j(j)) then
...
!  (in,opt)  h - The layer thicknesse; if not present take vertical differences of e.
  ...
  if (present(def_rat_u_2lay)) then ; do j=js,je ; do I=is-1,ie
    def_rat_u(I,j) = G%umask(I,j) * h_def_u(I,j) / &
                     (max(CS%Hmix, h_norm_u(I,j)) + h_neglect)
  ...
  do k=1,nkmb ; do J=js-1,je ; do i=is,ie
    if (present(h)) then
      ...
      h1 = e(i,j,k)-e(i,j,k+1) ; h2 = e(i+1,j,k)-e(i,j+1,k+1)
    endif
    ! Thickness deficits can not arise simply because a layer's bottom is bounded
    ! by the bathymetry.
  ...
  if (present(def_rat_v_2lay)) then ; do J=js-1,je ; do i=is,ie
    def_rat_v(i,J) = G%vmask(i,J) * h_def_v(i,J) / &
                      (max(CS%Hmix, h_norm_v(i,J)) + h_neglect)
...
!                  for this module
  character(len=128) :: version = '$Id: GOLD_vert_remap.F90,v 1.1.2.2 2011/10/07 22:48:13 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_vert_remap"  ! This module's name.
  logical :: use_temperature
  integer :: isd, ied, jsd, jed
  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed
 
  if (associated(CS)) then
    call GOLD_error(WARNING, "vert_remap_init called with an associated"// &
  ...
  CS%bulkmixedlayer = .false. ; CS%regularize_surface_layers = .false.
  CS%reg_sfc_detrain = .true.
  call read_param(param_file,"BULKMIXEDLAYER",CS%bulkmixedlayer)
  call read_param(param_file,"REGULARIZE_SURFACE_LAYERS",CS%regularize_surface_layers)
  call read_param(param_file,"REGULARIZE_SURFACE_DETRAIN",CS%reg_sfc_detrain)
  ...
  CS%h_def_tol1 = 0.5
  call read_param(param_file,"REG_SFC_DEFICIT_TOLERANCE", CS%h_def_tol1)
  ...
  call log_param(param_file, mod, "REGULARIZE_SURFACE_LAYERS", CS%regularize_surface_layers, &
                 ...
                 default=.false.)
  if (CS%regularize_surface_layers) then
    call log_param(param_file, mod, "REGULARIZE_SURFACE_DETRAIN", CS%reg_sfc_detrain, &
  ...
  call log_param(param_file, mod, "NKBL", CS%nkbl, &
                 ...
                 "BULKMIXEDLAYER is true.", units="nondim", default=1)
  call log_param(param_file, mod, "REG_SFC_DEFICIT_TOLERANCE", CS%h_def_tol1, &
