...
type, public :: entrain_diffusive_CS ; private
  ...
  real    :: Tolerance_Ent   ! The tolerance with which to solve for entrainment
                             ! values, in m.
  logical :: Riga_foibles    ! If true, use code that can sometimes give answers
                             ! that are non-reproducing across PE-count after
                             ! optimization.
...
! In the comments below, H is used as shorthand for the units of h, m or kg m-2.
  ...
  if (.not. associated(CS)) call GOLD_error(FATAL, &
         "GOLD_entrain_diffusive: Module must be initialized before it is used.")

  if (.not.(present(Kd_Lay) .or. present(Kd_int))) call GOLD_error(FATAL, &
      "GOLD_entrain_diffusive: Either Kd_Lay or Kd_int must be present in call.")
...
! This is the end of the section that might be iterated.
    ...
    if (CS%id_diff_work > 0) then
      ...
      if (associated(tv%eqn_of_state)) then
        ...
        do k=2,kmb ; do i=is,ie
          diff_work(i,j,k) = g_2dt * (tv%Rml(i,j,k)-tv%Rml(i,j,k-1)) * &
               (ea(i,j,k) * (h(i,j,k) + ea(i,j,k)) + &
                eb(i,j,k-1)*(h(i,j,k-1) + eb(i,j,k-1)))
        enddo ; enddo
        do k=K2,nz ; do i=is,ie
          if (k<kb(i)) then ; diff_work(i,j,k) = 0.0
          elseif (k==kb(i)) then
            diff_work(i,j,k) = g_2dt * (G%Rlay(k)-tv%Rml(i,j,kmb)) * &
                 (ea(i,j,k) * (h(i,j,k) + ea(i,j,k)) + &
                  eb(i,j,kmb)*(h(i,j,kmb) + eb(i,j,kmb)))
          else
...
!  max_ent = 1.0e14*G%Angstrom ! This is set to avoid roundoff problems.
  ...
  do i=is,ie
    ...
    Sref(i,1) = tv%Rml(i,j,1) - 1000.0
    h_bl(i,1) = h(i,j,1) + h_neglect
...
! also be returned.
  ...
  if (present(ddSlay_dE) .and. .not.present(dSlay)) call GOLD_error(FATAL, &
...
! constrained to be within the provided bounds.
  ...
  if (.not.CS%bulkmixedlayer) then
    call GOLD_error(FATAL, "determine_Ea_kb should not be called "//&
...
! unlimited maximum and comparing it to the limited value at max_ent_in.
  ...
  if (present(F_lim_maxent)) then
    ...
    do i=is,ie
      ...
      if (CS%Riga_foibles) may_use_best(i) = .true.
    enddo
    if (doany) then
...
!                 for this module
  ...
  character(len=128) :: version = '$Id: GOLD_entrain_diffusive.F90,v 1.1.2.23 2011/02/17 23:37:07 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod  = "GOLD_entrain_diffusive" ! This module's name.

  if (associated(CS)) then
    call GOLD_error(WARNING, "entrain_diffusive_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
  ...
  if (use_temperature) call read_param(param_file,"CORRECT_DENSITY",CS%correct_density)

  CS%max_ent_it = 5 ; call read_param(param_file,"MAX_ENT_IT",CS%max_ent_it)

  call read_param(param_file,"KD",Kd,.true.)
  call read_param(param_file,"DT",dt,.true.)
  CS%Tolerance_Ent = MAX(100.0*G%Angstrom,1.0e-4*sqrt(dt*Kd)) !
  call read_param(param_file,"TOLERANCE_ENT",CS%Tolerance_Ent)
  CS%Riga_foibles = .false.
  call read_param(param_file,"RIGA_ENTRAINMENT_FOIBLES",CS%Riga_foibles)
  ...
  call log_param(param_file, mod, "KD", Kd, &
                 ...
                 "TOLERANCE_ENT.", units="m2 s-1")
  call log_param(param_file, mod, "DT", dt, &
                 "The (baroclinic) dynamics time step.", units = "s")
  call log_param(param_file, mod, "TOLERANCE_ENT", CS%Tolerance_Ent, &
                 "The tolerance with which to solve for entrainment \n"//&
                 "values. This is only used when USE_H2000_SHEAR_MIXING \n"//&
                 "is false.", units="m", &
                 default=MAX(100.0*G%Angstrom,1.0e-4*sqrt(dt*Kd)))
  call log_param(param_file, mod, "RIGA_ENTRAINMENT_FOIBLES", CS%Riga_foibles, &
                 "If true, use code that can sometimes give answers that \n"//&
                 "that may not reproduce across PE-count after optimization.", &
                 default=.false.)
...
end module GOLD_entrain_diffusive
