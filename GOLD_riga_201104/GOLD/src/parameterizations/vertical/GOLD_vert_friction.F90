...
!*                                                                     *
!*    vertvisc_remean generates a new velocity built from the vertical *
!*  structure of one velocity but with the vertical mean velocity      *
!*  changed to give the specified vertically integrated transport,     *
!*  using the viscous remnant matrix to enforce the no-slip bottom     *
!*  boundary condition.                                                *
!*                                                                     *
...
public vertvisc, vertvisc_remnant, vertvisc_coef, vertvisc_remean
...
subroutine vertvisc(u, v, h, fluxes, visc, dt, OBC, G, CS)
...
!     velocity in the mixed layer if CS%nkml > 1 in a bulk mixed layer.
!
        ...
        if (CS%harmonic_visc) then
          ...
          do k=1,nz
            do i=is,ie ; if (do_i_shelf(i)) then
              ...
              hvel_shelf(i,k) = hvel(i,k)
              if (v(i,j,k) * (h(i,j+1,k)-h(i,j,k)) > 0) then
                z2 = max(zh(i), Ztop_min(i) - min(zcol(i),zcol2(i))) * I_HTbl(i)
                topfn = 1.0 / (1.0 + 0.09*z2**6)
                hvel_shelf(i,k) = min(hvel(i,k), (1.0-topfn)*h_arith + topfn*h_harm(i,k))
              endif
            endif ; enddo
...
!  surface mixed layer contributions to the vertical viscosity.
    ...
    if (CS%bulkmixedlayer .or. do_shelf) then ; do k=2,nz ; do i=is,ie
    ...
    elseif (CS%dynamic_viscous_ML .or. CS%bulkmixedlayer) then
      max_nk = 0
      do i=is,ie ; if (do_i(i)) then
        if (CS%bulkmixedlayer) nk_visc(i) = real(CS%nkml+1)
        if (work_on_u) then
...
! excessive velocities may be sent to a diagnostic reporting subroutine.
  ...
  logical :: trunc_any, dowrite(SZ1_(u))
  integer :: i, j, k, is, ie, js, je, Isq, Ieq, Jsq, Jeq, nz
...
subroutine vertvisc_remean(u, v, h, uhbt, vhbt, visc_rem_u, visc_rem_v,&
                           uav, vav, OBC, G, maxvel, h_at_u, h_at_v)
! This subroutine solves for the velocites uav and vav with the vertical
! structure of u and v but the vertical mean of ubtav and vbtav, such that
!   uav = u - u_cor * visc_rem_u
!   sum(uav * h_u) = ubtav * sum(h_u)
! This assumes that vertical viscosity has already been applied to u and v.
  ...
  real, dimension(NXMEMQ_,NYMEM_),     intent(in)    :: uhbt
  real, dimension(NXMEM_,NYMEMQ_),     intent(in)    :: vhbt
  real, dimension(NXMEMQ_,NYMEM_,NZ_), intent(in)    :: visc_rem_u
  real, dimension(NXMEM_,NYMEMQ_,NZ_), intent(in)    :: visc_rem_v
  real, dimension(NXMEMQ_,NYMEM_,NZ_), intent(out)   :: uav
  real, dimension(NXMEM_,NYMEMQ_,NZ_), intent(out)   :: vav
  type(ocean_OBC_type),                pointer       :: OBC
  type(ocean_grid_type),               intent(in)    :: G
  real,                                intent(in), optional :: maxvel
  real, dimension(NXMEMQ_,NYMEM_,NZ_), intent(in), optional :: h_at_u
  real, dimension(NXMEM_,NYMEMQ_,NZ_), intent(in), optional :: h_at_v
! Arguments: u - Zonal velocity, in m s-1.  Intent in/out.
!  (in)      v - Meridional velocity, in m s-1.
!  (in)      h - Layer thickness, in m.
...
!                         meridional (_v) directions.  Nondimensional between
!                         0 (at the bottom) and 1 (far above the bottom).
!  (in)      uav - Zonal velocity based on the vertical structure of u
!                  but its vertical mean giving a transport consistent with
!                  uhbt, in m s-1.
!  (in)      vav - Meridional velocity based on the vertical structure of v
!                  but its vertical mean giving a transport consistent with
!                  vhbt, in m s-1.
!  (in)      OBC - This open boundary condition type specifies whether, where,
!                  and what open boundary conditions are used.
!  (in)      G - The ocean's grid structure.
!  (in, opt) h_at_u - The thickness of each layer at u points, in m or nondim.
!  (in, opt) h_at_v - The thickness of each layer at v points, in m or nondim.

  real, dimension(SZ1_(u)) :: &
    uh_tot, &  ! The summed estimate of the zonal thickness fluxes, in m2 s-1.
    u_cor, &   ! The viscosity-weighted zonal-velocity correction in m s-1.
    visc_h_u   ! The summed estimates of the layer thicknesses at u points times
               ! the viscous remnant, in m.
  real, dimension(SZ1_(v)) :: &
    vh_tot, &  ! The summed estimate of the meridional thickness fluxes, in m2 s-1.
    v_cor, &   ! The viscosity-weighted meridional-velocity correction in m s-1.
    visc_h_v   ! The summed estimates of the layer thicknesses at v points times
               ! the viscous remnant, in m.
  real :: h_u, h_v ! Estimated thicknesses at u- and v-points, in m.
  logical :: write_trunc_warning
  integer :: i, j, k, is, ie, js, je, Isq, Ieq, Jsq, Jeq, nz
  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec ; nz = G%ke
  Isq = G%Iscq ; Ieq = G%Iecq ; Jsq = G%Jscq ; Jeq = G%Jecq
  write_trunc_warning = .false.

  ! This subroutine solves for the remeaned velocites uav and vav such that
  !   uav = u - u_cor * visc_rem_u
  !   sum(uav * h_u) = ubtav * sum(h_u)
  ! which imply that
  !   u_cor * sum(visc_rem_u * h_u) = sum(u * h_u) - ubtav * sum(h_u)
  ! This assumes that vertical viscosity has already been applied to u
  do j=js,je
    do I=Isq,Ieq ; uh_tot(I) = 0.0 ; u_cor(I) = 0.0 ; visc_h_u(I) = 0.0 ; enddo
!   if (present(h_at_u)) then
!     do k=1,nz ; do I=Isq,Ieq ; if (G%umask(I,j) > 0) then
!       h_u = G%dy_u(i,j) * h_at_u(I,j,k)
!       uh_tot(I) = uh_tot(I) + u(I,j,k) * h_u
!       visc_h_u(I) = visc_h_u(I) + h_u * visc_rem_u(I,j,k)
!     endif ; enddo ; enddo
!   else
      do k=1,nz ; do I=Isq,Ieq ; if (G%umask(I,j) > 0) then
        h_u = G%dy_u(i,j) * 0.5*(h(i,j,k) + h(i+1,j,k))
        uh_tot(I) = uh_tot(I) + u(I,j,k) * h_u
        visc_h_u(I) = visc_h_u(I) + h_u * visc_rem_u(I,j,k)
      endif ; enddo ; enddo
!   endif
    do i=Isq,Ieq ; if ((G%umask(I,j) > 0) .and. (visc_h_u(I) > 0)) then
      u_cor(I) = (uh_tot(I) - uhbt(I,j)) / visc_h_u(I)
      !   Here is an if-but statement.  It is required in case the vertical
      ! viscosity used in the barotropic solver is grossly inconsistent with
      ! the layer's viscosities, and the required barotropic transport is
      ! therefore unattainable.
      ! Limit visc_h_u to avoid generating absurdly large velocities.
      if (present(maxvel)) then ; if (abs(u_cor(I)) > 0.5*maxvel) then
        u_cor(I) = SIGN(0.5*maxvel,u_cor(I))
        write_trunc_warning = .true.
      endif ; endif
    endif ; enddo
    do k=1,nz ; do I=Isq,Ieq ; if (G%umask(I,j) > 0) then
      uav(I,j,k) = u(I,j,k) - u_cor(I) * visc_rem_u(I,j,k)
    endif ; enddo ; enddo
  enddo

  do J=Jsq,Jeq
    do i=is,ie ; vh_tot(i) = 0.0 ; u_cor(i) = 0.0 ; visc_h_v(i) = 0.0 ; enddo
!   if (present(h_at_v)) then
!     do k=1,nz ; do i=is,ie ; if (G%vmask(i,J) > 0) then
!       h_v = G%dx_v(i,J) * h_at_v(i,J,k)
!       vh_tot(i) = vh_tot(i) + v(i,J,k) * h_v
!       visc_h_v(i) = visc_h_v(i) + h_v * visc_rem_v(i,J,k)
!     endif ; enddo ; enddo
!   else
      do k=1,nz ; do i=is,ie ; if (G%vmask(i,J) > 0) then
        h_v = G%dx_v(i,J) * 0.5*(h(i,j,k) + h(i,j+1,k))
        vh_tot(i) = vh_tot(i) + v(i,J,k) * h_v
        visc_h_v(i) = visc_h_v(i) + h_v * visc_rem_v(i,J,k)
      endif ; enddo ; enddo
!   endif
    do i=is,ie ; if ((G%vmask(i,J) > 0) .and. (visc_h_v(i) > 0)) then
      v_cor(i) = (vh_tot(i) - vhbt(i,J)) / visc_h_v(i)
      ! Limit visc_h_v to avoid generating absurdly large velocities.
      if (present(maxvel)) then ; if (abs(v_cor(i)) > 0.5*maxvel) then
        v_cor(i) = SIGN(0.5*maxvel,v_cor(i))
        write_trunc_warning = .true.
      endif ; endif
    endif ; enddo
    do k=1,nz ; do i=is,ie ; if (G%vmask(i,J) > 0) then
      vav(i,J,k) = v(i,J,k) - v_cor(i) * visc_rem_v(i,J,k)
    endif ; enddo ; enddo
  enddo
  ...
  if (associated(OBC)) then ; if (OBC%apply_OBC_u) then
    do k=1,nz ; do j=js,je ; do I=Isq,Ieq
      if (OBC%OBC_mask_u(I,j) .and. (OBC%OBC_kind_u(I,j) == OBC_SIMPLE)) &
        uav(I,j,k) = OBC%u(I,j,k)
    enddo ; enddo ; enddo
  endif ; endif
  if (associated(OBC)) then ; if (OBC%apply_OBC_v) then
    do k=1,nz ; do J=Jsq,Jeq ; do i=is,ie
      if (OBC%OBC_mask_v(i,J) .and. (OBC%OBC_kind_v(i,J) == OBC_SIMPLE)) &
        vav(i,J,k) = OBC%v(i,J,k)
    enddo ; enddo ; enddo
  endif ; endif

  if (write_trunc_warning) call GOLD_error(WARNING, "GOLD_vert_friction(remean_trans): "// &
      "Velocity corrections were truncated at MAXVEL/2.")

end subroutine vertvisc_remean
...
!                 for this module
  ...
  character(len=128) :: version = '$Id: GOLD_vert_friction.F90,v 13.0.2.9.2.31 2011/04/13 19:24:22 Robert.Hallberg Exp $'
  ...
  CS%direct_stress = .false. ; CS%bulkmixedlayer = .false.
  CS%dynamic_viscous_ML = .false.
  CS%bottomdraglaw = .false. ; CS%Channel_drag = .false.
  CS%u_trunc_file = " " ; CS%v_trunc_file = " "
  call read_param(param_file, "BOTTOMDRAGLAW", CS%bottomdraglaw)
  call read_param(param_file, "CHANNEL_DRAG", CS%Channel_drag)
  call read_param(param_file, "DIRECT_STRESS", CS%direct_stress)
  call read_param(param_file, "DYNAMIC_VISCOUS_ML", CS%dynamic_viscous_ML)
  call read_param(param_file, "BULKMIXEDLAYER", CS%bulkmixedlayer)
  CS%nkml = 1 ; call read_param(param_file, "NKML", CS%nkml)
  call read_param(param_file, "U_TRUNC_FILE", CS%u_trunc_file)
  call read_param(param_file, "V_TRUNC_FILE", CS%v_trunc_file)
  CS%harmonic_visc = .true.
  call read_param(param_file, "HARMONIC_VISC", CS%harmonic_visc)
  CS%debug = .false. ; call read_param(param_file,"DEBUG",CS%debug)

  CS%maxvel = 1.0e6 ; CS%Hmix = -1.0e6 ; CS%Hmix_stress = -1.0e6
  if (.not.CS%bulkmixedlayer) &
    call read_param(param_file, "HMIX", CS%Hmix, .true.)
  if (CS%direct_stress) then
    call read_param(param_file, "HMIX", CS%Hmix_stress)
    call read_param(param_file, "HMIX_STRESS", CS%Hmix_stress)
  ...
  CS%Kvml = CS%Kv ; CS%Kvbbl = CS%Kv
  if (.not.CS%bulkmixedlayer) call read_param(param_file,"KVML",CS%Kvml,.true.)
  if (.not.CS%bottomdraglaw) call read_param(param_file,"KVBBL",CS%Kvbbl,.true.)

  call read_param(param_file, "HBBL", CS%Hbbl, .true.)
  call read_param(param_file, "MAXVEL", CS%maxvel)

  CS%CFL_based_trunc = .false. ; CS%CFL_trunc=1.0
  call read_param(param_file, "CFL_BASED_TRUNCATIONS", CS%CFL_based_trunc)
  call read_param(param_file, "CFL_TRUNCATE", CS%CFL_trunc)
  CS%CFL_report = CS%CFL_trunc
  call read_param(param_file, "CFL_REPORT", CS%CFL_report)
  ...
  call log_param(param_file, mod, "BOTTOMDRAGLAW", CS%bottomdraglaw, &
                 ...
                 "LINEAR_DRAG.")
  call log_param(param_file, mod, "CHANNEL_DRAG", CS%Channel_drag, &
  ...
  call log_param(param_file, mod, "DIRECT_STRESS", CS%direct_stress, &
  ...
  call log_param(param_file, mod, "DYNAMIC_VISCOUS_ML", CS%dynamic_viscous_ML, &
  ...
  call log_param(param_file, mod, "U_TRUNC_FILE", CS%u_trunc_file, &
                 "The absolute path to a file into which the accelerations \n"//&
                 "leading to zonal velocity truncations are written.  \n"//&
                 "Undefine this for efficiency if this diagnostic is not \n"//&
                 "needed.", default=" ")
  call log_param(param_file, mod, "V_TRUNC_FILE", CS%v_trunc_file, &
                 "The absolute path to a file into which the accelerations \n"//&
                 "leading to meridional velocity truncations are written. \n"//&
                 "Undefine this for efficiency if this diagnostic is not \n"//&
                 "needed.", default=" ")
  call log_param(param_file, mod, "HARMONIC_VISC", CS%harmonic_visc, &
                 "If true, use the harmonic mean thicknesses for \n"//&
                 "calculating the vertical viscosity. \n"//&
                 "###THIS DEFAULT SHOULD BE CHANGED.", default=.true.)
  if (CS%bulkmixedlayer) call log_param(param_file, mod, "NKML", CS%nkml, &
