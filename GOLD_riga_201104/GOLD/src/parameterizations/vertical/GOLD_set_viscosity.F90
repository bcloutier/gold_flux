...
use GOLD_variables, only : forcing, thermo_var_ptrs, directories
use GOLD_variables, only : vertvisc_type, ocean_OBC_type
...
type, public :: set_visc_CS ; private
  ...
  integer :: nkml           ! The minimum number of layers in the mixed layer.
...
!                 vertvisc_init.
  ...
  real :: h_at_vel(SZ1_(u),SZK_(G))! Layer thickness at a velocity point,
  ...
  real :: Rml(SZI_(G),SZJ_(G),max(tv%nk_Rml,1)) ! A copy of Rml, in kg m-3.
  ...
  if (.not.associated(CS)) call GOLD_error(FATAL,"GOLD_vert_friction(BBL): "//&
         "Module must be initialized before it is used.")
  if (.not.CS%bottomdraglaw) return

  use_EOS = associated(tv%eqn_of_state) .and. CS%BBL_use_EOS

  U_bg_sq = CS%drag_bg_vel * CS%drag_bg_vel
  cdrag_sqrt=sqrt(CS%cdrag)
  K2 = 2
  if (CS%bulkmixedlayer) K2 = nkmb+1
...
!GOMP(                                    Vol_quit, C2pi_3, U_bg_sq, cdrag_sqrt, K2 ))
  do j=G%Jscq,G%Jecq ; do m=1,2
    ...
    if (use_EOS .or. .not.CS%linear_drag) then
...
! the quadratic bottom drag.
        ...
        do k=nz,1,-1
          ...
          if (use_EOS .and. (hweight >= 0.0)) then ; if (m==1) then
    ...
    if (use_EOS) then
      ...
      if (m==1) then ; do k=1,nz ; do i=is,ie
        press(I) = press(I) + G%H_to_Pa * 0.5 * (h(i,j,k) + h(i+1,j,k))
...
! 2000 (unpublished and lost manuscript).
      ...
      else  ! Use Rlay and/or tv%Rml as density variables.
        Rhtot = 0.0
        do k=nz,K2,-1
...
!                 vertvisc_init.
  ...
  real :: Rho0x400_G  ! 400*Rho0/G_Earth, in kg s2 m-4.  The 400 is a
                      ! constant proposed by Killworth and Edwards, 1999.
  logical :: use_EOS, do_any, do_any_shelf, do_i(SZ1_(u))
  ...
  if (.not.associated(CS)) call GOLD_error(FATAL,"GOLD_vert_friction(visc_ML): "//&
         "Module must be initialized before it is used.")
  ...
  if (associated(fluxes%frac_shelf_h)) then
    ...
    ! be allocated.
    if (.not.associated(fluxes%frac_shelf_u)) call GOLD_error(FATAL, &
      "set_viscous_ML: fluxes%frac_shelf_h is associated, but " // &
      "fluxes%frac_shelf_u is not.")
    if (.not.associated(fluxes%frac_shelf_v)) call GOLD_error(FATAL, &
      "set_viscous_ML: fluxes%frac_shelf_h is associated, but " // &
...
!GOMP(                                    dt_Rho0, h_neglect, h_tiny, g_H_Rho0))
    ...
    if (CS%dynamic_viscous_ML) then
      ...
      do I=Isq,Ieq
        ...
        if (G%umask(I,j) < 0.5) then
          do_i(I) = .false. ; visc%nkml_visc_u(I,j) = CS%nkml
        else
      ...
      if (do_any) then ; do k=1,nz
        if (k > CS%nkml) then
          do_any = .false.
          if (use_EOS .and. (k==CS%nkml+1)) then
          ...
          do I=Isq,Ieq ; if (do_i(I)) then
            ...
            if (hlay > h_tiny) then ! Only consider non-vanished layers.
              ...
              elseif (CS%bulkmixedlayer .and. k<tv%nk_Rml) then
                Rlay = (h(i,j,k)*tv%Rml(i,j,k) + h(i+1,j,k)*tv%Rml(i+1,j,k)) * I_2hlay
                gHprime = g_H_Rho0 * (Rlay*htot(I) - Rhtot(I))
              else
        ...
        do I=Isq,Ieq ; if (do_i(I)) then
          ...
          elseif (CS%bulkmixedlayer .and. k<tv%nk_Rml) then
            Rhtot(i) = Rhtot(i) + 0.5 * (h(i,j,k)*tv%Rml(i,j,k) + h(i+1,j,k)*tv%Rml(i+1,j,k))
          else
  ...
  !  by Killworth and Edwards, 1999, in equation (2.20).
        ...
        if (use_EOS) then
          ...
          Rhtot = 0.0
          do k=1,nz-1
            if (CS%bulkmixedlayer .and. (k<=nkmb)) then
              Rlay = 0.5*(tv%Rml(i,j,k) + tv%Rml(i+1,j,k))
            else ; Rlay = G%Rlay(k) ; endif
            if (CS%bulkmixedlayer .and. (k+1<=nkmb)) then
              Rlb = 0.5*(tv%Rml(i,j,k+1) + tv%Rml(i+1,j,k+1))
            else ; Rlb = G%Rlay(k+1) ; endif
          ...
          enddo
          if (CS%bulkmixedlayer .and. (nz<=nkmb)) then
            if (0.5*(tv%Rml(i,j,nz) + tv%Rml(i+1,j,nz))*htot(i) - Rhtot(i) < ustarsq) &
              htot(i) = htot(i) + h_at_vel(i,nz)
          else
...
!GOMP(                                    dt_Rho0, h_neglect, h_tiny, g_H_Rho0))
    ...
    if (CS%dynamic_viscous_ML) then
      ...
      do i=is,ie
        ...
        if (G%vmask(i,J) < 0.5) then
          do_i(i) = .false. ; visc%nkml_visc_v(i,J) = CS%nkml
        else
          do_i(i) = .true. ; do_any = .true.
          k_massive(i) = CS%nkml
          Thtot(i) = 0.0 ; Shtot(i) = 0.0 ; Rhtot(i) = 0.0
      ...
      if (do_any) then ; do k=1,nz
            ...
            if (hlay > h_tiny) then ! Only consider non-vanished layers.
              ...
              if (use_EOS) then
                ...
                gHprime = g_H_Rho0 * (dR_dT(i) * (T_lay*htot(i) - Thtot(i)) + &
                                      dR_dS(i) * (S_lay*htot(i) - Shtot(i)))
              elseif (CS%bulkmixedlayer .and. k<tv%nk_Rml) then
                Rlay = (h(i,j,k)*tv%Rml(i,j,k) + h(i,j+1,k)*tv%Rml(i,j+1,k)) / &
                       (h(i,j,k) + h(i,j+1,k))
                gHprime = g_H_Rho0 * (Rlay*htot(i) - Rhtot(i))
              else
                gHprime = g_H_Rho0 * (G%Rlay(k)*htot(i) - Rhtot(i))
        ...
        do i=is,ie ; if (do_i(i)) then
          ...
          elseif (CS%bulkmixedlayer .and. k<tv%nk_Rml) then
            Rhtot(i) = Rhtot(i) + 0.5 * (h(i,j,k)*tv%Rml(i,j,k) + h(i,j+1,k)*tv%Rml(i,j+1,k))
          else
  ...
  !  by Killworth and Edwards, 1999, in equation (2.20).
        ...
        if (use_EOS) then
          ...
          do k=1,nz-1
            if (CS%bulkmixedlayer .and. (k<=nkmb)) then
              Rlay = 0.5*(tv%Rml(i,j,k) + tv%Rml(i,j+1,k))
            else ; Rlay = G%Rlay(k) ; endif
            if (CS%bulkmixedlayer .and. (k+1<=nkmb)) then
              Rlb = 0.5*(tv%Rml(i,j,k+1) + tv%Rml(i,j+1,k+1))
            else ; Rlb = G%Rlay(k+1) ; endif
          ...
          enddo
          if (CS%bulkmixedlayer .and. (nz<=nkmb)) then
            if (0.5*(tv%Rml(i,j,nz) + tv%Rml(i,j+1,nz))*htot(i) - Rhtot(i) < ustarsq) &
              htot(i) = htot(i) + h_at_vel(i,nz)
          else
            if (G%Rlay(nz)*htot(i) - Rhtot(i) < ustarsq) htot(i) = htot(i) + h_at_vel(i,nz)
          endif
...
!                 for this module
  real    :: Csmag_chan_dflt, TKE_decay_dflt
  integer :: isd, ied, jsd, jed, Isdq, Iedq, Jsdq, Jedq, nz
  logical :: use_kappa_shear, adiabatic, double_diffusion
  character(len=128) :: version = '$Id: GOLD_set_viscosity.F90,v 1.1.2.2.2.36 2011/04/05 18:39:05 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_set_visc"  ! This module's name.

  if (associated(CS)) then
    call GOLD_error(WARNING, "set_visc_init called with an associated "// &
                            "control structure.")
    return
  endif
  allocate(CS)
  ...
  CS%diag => diag
  CS%bulkmixedlayer = .false. ; CS%bottomdraglaw = .false.
  CS%linear_drag = .false. ; CS%Channel_drag = .false. ; CS%RiNo_mix = .false.
  use_kappa_shear = .false. ; adiabatic = .false.
  call read_param(param_file,"BOTTOMDRAGLAW",CS%bottomdraglaw)
  call read_param(param_file,"CHANNEL_DRAG",CS%Channel_drag)
  call read_param(param_file,"LINEAR_DRAG",CS%linear_drag)
  call read_param(param_file,"ADIABATIC",adiabatic)
  if (.not.adiabatic) then
    call read_param(param_file,"BULKMIXEDLAYER",CS%bulkmixedlayer)
    call read_param(param_file,"USE_JACKSON_PARAM",use_kappa_shear)
    call read_param(param_file,"RINOMIX",CS%RiNo_mix)
  ...
  if (CS%dynamic_viscous_ML) then
    CS%bulk_Ri_ML = 0.0 ; CS%TKE_decay = 0.0 ; CS%use_omega = .false.
    call read_param(param_file, "BULK_RI_ML", CS%bulk_Ri_ML)
    call read_param(param_file, "BULK_RI_ML_VISC", CS%bulk_Ri_ML)
    call read_param(param_file, "TKE_DECAY", CS%TKE_decay)
    TKE_decay_dflt = CS%TKE_decay
    call read_param(param_file, "TKE_DECAY_VISC", CS%TKE_decay)
  ...
  CS%BBL_thick_min = 0.0 ; CS%BBL_use_EOS = .false.

  call read_param(param_file,"HBBL",CS%Hbbl,.true.)
  if (CS%bottomdraglaw) then
    call read_param(param_file,"CDRAG",CS%cdrag,.true.)
    call read_param(param_file,"DRAG_BG_VEL",CS%drag_bg_vel,.true.)
    call read_param(param_file,"BBL_USE_EOS",CS%BBL_use_EOS)
  endif
  call read_param(param_file,"BBL_THICK_MIN",CS%BBL_thick_min)
  CS%Htbl_shelf_min = CS%BBL_thick_min
  call read_param(param_file,"HTBL_SHELF_MIN",CS%Htbl_shelf_min)
  CS%Htbl_shelf = CS%Hbbl
  call read_param(param_file,"HTBL_SHELF",CS%Htbl_shelf)
  CS%KV_BBL_min = 0.0 ; CS%KV_TBL_MIN = 0.0
  ! ###  call read_param(param_file,"KV",CS%KV_BBL_min)
  ! ###  call read_param(param_file,"KV",CS%KV_TBL_min)
  call read_param(param_file,"KV_BBL_MIN",CS%KV_BBL_min)
  call read_param(param_file,"KV_TBL_MIN",CS%KV_TBL_min)

  if (CS%Channel_drag) then
    CS%c_Smag = -1.0 ; cSmag_chan_dflt = 0.15
    call read_param(param_file,"SMAG_LAP_CONST",CS%c_Smag)
    if (CS%c_Smag >= 0.0) cSmag_chan_dflt = CS%c_Smag
    call read_param(param_file,"SMAG_CONST_CHANNEL",CS%c_Smag)
  ...
  if (CS%bottomdraglaw) then
    ...
    call log_param(param_file, mod, "DRAG_BG_VEL", CS%drag_bg_vel, &
                 ...
                 "BOTTOMDRAGLAW is defined.", units="m s-1")
    call log_param(param_file, mod, "BBL_USE_EOS", CS%BBL_use_EOS, &
  ...
  call log_param(param_file, mod, "LINEAR_DRAG", CS%linear_drag, &
  ...
  call log_param(param_file, mod, "CHANNEL_DRAG", CS%Channel_drag, &
                 ...
                 "LINEAR_DRAG.")
  if (CS%Channel_drag) then
    call log_param(param_file, mod, "SMAG_CONST_CHANNEL", CS%c_Smag, &
                 ...
                 "it is defined, or 0.15 if it is not.", units="nondim", &
                 default=cSmag_chan_dflt)
  endif
  if (adiabatic) then
    ...
    call log_param(param_file, mod, "USE_JACKSON_PARAM", use_kappa_shear, &
    ...
    call log_param(param_file, mod, "RINOMIX", CS%RiNo_mix, &
                 ...
                 default=.false.)
    call log_param(param_file, mod, "DOUBLE_DIFFUSION", double_diffusion, &
  ...
  call log_param(param_file, mod, "HTBL_SHELF", CS%Htbl_shelf, &
  ...
  call log_param(param_file, mod, "BBL_THICK_MIN", CS%BBL_thick_min, &
  ...
  call log_param(param_file, mod, "HTBL_SHELF_MIN", CS%Htbl_shelf_min, &
                 "The minimum top boundary layer thickness that can be \n"//&
                 "used with BOTTOMDRAGLAW. This might be \n"//&
                 "Kv / (cdrag * drag_bg_vel) to give Kv as the minimum \n"//&
                 "near-top viscosity.", units="m", default=0.0)
  call log_param(param_file, mod, "KV_BBL_MIN", CS%KV_BBL_min, &
                 "The minimum viscosities in the bottom boundary layer.", &
                 units="m2 s-1", default=0.0)
  call log_param(param_file, mod, "KV_TBL_MIN", CS%KV_TBL_min, &
  ...
  if (CS%dynamic_viscous_ML) then
    ...
    call log_param(param_file, mod, "TKE_DECAY_VISC", CS%TKE_decay, &
  ...
  CS%BBL_thick_min = CS%BBL_thick_min * G%m_to_H
