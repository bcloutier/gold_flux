...
use GOLD_cpu_clock, only : cpu_clock_id, cpu_clock_begin, cpu_clock_end, CLOCK_MODULE
...
use GOLD_io, only : field_exists, file_exists, read_data
use GOLD_time_manager, only : time_type, time_type_to_real
...
type, public :: tidal_forcing_CS ; private
  ...
  real PTR_, dimension(NXMEM_,NYMEM_,C3_) :: sin_struct, cos_struct
...
!                 for this module.
  real :: phase(SZ1_(G%D), SZ2_(G%D))  ! The phase of some tidal constituent.
  real :: deg_to_rad
  ...
  character(len=128) :: version = '$Id: GOLD_tidal_forcing.F90,v 13.0.2.3.2.8 2010/11/30 23:00:03 rwh Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_tidal_forcing" ! This module's name.
  ...
  if (associated(CS)) then
    call GOLD_error(WARNING, "tidal_forcing_init called with an associated "// &
                            "control structure.")
    return
  endif

  tides = .false. ; call read_param(param_file,"TIDES",tides)
  if (.not.tides) return
  ...
  ALLOC(CS%sin_struct(isd:ied,jsd:jed,3))
  ALLOC(CS%cos_struct(isd:ied,jsd:jed,3))
  deg_to_rad = 4.0*ATAN(1.0)/180.0
  do j=js-1,je+1 ; do i=is-1,ie+1
    CS%sin_struct(i,j,1) = -sin(2.0*G%geolath(i,j)*deg_to_rad) * cos(G%geolonh(i,j)*deg_to_rad)
    CS%cos_struct(i,j,1) = sin(2.0*G%geolath(i,j)*deg_to_rad) * sin(G%geolonh(i,j)*deg_to_rad)
    CS%sin_struct(i,j,2) = -cos(G%geolath(i,j)*deg_to_rad)**2 * sin(2.0*G%geolonh(i,j)*deg_to_rad)
    CS%cos_struct(i,j,2) = cos(G%geolath(i,j)*deg_to_rad)**2 * cos(2.0*G%geolonh(i,j)*deg_to_rad)
    CS%sin_struct(i,j,3) = 0.0
    CS%cos_struct(i,j,3) = (0.5-1.5*sin(G%geolath(i,j)*deg_to_rad)**2)
  enddo ; enddo

  ! Determine how many tidal components are to be used.
  use_M2 = .false. ; call read_param(param_file,"TIDE_M2",use_M2) ; if (use_M2) nc=nc+1
  use_S2 = .false. ; call read_param(param_file,"TIDE_S2",use_S2) ; if (use_S2) nc=nc+1
  use_N2 = .false. ; call read_param(param_file,"TIDE_N2",use_N2) ; if (use_N2) nc=nc+1
  use_K2 = .false. ; call read_param(param_file,"TIDE_K2",use_K2) ; if (use_K2) nc=nc+1
  use_K1 = .false. ; call read_param(param_file,"TIDE_K1",use_K1) ; if (use_K1) nc=nc+1
  use_O1 = .false. ; call read_param(param_file,"TIDE_O1",use_O1) ; if (use_O1) nc=nc+1
  use_P1 = .false. ; call read_param(param_file,"TIDE_P1",use_P1) ; if (use_P1) nc=nc+1
  use_Q1 = .false. ; call read_param(param_file,"TIDE_Q1",use_Q1) ; if (use_Q1) nc=nc+1
  use_MF = .false. ; call read_param(param_file,"TIDE_MF",use_MF) ; if (use_MF) nc=nc+1
  use_MM = .false. ; call read_param(param_file,"TIDE_MM",use_MM) ; if (use_MM) nc=nc+1

  if (nc == 0) then
    call GOLD_error(FATAL, "tidal_forcing_init: "// &
        "TIDES are defined, but no tidal constituents are used.")
    return
  endif

  CS%tidal_sal_from_file = .false. ; CS%use_prev_tides = .false.
  call read_param(param_file,"TIDAL_SAL_FROM_FILE",CS%tidal_sal_from_file)
  call read_param(param_file,"USE_PREVIOUS_TIDES",CS%use_prev_tides)

  CS%use_sal_scalar = .not.CS%tidal_sal_from_file
  call read_param(param_file,"TIDE_USE_SAL_SCALAR",CS%use_sal_scalar)
  ! If it is being used, sal_scalar MUST be specified in param_file.
  if (CS%use_sal_scalar .or. CS%use_prev_tides) &
    call read_param(param_file,"TIDE_SAL_SCALAR_VALUE",CS%sal_scalar,FAIL_IF_MISSING)

  if (nc > MAX_CONSTITUENTS) then
    write(mesg,'("Increase MAX_CONSTITUENTS in GOLD_tidal_forcing.F90 to at least",I3, &
                &"to accomodate all the registered tidal constituents.")') nc
    call GOLD_error(FATAL, "GOLD_tidal_forcing"//mesg)
  endif

  do c=1,4*MAX_CONSTITUENTS ; tidal_input_files(c) = "" ; enddo

  if (CS%tidal_sal_from_file .or. CS%use_prev_tides) &
  ...
  if (use_M2) then
    ...
    call read_param(param_file,"TIDE_M2_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_M2_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_M2_PHASE_T0",CS%phase0(c))
  endif
  ...
  if (use_S2) then
    ...
    freq_def(c) = CS%freq(c) ; love_def(c) = CS%love_no(c)
    amp_def(c) = CS%amp(c) ; phase0_def(c) = CS%phase0(c)
    call read_param(param_file,"TIDE_S2_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_S2_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_S2_PHASE_T0",CS%phase0(c))
  endif
  ...
  if (use_N2) then
    ...
    freq_def(c) = CS%freq(c) ; love_def(c) = CS%love_no(c)
    amp_def(c) = CS%amp(c) ; phase0_def(c) = CS%phase0(c)
    call read_param(param_file,"TIDE_N2_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_N2_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_N2_PHASE_T0",CS%phase0(c))
  endif
  ...
  if (use_K2) then
    ...
    freq_def(c) = CS%freq(c) ; love_def(c) = CS%love_no(c)
    amp_def(c) = CS%amp(c) ; phase0_def(c) = CS%phase0(c)
    call read_param(param_file,"TIDE_K2_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_K2_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_K2_PHASE_T0",CS%phase0(c))
  endif
  ...
  if (use_K1) then
    ...
    freq_def(c) = CS%freq(c) ; love_def(c) = CS%love_no(c)
    amp_def(c) = CS%amp(c) ; phase0_def(c) = CS%phase0(c)
    call read_param(param_file,"TIDE_K1_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_K1_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_K1_PHASE_T0",CS%phase0(c))
  endif
  ...
  if (use_O1) then
    ...
    freq_def(c) = CS%freq(c) ; love_def(c) = CS%love_no(c)
    amp_def(c) = CS%amp(c) ; phase0_def(c) = CS%phase0(c)
    call read_param(param_file,"TIDE_O1_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_O1_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_O1_PHASE_T0",CS%phase0(c))
  endif
  ...
  if (use_P1) then
    ...
    freq_def(c) = CS%freq(c) ; love_def(c) = CS%love_no(c)
    amp_def(c) = CS%amp(c) ; phase0_def(c) = CS%phase0(c)
    call read_param(param_file,"TIDE_P1_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_P1_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_P1_PHASE_T0",CS%phase0(c))
  endif
  ...
  if (use_Q1) then
    ...
    freq_def(c) = CS%freq(c) ; love_def(c) = CS%love_no(c)
    amp_def(c) = CS%amp(c) ; phase0_def(c) = CS%phase0(c)
    call read_param(param_file,"TIDE_Q1_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_Q1_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_Q1_PHASE_T0",CS%phase0(c))
  endif
  ...
  if (use_MF) then
    ...
    freq_def(c) = CS%freq(c) ; love_def(c) = CS%love_no(c)
    amp_def(c) = CS%amp(c) ; phase0_def(c) = CS%phase0(c)
    call read_param(param_file,"TIDE_MF_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_MF_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_MF_PHASE_T0",CS%phase0(c))
  endif
  ...
  if (use_MM) then
    ...
    freq_def(c) = CS%freq(c) ; love_def(c) = CS%love_no(c)
    amp_def(c) = CS%amp(c) ; phase0_def(c) = CS%phase0(c)
    call read_param(param_file,"TIDE_MM_FREQ",CS%freq(c))
    call read_param(param_file,"TIDE_MM_AMP",CS%amp(c))
    call read_param(param_file,"TIDE_MM_PHASE_T0",CS%phase0(c))
  endif
  ...
  call log_param(param_file, mod, "TIDES", tides, &
                 "If true, apply tidal momentum forcing.")
  call log_param(param_file, mod, "TIDE_M2", use_M2, &
                 "If true, apply tidal momentum forcing at the M2 \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDE_S2", use_S2, &
                 "If true, apply tidal momentum forcing at the S2 \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDE_N2", use_N2, &
                 "If true, apply tidal momentum forcing at the N2 \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDE_K2", use_K2, &
                 "If true, apply tidal momentum forcing at the K2 \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDE_K1", use_K1, &
                 "If true, apply tidal momentum forcing at the K1 \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDE_O1", use_O1, &
                 "If true, apply tidal momentum forcing at the O1 \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDE_P1", use_P1, &
                 "If true, apply tidal momentum forcing at the P1 \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDE_Q1", use_Q1, &
                 "If true, apply tidal momentum forcing at the Q1 \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDE_MF", use_MF, &
                 "If true, apply tidal momentum forcing at the MF \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDE_MM", use_MM, &
                 "If true, apply tidal momentum forcing at the MM \n"//&
                 "frequency. This is only used if TIDES is true.", &
                 default=.false.)
  call log_param(param_file, mod, "TIDAL_SAL_FROM_FILE", CS%tidal_sal_from_file, &
  ...
  call log_param(param_file, mod, "USE_PREVIOUS_TIDES", CS%use_prev_tides, &
                 "If true, use the SAL from the previous iteration of the \n"//&
                 "tides to facilitate convergent iteration. \n"//&
                 "This is only used if TIDES is true.", default=.false.)
  call log_param(param_file, mod, "TIDE_USE_SAL_SCALAR", CS%use_sal_scalar, &
                 "If true, use the scalar approximation when calculating \n"//&
                 "self-attraction and loading. \n"//&
                 "This is only used if TIDES is true.", &
                 default=.not.CS%tidal_sal_from_file)
  if (CS%use_sal_scalar .or. CS%use_prev_tides) &
    call log_param(param_file, mod, "TIDE_SAL_SCALAR_VALUE", CS%sal_scalar, &
  ...
  do c=1,nc
    call log_param(param_file, mod, "TIDE_"//trim(CS%const_name(c))//"_FREQ", CS%freq(c), &
    ...
    call log_param(param_file, mod, "TIDE_"//trim(CS%const_name(c))//"_AMP", CS%amp(c), &
                   "Amplitude of the "//trim(CS%const_name(c))//" tidal constituent. \n"//&
                   "This is only used if TIDES and TIDE_"//trim(CS%const_name(c))// &
                   " are true.", units="m", default=amp_def(c))
    call log_param(param_file, mod, "TIDE_"//trim(CS%const_name(c))//"_PHASE_T0", CS%phase0(c), &
...
subroutine find_in_files(tidal_input_files,varname,array,G)
  ...
  do nf=size(tidal_input_files),1,-1
    if (file_exists(tidal_input_files(nf), G%Domain)) then
      call GOLD_error(FATAL, "GOLD_tidal_forcing.F90: Unable to find "// &
         trim(varname)//" in any of the tidal input files, last tried "// &
         trim(tidal_input_files(nf)))
    endif
  enddo

  call GOLD_error(FATAL, "GOLD_tidal_forcing.F90: Unable to find any of the "// &
...
!                              local value of eta, nondim.
  ...
  real :: eta_astro(SZ1_(eta),SZ2_(eta))
  real :: eta_SAL(SZ1_(eta),SZ2_(eta))
...
subroutine tidal_forcing_end(CS)
  ...
  DEALLOC(CS%sin_struct) ; DEALLOC(CS%cos_struct)
