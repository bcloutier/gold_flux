...
use GOLD_error_handler, only : GOLD_error, FATAL, WARNING, NOTE, GOLD_mesg
...
use GOLD_restart, only : GOLD_restart_CS, register_restart_field
use GOLD_variables, only : vertvisc_type
...
! (MEKE).
  ...
  if (.not.associated(CS)) call GOLD_error(FATAL, &
         "GOLD_MEKE: Module must be initialized before it is used.")
  if (.not.associated(MEKE)) call GOLD_error(FATAL, &
         "GOLD_MEKE: MEKE must be initialized before it is used.")
...
! else ! if MEKE%MEKE
!   call GOLD_error(FATAL, "GOLD_MEKE: MEKE%MEKE is not associated!")
  endif
...
! Local variables
  ...
  character(len=128) :: version = '$Id: GOLD_MEKE.F90,v 1.1.2.15 2011/09/19 16:10:36 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_MEKE" ! This module's name.

  is = G%isc ; ie = G%iec ; js = G%jsc ; je = G%jec ; nz = G%ke
  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed

  if (.not. associated(MEKE)) then
    call GOLD_error(WARNING, "MEKE_init called with NO associated "// &
                            "MEKE-type structure.")
    return
  endif
  ...
  call GOLD_mesg("MEKE_init: reading parameters ", 5)

  CS%diag => diag

! Set run-time parameters.
  CS%MEKE_damping = 0.0; call read_param(param_file,"MEKE_DAMPING",CS%MEKE_damping)
  CS%MEKE_GMcoeff =-1. ; call read_param(param_file,"MEKE_GMCOEFF",CS%MEKE_GMcoeff)
  CS%MEKE_FrCoeff =-1. ; call read_param(param_file,"MEKE_FRCOEFF",CS%MEKE_FrCoeff)
  CS%MEKE_BGsrc   = 0.0; call read_param(param_file,"MEKE_BGSRC",CS%MEKE_BGsrc)
  CS%MEKE_Cd_scale = 0.0 ; call read_param(param_file,"MEKE_CD_SCALE",CS%MEKE_Cd_scale)
  CS%MEKE_Uscale  = 1. ; call read_param(param_file,"MEKE_USCALE",CS%MEKE_Uscale)
  CS%MEKE_dtScale = 1. ; call read_param(param_file,"MEKE_DTSCALE",CS%MEKE_dtScale)
  CS%MEKE_Kh      =-1. ; call read_param(param_file,"MEKE_KH",CS%MEKE_Kh)
  CS%MEKE_KhCoeff =-1. ; call read_param(param_file,"MEKE_KHCOEFF",CS%MEKE_KhCoeff)
  CS%visc_drag = .false. ; call read_param(param_file,"MEKE_VISC_DRAG",CS%visc_drag)
  CS%cdrag = 0.0       ; call read_param(param_file,"CDRAG",CS%cdrag)
  MEKE%KhTh_Fac  = 1.0 ; call read_param(param_file,"MEKE_KHTH_FAC",MEKE%KhTh_Fac)
  MEKE%KhTr_Fac  = 1.0 ; call read_param(param_file,"MEKE_KHTR_FAC",MEKE%KhTr_Fac)
  CS%KhMEKE_Fac = 0.0  ; call read_param(param_file,"MEKE_KHMEKE_FAC",CS%KhMEKE_Fac)
...
! Register fields for output from this module.
  ...
  call log_param(param_file, mod, "MEKE_DAMPING", CS%MEKE_damping, &
                 "The local depth-indepented MEKE dissipation rate.", &
                 units="s-1", default=0.0)
  call log_param(param_file, mod, "MEKE_CD_SCALE", CS%MEKE_Cd_scale, &
  ...
  call log_param(param_file, mod, "MEKE_GMCOEFF", CS%MEKE_GMcoeff, &
  ...
  call log_param(param_file, mod, "MEKE_FRCOEFF", CS%MEKE_FrCoeff, &
  ...
  call log_param(param_file, mod, "MEKE_BGSRC", CS%MEKE_BGsrc, &
                 "A background energy source for MEKE.", units="W kg-1", &
                 default=0.0)
  call log_param(param_file, mod, "MEKE_KH", CS%MEKE_Kh, &
                 "A background lateral diffusivity of MEKE, or a \n"//&
                 "negative value not to apply lateral diffusion to MEKE.", &
                 units="m2 s-1", default=-1.0)
  call log_param(param_file, mod, "MEKE_DTSCALE", CS%MEKE_dtScale, &
                 "A scaling factor to accelerate the time evolution of MEKE.", &
                 units="nondim", default=1.0)
  call log_param(param_file, mod, "MEKE_KHCOEFF", CS%MEKE_KhCoeff, &
                 ...
                 units="nondim", default=-1.0)
  call log_param(param_file, mod, "MEKE_USCALE", CS%MEKE_Uscale, &
  ...
  call log_param(param_file, mod, "CDRAG", CS%cdrag, &
                 ...
                 default=0.0)
  call log_param(param_file, mod, "MEKE_VISC_DRAG", CS%visc_drag, &
                 "If true, use the vertvisc_type to calculate the bottom \n"//&
                 "drag acting on MEKE.", default=.false.)
  call log_param(param_file, mod, "MEKE_KHTH_FAC", MEKE%KhTh_fac, &
                 "A factor that maps MEKE%Kh to KhTh.", units="nondim", &
                 default=1.0)
  call log_param(param_file, mod, "MEKE_KHTR_FAC", MEKE%KhTr_fac, &
                 "A factor that maps MEKE%Kh to KhTr.", units="nondim", &
                 default=1.0)
  call log_param(param_file, mod, "MEKE_KHMEKE_FAC", CS%KhMEKE_Fac, &
...
! Allocate control structure
  if (associated(MEKE)) then
    call GOLD_error(WARNING, "MEKE_alloc_register_restart called with an associated "// &
                             "MEKE type.")
    return
  else; allocate(MEKE); endif

! Allocate memory
  call GOLD_mesg("MEKE_alloc_register_restart: allocating and registering", 5)
  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed
  if (MEKE_damping>=0.) then
    allocate(MEKE%MEKE(isd:ied,jsd:jed)) ; MEKE%MEKE(:,:) = 0.0
    vd = vardesc("MEKE","Mesoscale Eddy Kinetic Energy",'h','1','s',"m2 s-2", 'd')
  ...
  if (MEKE_KhCoeff>=0.) then
    ...
    vd = vardesc("MEKE_Kh","Lateral diffusivity from Mesoscale Eddy Kinetic Energy",'h','1','s',"m2 s-1", 'd')
