...
!       largest value from several parameterizations.
        if (CS%Smagorinsky_Ah) then
          ...
          if (CS%bound_Ah) Ah = MIN(Ah, CS%AH_MAX_xx(i,j))
        else
          Ah = CS%Ah_bg_xx(i,j)
      ...
      if (CS%Laplacian) then
        ...
        if (CS%Smagorinsky_Kh) then
          ...
          if (CS%bound_Kh) Kh = MIN(Kh, CS%KH_MAX_xx(i,j))
        else
        ...
        str_xx(i,j) = str_xx(i,j) - Kh * sh_xx(i,j)
      endif ! Laplacian
    ...
    do J=js-1,Jeq ; do I=is-1,Ieq
      ...
      ! largest value from several parameterizations.
        if (CS%Smagorinsky_Ah) then
          ...
          if (CS%bound_Ah) Ah = MIN(Ah, CS%AH_MAX_xy(I,J))
        else
          Ah = CS%Ah_bg_xy(I,J)
        endif ! Smagorinsky_Ah
      ...
      if (CS%Laplacian) then
        ...
        if (CS%Smagorinsky_Kh) then
          ...
          if (CS%bound_Kh) Kh = MIN(Kh, CS%KH_MAX_xy(I,J))
        else
        ...
        str_xy(I,J) = str_xy(I,J) - Kh * sh_xy(I,J)
      endif ! Laplacian
...
!                 for this module
  ...
  integer :: i, j
  character(len=128) :: version = '$Id: GOLD_hor_visc.F90,v 13.0.2.3.2.20 2011/08/10 15:43:52 Robert.Hallberg Exp $'
  ...
  call read_param(param_file,"LAPLACIAN",CS%Laplacian,.true.)
  CS%biharmonic = .not.CS%Laplacian
  call read_param(param_file,"BIHARMONIC",CS%biharmonic)

  CS%bound_Kh = .true. ; CS%Smagorinsky_Kh = .false.
  CS%bound_Ah = .true. ; CS%Smagorinsky_Ah = .false.
  CS%no_slip = .false. ; CS%bound_Coriolis = .false.
  if (CS%Laplacian) then
    call read_param(param_file,"BOUND_KH",CS%bound_Kh)
    call read_param(param_file,"SMAGORINSKY_KH",CS%Smagorinsky_Kh)

    call read_param(param_file,"KH",Kh,.true.)
    Kh_vel_scale = 0.0 ; call read_param(param_file,"KH_VEL_SCALE",Kh_vel_scale)
    if (CS%Smagorinsky_Kh) &
      call read_param(param_file,"SMAG_LAP_CONST",Smag_Lap_const,.true.)
  endif
  if (CS%biharmonic) then
    call read_param(param_file,"BOUND_AH",CS%bound_Ah)
    call read_param(param_file,"SMAGORINSKY_AH",CS%Smagorinsky_Ah)

    call read_param(param_file,"AH",Ah,.true.)
    Ah_vel_scale = 0.0 ; call read_param(param_file,"AH_VEL_SCALE",Ah_vel_scale)
    if (CS%Smagorinsky_Ah) then
      call read_param(param_file,"SMAG_BI_CONST",Smag_bi_const,.true.)
      call read_param(param_file,"BOUND_CORIOLIS",CS%bound_Coriolis)
      bound_Cor_def = CS%bound_Coriolis
      call read_param(param_file,"BOUND_CORIOLIS_BIHARM",CS%bound_Coriolis)
      if (CS%bound_Coriolis) then
        call read_param(param_file,"MAXVEL",maxvel,.true.)
        bound_Cor_vel = maxvel
        call read_param(param_file,"BOUND_CORIOLIS_VEL",bound_Cor_vel)
      endif
    endif
  endif
  call read_param(param_file,"NOSLIP",CS%no_slip)
  if (CS%bound_Kh .or. CS%bound_Ah) call read_param(param_file,"DT",dt)
  ...
  if (CS%Laplacian) then
    ...
    if (CS%bound_Kh) then
      ALLOC(CS%Kh_Max_xx(I_METRIC_SZ,J_METRIC_SZ)) ; CS%Kh_Max_xx(:,:) = 0.0
      ALLOC(CS%Kh_Max_xy(I_METRICQ_SZ,J_METRICQ_SZ)) ; CS%Kh_Max_xy(:,:) = 0.0
    endif
    if (CS%Smagorinsky_Kh) then
  ...
  if (CS%biharmonic) then
    ...
    if (CS%bound_Ah) then
      ALLOC(CS%Ah_Max_xx(I_METRIC_SZ,J_METRIC_SZ)) ; CS%Ah_Max_xx(:,:) = 0.0
      ALLOC(CS%Ah_Max_xy(I_METRICQ_SZ,J_METRICQ_SZ)) ; CS%Ah_Max_xy(:,:) = 0.0
    endif
    if (CS%Smagorinsky_Ah) then
  ...
  if (CS%biharmonic) then
   ...
   ! this to be less than 1/3, rather than 1/2 as before.
    ...
    BoundCorConst = 1.0 / (5.0*(maxvel*maxvel))
    do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
      grid_sp_h2 = (2.0*CS%DX2h(i,j)*CS%DY2h(i,j)) / (CS%DX2h(i,j)+CS%DY2h(i,j))
  ...
  call log_param(param_file, mod, "LAPLACIAN", CS%Laplacian, &
                 "If true, use a Laplacian horizontal viscosity.")
  call log_param(param_file, mod, "BIHARMONIC", CS%biharmonic, &
                 "If true, se a biharmonic horizontal viscosity. \n"//&
                 "BIHARMONIC may be used with LAPLACIAN, and by default \n"//&
                 "BIHARMONIC is true if LAPLACIAN is false.", &
                 default=.not.CS%Laplacian)
  if (CS%Laplacian) then
    call log_param(param_file, mod, "KH", Kh, &
                 "The background Laplacian horizontal viscosity.", &
                 units = "m2 s-1")
    call log_param(param_file, mod, "KH_VEL_SCALE", Kh_vel_scale, &
                 "The velocity scale which is multiplied by the grid \n"//&
                 "spacing to calculate the Laplacian viscosity. \n"//&
                 "The final viscosity is the largest of this scaled \n"//&
                 "viscosity, the Smagorinsky viscosity and KH.", units="m s-1")
    call log_param(param_file, mod, "SMAGORINSKY_KH", CS%Smagorinsky_Kh, &
                 "If true, use a Smagorinsky nonlinear eddy viscosity.", &
                 default=.false.)
    if (CS%Smagorinsky_Kh) &
      call log_param(param_file, mod, "SMAG_LAP_CONST", Smag_Lap_const, &
                 "The nondimensional Laplacian Smagorinsky constant, \n"//&
                 "often 0.15.", units="nondim")
    call log_param(param_file, mod, "BOUND_KH", CS%bound_Kh, &
                 "If true, the Laplacian coefficient is locally limited \n"//&
                 "to be stable.", default=.true.)
  endif
  if (CS%biharmonic) then
    call log_param(param_file, mod, "AH", Ah, &
                 "The background biharmonic horizontal viscosity.", &
                 units = "m4 s-1")
    call log_param(param_file, mod, "AH_VEL_SCALE", Ah_vel_scale, &
                 "The velocity scale which is multiplied by the cube of \n"//&
                 "the grid spacing to calculate the Laplacian viscosity. \n"//&
                 "The final viscosity is the largest of this scaled \n"//&
                 "viscosity, the Smagorinsky viscosity and AH.", units="m s-1")
    call log_param(param_file, mod, "SMAGORINSKY_AH", CS%Smagorinsky_Ah, &
                 "If true, use a biharmonic Smagorinsky nonlinear eddy \n"//&
                 "viscosity.", default=.false.)
    call log_param(param_file, mod, "BOUND_AH", CS%bound_Ah, &
                 "If true, the biharmonic coefficient is locally limited \n"//&
                 "to be stable.", default=.true.)
    if (CS%Smagorinsky_Ah) then
      call log_param(param_file, mod, "SMAG_BI_CONST",Smag_bi_const, &
                 "The nondimensional biharmonic Smagorinsky constant, \n"//&
                 "often 0.015.", units="nondim")
      call log_param(param_file, mod, "BOUND_CORIOLIS_BIHARM", CS%bound_Coriolis, &
                 ...
                 "value of BOUND_CORIOLIS", default=bound_Cor_def)
      if (CS%bound_Coriolis) call log_param(param_file, mod, &
  ...
  call log_param(param_file, mod, "NOSLIP", CS%no_slip, &
  ...
  if (CS%bound_Kh .or. CS%bound_Ah) call log_param(param_file, mod, "DT", dt, &
