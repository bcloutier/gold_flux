...
!*  by GOLD.  This routine is intended to be modified by the user to   *
...
!*                                                                     *
!*    This subroutine is also used by GOLD-related preprocessing and   *
...
!*  in GOLD_grid_macros.h.  These macros allow the metrics to be       *
...
!*                                                                     *
!*    Macros written all in capital letters are from GOLD_memory.h.    *
!*                                                                     *
...
use GOLD_diag_mediator, only : set_axes_info
use GOLD_domains, only : pass_var, pass_vector, pe_here, root_PE, broadcast
use GOLD_checksums, only : hchksum, qchksum, uchksum, vchksum
use GOLD_domains, only : AGRID, BGRID_NE, CGRID_NE, To_All, Scalar_Pair
use GOLD_domains, only : To_North, To_South, To_East, To_West
use GOLD_domains, only : GOLD_define_domain, GOLD_define_IO_domain
use GOLD_domains, only : GOLD_domain_type
use GOLD_error_handler, only : GOLD_error, GOLD_mesg, FATAL, NOTE, is_root_pe
use GOLD_file_parser, only : read_param, log_param, log_version, param_file_type
use GOLD_grid, only : ocean_grid_type
use GOLD_io, only : read_data, slasher, file_exists
use GOLD_io, only : CORNER, NORTH_FACE, EAST_FACE
...
type, public :: GPS ; private
  ...
  integer :: nxtot, nytot         ! Duplicates of nxtot and nytot from GOLD_dom
end type GPS
...
!  Bracket the root.
  ...
  do while (fnbot > 0.0)
    ...
    if ((itt > 50) .and. (fnbot > 0.0)) then
      ...
      call GOLD_error(FATAL,warnmesg)
    endif
  enddo
...
!  sets of points.
  character(len=128) :: version = '$Id: GOLD_grid_initialize.F90,v 1.1.2.21 2011/09/19 16:10:35 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  logical :: debug
  character(len=256) :: config

  call GOLD_mesg("  GOLD_grid_init.F90, set_grid_metrics: allocating metrics", 5)
 
  call allocate_metrics(G)

  call read_param(param_file,"GRID_CONFIG",config,.true.)
  debug=.false.; call read_param(param_file,"DEBUG",debug)

  call log_version(param_file, "GOLD_grid_init", version, tagname, "")
  call log_param(param_file, "GOLD_grid_init", "GRID_CONFIG", config, &
                 "A character string that determines the method for \n"//&
                 "defining the horizontal grid.  Current options are: \n"//&
                 " \t file - read the grid from a file set by GRID_FILE.\n"//&
                 " \t mosaic - read the grid from a mosaic (supergrid) \n"//&
                 " \t          file set by GRID_FILE.\n"//&
                 " \t cartesian - use a (flat) Cartesian grid.\n"//&
                 " \t spherical - use a simple spherical grid.\n"//&
                 " \t mercator - use a Mercator spherical grid.")
  call log_param(param_file, "GOLD_grid_init", "DEBUG", debug, &
                 "If true, write out verbose debugging data.", default=.false.)

  select case (trim(config))
    case ("file");      call set_grid_metrics_from_file(G, param_file)
    ...
    case default ; call GOLD_error(FATAL,  "GOLD_grid_init: set_grid_metrics"//&
...
! This call sets up the diagnostic axes.
  ...
  if (debug) call grid_metrics_chksum('GOLD_grid_init/set_grid_metrics',G)
...
!  sets of points.
  ...
  call GOLD_mesg("  GOLD_grid_init.F90, set_grid_derived_metrics: deriving metrics", 5)
 
  do j=jsd,jed ; do i=isd,ied
    if (G%DXh(i,j) <= 0.0) then
      write(warnmesg,68)  pe_here(),"DXh",i,j,G%DXh(i,j),Epsln
      call GOLD_error(NOTE,warnmesg)
      G%DXh(i,j) = Epsln
    endif
  ...
  do J=Jsdq,Jedq ; do I=Isdq,Iedq
    ...
    G%DXDYq(I,J) = G%DXq(I,J) * G%DYq(I,J)
    G%IDXDYq(I,J) = G%IDXq(I,J) * G%IDYq(I,J)
  enddo ; enddo
...
halo=1 ! AJA
  ...
  do i=isd,ied ; do j=jsd,jed ; tempH(i,j) = G%DXDYh(i,j) ; enddo ; enddo
  call hchksum(tempH,trim(parent)//': DXDYh',G,haloshift=halo)
 
  do I=Isdq,Iedq ; do J=Jsdq,Jedq ; tempQ(I,J) = G%DXDYq(I,J) ; enddo ; enddo
  call qchksum(tempQ,trim(parent)//': DXDYq',G,haloshift=halo)
 
  do i=isd,ied ; do j=jsd,jed ; tempH(i,j) = G%IDXDYh(i,j) ; enddo ; enddo
  call hchksum(tempH,trim(parent)//': IDXDYh',G,haloshift=halo)
 
  do I=Isdq,Iedq ; do J=Jsdq,Jedq ; tempQ(I,J) = G%IDXDYq(I,J) ; enddo ; enddo
  call qchksum(tempQ,trim(parent)//': IDXDYq',G,haloshift=halo)
...
!                         model parameter values.
  ...
  real, dimension(G%Isdq:G%Iedq,G%Jsdq:G%Jedq) :: dxq, dyq, dxdyq
  ...
  type(GOLD_domain_type) :: SGdom ! Supergrid domain
  integer :: i, j
  integer :: npei,npej
  ...
  call GOLD_mesg("   GOLD_grid_init.F90, set_grid_metrics_from_mosaic: reading grid", 5)

  inputdir = "." ; call read_param(param_file,"INPUTDIR",inputdir)
  call read_param(param_file,"GRID_FILE",grid_file,.true.)
  filename = trim(slasher(inputdir)) // trim(grid_file)
  if (.not.file_exists(filename)) &
    call GOLD_error(FATAL," set_grid_metrics_from_mosaic: Unable to open "//&
                           trim(filename))

! Initialize everything to a small number
  dxu(:,:)=Epsln; dyu(:,:)=Epsln
  dxv(:,:)=Epsln; dyv(:,:)=Epsln
  dxq(:,:)=Epsln; dyq(:,:)=Epsln; dxdyq(:,:)=Epsln
...
! Define a domain for the supergrid (SGdom)
  ...
  call GOLD_define_domain(global_indices, SGdom%layout, SGdom%mpp_domain, &
         xflags=G%domain%X_FLAGS, yflags=G%domain%Y_FLAGS, &
         xhalo=SGdom%nx_halo, yhalo=SGdom%ny_halo, &
         xextent=exni,yextent=exnj, &
         symmetry=.true., name="GOLD_MOSAIC")
  if (SGdom%use_io_layout) &
    call GOLD_define_IO_domain(SGdom%mpp_domain, SGdom%io_layout)
...
! Read X from the supergrid
 ...
 !if (err.ne.0) &
 !  call GOLD_error(FATAL," set_grid_metrics_from_mosaic: read_LRG(x) failed!")
...
! Read Y from the supergrid
 ...
 !if (err.ne.0) &
 !  call GOLD_error(FATAL," set_grid_metrics_from_mosaic: read_LRG(y) failed!")
...
! Read DX,DY from the supergrid
 ...
 !if (err.ne.0) &
 !  call GOLD_error(FATAL," set_grid_metrics_from_mosaic: read_LRG(dx) failed!")
 !call read_LRG_supergrid(filename,dv,dy=tmpU,err=err)
 !if (err.ne.0) &
 !  call GOLD_error(FATAL," set_grid_metrics_from_mosaic: read_LRG(dy) failed!")
...
! Read AREA from the supergrid
 ...
 !  call GOLD_error(FATAL," set_grid_metrics_from_mosaic: read_LRG(A) failed!")
  ...
  do I=G%Isdq,G%Iedq ; do J=G%Jsdq,G%Jedq
    G%DXq(I,J) = dxq(I,J) ; G%DYq(I,J) = dyq(I,J) ; G%DXDYq(I,J) = dxdyq(I,J)
  enddo ; enddo
  ...
  call log_param(param_file, "GOLD_grid_init set_grid_metrics_from_mosaic", &
                 "GRID_FILE", filename, &
...
subroutine set_grid_metrics_from_file(G,param_file)
  type(ocean_grid_type),intent(inout) :: G
  type(param_file_type), intent(in)    :: param_file
! Arguments:
!  (inout)   G - The ocean's grid structure.
...
!                         model parameter values.
  ...
  real, dimension(G%isd :G%ied ,G%jsd :G%jed ) :: tempH, tempH2, tempH3, tempH4
  real, dimension(G%Isdq:G%Iedq,G%Jsdq:G%Jedq) :: tempQ, tempQ2, tempQ3, tempQ4
  real, dimension(G%Isdq:G%Iedq,G%jsd :G%jed ) :: tempE, tempE2
  real, dimension(G%isd :G%ied ,G%Jsdq:G%Jedq) :: tempN, tempN2
  ...
  isd = G%isd ; ied = G%ied ; jsd = G%jsd ; jed = G%jed
  isdq = G%isdq ; iedq = G%iedq ; jsdq = G%jsdq ; jedq = G%jedq
  call GOLD_mesg("   GOLD_grid_init.F90, set_grid_metrics_from_file: reading grid", 5)

  inputdir = "." ; call read_param(param_file,"INPUTDIR",inputdir)
  call read_param(param_file,"GRID_FILE",grid_file,.true.)
  filename = trim(slasher(inputdir)) // trim(grid_file)
  if (.not.file_exists(filename, G%Domain)) &
      call GOLD_error(FATAL," set_grid_metrics_from_file: Unable to open " &
                     //trim(filename))
! Read the geographic latitudes and longitudes at all points.
  call Read_and_extrapolate(filename,"x_T",G%geolonh,G)
  call Read_and_extrapolate(filename,"y_T",G%geolath,G)    
  call Read_and_extrap_pairs_Qpt(filename,"y_C","y_C",G%geolatq,tempQ,G)
  call Read_and_extrap_pairs_Upt(filename,"y_E","y_N",G%geolatu,G%geolatv,G)
  call Read_and_extrap_pairs_Qpt(filename,"x_C","x_C",G%geolonq,tempQ,G)
  call Read_and_extrap_pairs_Upt(filename,"x_E","x_N",G%geolonu,G%geolonv,G)


! The following code adds the lengths of subsegments.  This is
! necessary because of omissions in the grid-specification file.

    call Read_and_extrapolate(filename,"area_T",tempH,G)
    do j=jsd,jed ; do i=isd,ied
     G%DXDYh(i,j) = tempH(i,j)
    enddo ; enddo
      
    call Read_and_extrap_pairs_Hpt(filename,"ds_01_11_T","ds_10_11_T",tempH,tempH3,G)
    call Read_and_extrap_pairs_Hpt(filename,"ds_11_21_T","ds_11_12_T",tempH2,tempH4,G)
    do j=jsd,jed ; do i=isd,ied
      G%DXh(i,j) = tempH(i,j)  + tempH2(i,j)
      G%DYh(i,j) = tempH3(i,j) + tempH4(i,j)
    enddo ; enddo 
    
    call Read_and_extrap_pairs_Qpt(filename,"ds_01_11_C","ds_10_11_C",tempQ,tempQ3,G)
    call Read_and_extrap_pairs_Qpt(filename,"ds_11_21_C","ds_11_12_C",tempQ2,tempQ4,G)
    do j=jsdq,jedq ; do i=isdq,iedq
      G%DXq(i,j) = tempQ(i,j)  + tempQ2(i,j)
      G%DYq(i,j) = tempQ3(i,j) + tempQ4(i,j)
    enddo ; enddo

    call Read_and_extrap_pairs_Upt(filename,"ds_01_11_E","ds_10_11_N",tempE,tempN,G)
    call Read_and_extrap_pairs_Upt(filename,"ds_11_21_E","ds_11_12_N",tempE2,tempN2,G)
    do j=jsd,jed ; do i=Isdq,Iedq ; G%DXu(i,j) = tempE(i,j) + tempE2(i,j); enddo;enddo     
    do j=Jsdq,Jedq ; do i=isd,ied ; G%DYv(i,j) = tempN(i,j) + tempN2(i,j); enddo;enddo
      
    call Read_and_extrap_pairs_Upt(filename,"ds_10_11_E","ds_01_11_N",tempE,tempN,G)
    call Read_and_extrap_pairs_Upt(filename,"ds_11_12_E","ds_11_21_N",tempE2,tempN2,G)
    do j=jsd,jed ; do i=Isdq,Iedq ; G%DYu(i,j) = tempE(i,j) + tempE2(i,j); enddo ; enddo
    do j=Jsdq,Jedq ; do i=isd,ied ; G%DXv(i,j) = tempN(i,j) + tempN2(i,j); enddo ; enddo

    call read_data(filename,"lat_t", G%gridlath(ny_halo+1:ny_halo+G%Domain%nytot))
    call read_data(filename,"lat_c", G%gridlatq(ny_halo+1:ny_halo+G%Domain%nytot))
    G%gridlatq(ny_halo) = 2.0*G%gridlatq(ny_halo+1)-G%gridlatq(ny_halo+2)
    call read_data(filename,"lon_t", G%gridlonh(nx_halo+1:nx_halo+nxtot))
    call read_data(filename,"lon_c", G%gridlonq(nx_halo+1:nx_halo+nxtot))
    G%gridlonq(nx_halo) = 2.0*G%gridlonq(nx_halo+1)-G%gridlonq(nx_halo+2)

!    close_file(&cdfid)
  call log_param(param_file, "GOLD_grid_init set_grid_metrics_from_file", &
                 "GRID_FILE", filename, &
                 "Name of the file from which to read horizontal grid data.")
end subroutine set_grid_metrics_from_file
...
!  sets of points.
  ...
  character(len=60) :: axis_units
  character(len=48)  :: mod ! This module & subroutine's name.
  ...
  call GOLD_mesg("  GOLD_grid_init.F90, set_grid_metrics_cartesian: setting metrics", 5)
...
! point to the same scalars, covering all i and j and the 4 subgrids.
  ...
  do j=jsd,jed ; do i=isd,ied
    ...
    G%DXDYh(i,j) = G%DXDYq(I,J) ; G%IDXDYh(i,j) = G%IDXDYq(I,J)
  enddo ; enddo
  ...
  mod = "GOLD_grid_init set_grid_metrics_cartesian"
...
!  sets of points.
  ...
  call GOLD_mesg("  GOLD_grid_init.F90, set_grid_metrics_simple_spherical: "// &
                "setting metrics", 5)
 
!    Calculate the values of the metric terms that might be used
!  and save them in arrays.
  PI = 4.0*atan(1.0); PI_180 = atan(1.0)/45.
  call read_param(param_file,"SOUTHLAT",south_lat, .true.)
  call read_param(param_file,"LENLAT",len_lat, .true.)
  west_lon = 0.0 ; call read_param(param_file,"WESTLON",west_lon)
  call read_param(param_file,"LENLON",len_lon, .true.)
  call read_param(param_file,"RAD_EARTH",Rad_Earth,.true.)
  ...
  if (trim(axis_units) .ne. 'degrees') call GOLD_error(FATAL, &
    "GOLD_grid_init.F90, set_grid_metrics_simple_spherical: "// &
...
!   G%DXDYh(i,j) = Rad_Earth**2*dLon*dLat*ABS(SIN(latitude)-SIN(dL_di))
    G%DXDYh(i,j) = G%DXh(i,j) * G%DYh(i,j)
  enddo; enddo

  mod = "GOLD_grid_init set_grid_metrics_spherical"
  call log_param(param_file, mod, "SOUTHLAT", south_lat, &
...
!  The metric terms within the computational domain are set here.
  ...
  call GOLD_mesg("  GOLD_grid_init.F90, set_grid_metrics_mercator: setting metrics", 5)
 
!    Calculate the values of the metric terms that might be used
!  and save them in arrays.
  PI = 4.0*atan(1.0) ; PI_2 = 0.5*PI
  call read_param(param_file,"SOUTHLAT",GP%south_lat, .true.)
  call read_param(param_file,"LENLAT",GP%len_lat, .true.)
  GP%west_lon = 0.0 ; call read_param(param_file,"WESTLON",GP%west_lon)
  call read_param(param_file,"LENLON",GP%len_lon, .true.)
  call read_param(param_file,"RAD_EARTH",GP%Rad_Earth,.true.)

  GP%axis_units='degrees';call read_param(param_file,"AXIS_UNITS",GP%axis_units)
  GP%isotropic = .false.; call read_param(param_file,"ISOTROPIC",GP%isotropic)
  GP%equator_reference = .true.; call read_param(param_file,"EQUATOR_REFERENCE",GP%equator_reference)
  GP%Lat_enhance_factor = 1.0; call read_param(param_file,"LAT_ENHANCE_FACTOR",GP%Lat_enhance_factor)
  GP%Lat_eq_enhance = 0.0; call read_param(param_file,"LAT_EQ_ENHANCE",GP%Lat_eq_enhance)
...
# ifndef XMETRIC_J
  do J=Jsdq+1,Jedq ; do I=Isdq+1,Iedq
    if (ABS(cos(yq(I,J))-cos(yq(I,J-1))) > 1e-12) then
      call GOLD_error(FATAL,"ERROR setting metric terms: XMETRIC_J should be defined in GOLD_memory.h!")
    endif
  enddo ; enddo
# endif
# ifndef XMETRIC_I
  do J=Jsdq+1,Jedq ; do I=Isdq+1,Iedq
    if (ABS(dx_di(xq(I,J),GP)-dx_di(xq(I-1,J),GP)) > 1e-12*dx_di(xq(I,J),GP)) &
      call GOLD_error(FATAL,"ERROR setting metric terms: XMETRIC_I should be defined in GOLD_memory.h!")
  enddo ; enddo
# endif
# ifndef YMETRIC_J
  do J=Jsdq+1,Jedq ; do I=Isdq+1,Iedq
    if (ABS(dy_dj(yq(I,J),GP)-dy_dj(yq(I,J-1),GP)) > 1e-12*dy_dj(yq(I,J),GP)) &
      call GOLD_error(FATAL,"ERROR setting metric terms: YMETRIC_J should be defined in GOLD_memory.h!")
  enddo ; enddo
# endif
  ...
  mod = "GOLD_grid_init set_grid_metrics_mercator"
  call log_param(param_file, mod, "SOUTHLAT", GP%south_lat, &
                 "The southern latitude of the domain.", units="degrees")
  call log_param(param_file, mod, "LENLAT", GP%len_lat, &
                 "The latitudinal length of the domain.", units="degrees")
  call log_param(param_file, mod, "WESTLON", GP%west_lon, &
                 "The western longitude of the domain.", units="degrees", &
                 default=0.0)
  call log_param(param_file, mod, "LENLON", GP%len_lon, &
                 "The longitudinal length of the domain.", units="degrees")
  call log_param(param_file, mod, "RAD_EARTH", GP%Rad_Earth, &
                 "The radius of the Earth.", units="m")
  call log_param(param_file, mod, "ISOTROPIC", GP%isotropic, &
...
subroutine Read_and_extrapolate(filename, name, var, G)
  character(len=*),                         intent(in)    :: filename
  character(len=*),                         intent(in)    :: name
  type(ocean_grid_type),                    intent(inout) :: G
  real, dimension(G%isd:G%ied,G%jsd:G%jed), intent(out)   ::  var
  real, dimension(G%isd:G%ied,G%jsd:G%jed,1) :: temp

  ! This subroutine reads in an array on the tracer subgrid.  Because of the
  ! interchange of logical east- and north-faces on some grids, scalars on
  ! those subgrids should be read by Read_and_extrap_pairs.
  integer :: i, j
  
  var(:,:) = -1.0e34
  temp(:,:,1) = -1.0e34
  
  call read_data(filename,name, temp, domain = G%Domain%mpp_domain)
  ...
  do j=G%jsc,G%jec ; do i=G%iec+1,G%ied
    if (var(i,j) < -9.99e33) var(i,j) = 2.0*var(i-1,j)-var(i-2,j)
  enddo ; enddo
  do j=G%jsc,G%jec ; do i=G%isc-1,G%isd,-1
    if (var(i,j) < -9.99e33) var(i,j) = 2.0*var(i+1,j)-var(i+2,j)
  enddo ; enddo
  do j=G%jec+1,G%jed ; do i=G%isd,G%ied
    if (var(i,j) < -9.99e33) var(i,j) = 2.0*var(i,j-1)-var(i,j-2)
  enddo ; enddo
  do j=G%jsc-1,G%jsd,-1 ; do i=G%isd,G%ied
    if (var(i,j) < -9.99e33) var(i,j) = 2.0*var(i,j+1)-var(i,j+2)
  enddo ; enddo
end subroutine Read_and_extrapolate


subroutine Read_and_extrap_pairs_Hpt(filename, name_x, name_y, var_x, var_y, G)
  character(len=*), intent(in) :: filename
  character(len=*), intent(in) :: name_x, name_y
  type(ocean_grid_type), intent(inout) :: G
  real, intent(inout) :: var_x(G%isd:G%ied,G%jsd:G%jed)
  real, intent(inout) :: var_y(G%isd:G%ied,G%jsd:G%jed) 
  real, dimension(G%isd:G%ied,G%jsd:G%jed,1) :: temp_x, temp_y
  ...
  do j=jsd,jed ; do i=isd,ied
    var_x(i,j) = -1.0e34; var_y(i,j) = -1.0e34
    temp_x(i,j,1) = -1.0e34; temp_y(i,j,1) = -1.0e34
  enddo ; enddo
  
  call read_data(filename, name_x, temp_x, domain = G%Domain%mpp_domain)
  call read_data(filename, name_y, temp_y, domain = G%Domain%mpp_domain)     
  
  do j=jsd,jed ; do i=isd,ied
    var_x(i,j) = temp_x(i,j,1) ; var_y(i,j) = temp_y(i,j,1)
  enddo ; enddo  
    
  call pass_vector(var_x, var_y, G%Domain, To_All+Scalar_Pair, AGRID)

  do j=jsc,jec ; do i=iec+1,ied
    if (var_x(i,j) < -9.99e33) var_x(i,j) = 2.0*var_x(i-1,j)-var_x(i-2,j)
    if (var_y(i,j) < -9.99e33) var_y(i,j) = 2.0*var_y(i-1,j)-var_y(i-2,j) 
  enddo ; enddo
  do j=jsc,jec ; do i=isc-1,isd,-1
    if (var_x(i,j) < -9.99e33) var_x(i,j) = 2.0*var_x(i+1,j)-var_x(i+2,j)
    if (var_y(i,j) < -9.99e33) var_y(i,j) = 2.0*var_y(i+1,j)-var_y(i+2,j)
  enddo ; enddo
  do j=jec+1,jed ; do i=isd,ied
    if (var_x(i,j) < -9.99e33) var_x(i,j) = 2.0*var_x(i,j-1)-var_x(i,j-2)
    if (var_y(i,j) < -9.99e33) var_y(i,j) = 2.0*var_y(i,j-1)-var_y(i,j-2)
  enddo ; enddo
  do j=jsc-1,jsd,-1 ; do i=isd,ied
    if (var_x(i,j) < -9.99e33) var_x(i,j) = 2.0*var_x(i,j+1)-var_x(i,j+2)
    if (var_y(i,j) < -9.99e33) var_y(i,j) = 2.0*var_y(i,j+1)-var_y(i,j+2)
  enddo ; enddo  
  
end subroutine Read_and_extrap_pairs_Hpt

subroutine Read_and_extrap_pairs_Qpt(filename, name_x, name_y, var_x, var_y, G)
  character(len=*),                             intent(in) :: filename
  character(len=*),                             intent(in) :: name_x, name_y
  type(ocean_grid_type),                        intent(inout) :: G
  real, dimension(G%isdq:G%iedq,G%jsdq:G%jedq), intent(inout) :: var_x, var_y

  real, dimension(G%isd:G%ied,G%jsd:G%jed,1) :: temp_x, temp_y
  integer :: i, j, isc, iec, jsc, jec, isd, ied, jsd, jed
  isc = G%Isc ; iec = G%Iec ; jsc = G%Jsc ; jec = G%Jec    
  isd = G%Isdq ; ied = G%Iedq ; jsd = G%Jsdq ; jed = G%Jedq 
 
  temp_x(:,:,1) = -1.0e34 ; temp_y(:,:,1) = -1.0e34
  var_x(:,:) = -1.0e34 ; var_y(:,:) = -1.0e34
   
  call read_data(filename, name_x, temp_x, domain = G%Domain%mpp_domain)
  call read_data(filename, name_y, temp_y, domain = G%Domain%mpp_domain)     
  
  call pass_vector(temp_x, temp_y, G%Domain, To_All+Scalar_Pair, AGRID)
  ...
  call pass_vector(var_x, var_y, G%Domain, To_All+Scalar_Pair, BGRID_NE)
   
end subroutine Read_and_extrap_pairs_Qpt

subroutine Read_and_extrap_pairs_Upt(filename, name_x, name_y, var_x, var_y, G)
  character(len=*), intent(in) :: filename
  character(len=*), intent(in) :: name_x, name_y
  type(ocean_grid_type), intent(inout) :: G
  real, intent(inout) :: var_x(G%isdq:G%iedq,G%jsd:G%jed)
  real, intent(inout) :: var_y(G%isd:G%ied,G%jsdq:G%jedq) 
  real, dimension(G%isd:G%ied,G%jsd:G%jed,1) :: temp_x, temp_y
  integer :: i, j
  ...
  do j=G%jsc,G%jec ; do i=G%iec+1,G%Iedq
    if (var_x(i,j) < -9.99e33) var_x(i,j) = 2.0*var_x(i-1,j)-var_x(i-2,j)
  enddo ; enddo
  do j=G%jsc,G%jec ; do i=G%isc-1,G%Isdq,-1
    if (var_x(i,j) < -9.99e33) var_x(i,j) = 2.0*var_x(i+1,j)-var_x(i+2,j)
  enddo ; enddo
  do j=G%jec+1,G%jed ; do i=G%Isdq,G%Iedq
    if (var_x(i,j) < -9.99e33) var_x(i,j) = 2.0*var_x(i,j-1)-var_x(i,j-2)
  enddo ; enddo
  do j=G%jsc-1,G%jsd,-1 ; do i=G%Isdq,G%Iedq
    if (var_x(i,j) < -9.99e33) var_x(i,j) = 2.0*var_x(i,j+1)-var_x(i,j+2)
  enddo ; enddo   
  ...
  do j=G%jsc,G%jec ; do i=G%isc-1,G%isd,-1
    if (var_y(i,j) < -9.99e33) var_y(i,j) = 2.0*var_y(i+1,j)-var_y(i+2,j)
  enddo ; enddo
  do j=G%jec+1,G%Jedq ; do i=G%isd,G%ied
    if (var_y(i,j) < -9.99e33) var_y(i,j) = 2.0*var_y(i,j-1)-var_y(i,j-2)
  enddo ; enddo
  do j=G%jsc-1,G%Jsdq,-1 ; do i=G%isd,G%ied
    if (var_y(i,j) < -9.99e33) var_y(i,j) = 2.0*var_y(i,j+1)-var_y(i,j+2)
  enddo ; enddo
  
  call pass_vector(var_x, var_y, G%Domain, To_All+Scalar_Pair, CGRID_NE)
  
end subroutine Read_and_extrap_pairs_Upt
...
subroutine initialize_masks(G, param_file)
  type(ocean_grid_type), intent(inout) :: G
  type(param_file_type), intent(in)    :: param_file
! Arguments:
!  (inout)   G - The ocean's grid structure.
...
!!!!!!!!!!!!!!!!! Mehmet !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  apply_OBC_u_flather_east = .false. ; call read_param(param_file,"APPLY_OBC_U_FLATHER_EAST",apply_OBC_u_flather_east)
  apply_OBC_u_flather_west = .false. ; call read_param(param_file,"APPLY_OBC_U_FLATHER_WEST",apply_OBC_u_flather_west)
  apply_OBC_v_flather_north = .false. ; call read_param(param_file,"APPLY_OBC_V_FLATHER_NORTH",apply_OBC_v_flather_north)
  apply_OBC_v_flather_south = .false. ; call read_param(param_file,"APPLY_OBC_V_FLATHER_SOUTH",apply_OBC_v_flather_south)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  

!!!!!!!!!!!!!!!! Mehmet !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  isd = G%isd; isd_global = G%isd_global
  jsd = G%jsd; jsd_global = G%jsd_global
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  
...
if (G%symmetric) then
  ...
  if (apply_OBC_u_flather_west) then
    do j=G%jsd,G%jed ; do i=G%isc,G%iec
       if ((I+G%isd_global-G%isd) .eq. G%domain%nx_halo+1) then
         G%D(i-1,j)=G%D(i,j)
       endif
    enddo; enddo       
  endif
 ...
 if (apply_OBC_u_flather_east) then
   do j=G%jsd,G%jed ; do i=G%isc,G%iec
      if ((I+G%isd_global-G%isd) .eq. G%domain%nxtot+G%domain%nx_halo) then
        G%D(i+1,j)=G%D(i,j)
      endif
   enddo; enddo    
 endif
 ...
 if (apply_OBC_v_flather_north) then
   do j=G%jsc,G%jec ; do i=G%isd,G%ied
      if ((J+G%jsd_global-G%jsd) .eq. G%domain%nytot+G%domain%ny_halo) then
        G%D(i,j+1)=G%D(i,j)
      endif
   enddo; enddo    
 endif
 ...
 if (apply_OBC_v_flather_south) then
   do j=G%jsc,G%jec ; do i=G%isd,G%ied
      if ((J+G%jsd_global-G%jsd) .eq. G%domain%ny_halo+1) then
        G%D(i,j-1)=G%D(i,j)
      endif
   enddo; enddo
 endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
  ...
  do j=G%Jsdq,G%Jedq ; do i=G%isd,G%ied
    ...
    G%dx_v_obc(i,j) = G%vmask(i,j)*G%DXv(i,j)           !Mehmet    
  ...
  do j=G%jsd,G%jed ; do i=G%Isdq,G%Iedq  
    ...
    G%dy_u_obc(i,j) = G%umask(i,j)*G%DYu(i,j)           !Mehmet    
    G%dxdy_u(i,j) = G%DXu(i,j)*G%dy_u(i,j)
    G%Idxdy_u(i,j) = 0.0
    if (G%dxdy_u(i,j) > 0.0) G%Idxdy_u(i,j) = G%umask(i,j) / G%dxdy_u(i,j)
  enddo ; enddo

  call log_param(param_file, "GOLD_grid_init initialize_masks", &
...
end module GOLD_grid_initialize
