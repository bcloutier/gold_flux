...
use GOLD_io, only : write_version_number
use GOLD_time_manager, only : time_type, set_time, time_type_to_real
use GOLD_variables, only : directories, forcing, surface
...
!                 call to initialize_ice_shelf.
  ...
  if (.not. associated(CS)) call GOLD_error(FATAL, "shelf_calc_flux: "// &
...
subroutine initialize_ice_shelf(Time, CS, fluxes, Time_in, solo_mode_in)
  ...
  character(len=128) :: version = '$Id: GOLD_ice_shelf.F90,v 1.1.2.20.2.1 2011/10/26 17:21:08 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: null_ice_shelf_ogrp $'
  character(len=40)  :: mod = "GOLD_ice_shelf"  ! This module's name.

  if (associated(CS)) then
    call GOLD_error(WARNING, "shelf_model_init called with an associated "// &
                             "control structure.")
    return
  endif
  ...
  ! GOLD's grid and infrastructure.
  ...
  call GOLD_error(FATAL, "This version of the code does not have an active "//&
                         "ice shelf available.")
