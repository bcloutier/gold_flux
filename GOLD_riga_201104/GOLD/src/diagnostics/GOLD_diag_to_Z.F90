...
type, public :: diag_to_Z_CS ; private
  integer :: nkml           ! The number of parts to the mixed layer.
...
!                 diagnostics_init.
  ...
  integer :: i, j, k, k2, kz, is, ie, js, je, Isq, Ieq, Jsq, Jeq, nz, m
        ...
        do kz=1,CS%nz_zspace
          ...
          if (linear_velocity_profiles) then
            ...
            if (k /= k_bot_prev) then
              ...
              slope = 0.0 ! ; curv = 0.0
              if ((k < nz_valid(I)) .and. (k > CS%nkml)) call &
    ...
    do kz=1,CS%nz_zspace+1 ; do i=is,ie
      ...
      if ((kL(i)>1) .and. (kL(i)<=nz+1)) then
        if (e(i,kL(i)-1) == e(i,kL(i))) call GOLD_error(WARNING, &
          "calc_Zint_diags: Interface depths equal.")
...
!                 diagnostics_init.
  ...
  if (CS%nz_zspace > 0) then
    ...
    id_test = register_diag_field('ocean_model', name, G%axesh1, Time, &
                                  long_name, units, missing_value=CS%missing_tr(m))
    if (id_test>0 .and. is_root_pe()) call GOLD_error(WARNING, &
...
!                 for this module
  character(len=128) :: version = '$Id: GOLD_diag_to_Z.F90,v 1.1.4.3.2.18 2010/11/30 23:00:02 rwh Exp $'
  ...
  if (bulkmixedlayer) call read_param(param_file,"NKML",CS%nkml)
  
  ! Read in z-space info from a NetCDF file.
  zgrid_file = "" ; call read_param(param_file,"Z_OUTPUT_GRID_FILE", zgrid_file)
  if (len_trim(zgrid_file) > 0) then
    in_dir = "./" ; call read_param(param_file,"INPUTDIR",in_dir) ; in_dir = slasher(in_dir)  
  ...
  if (CS%nz_zspace > 0) then
    call log_param(param_file, mod, "BULKMIXEDLAYER", bulkmixedlayer, &
                 ...
                 "with transitional buffer layers.")
    if (bulkmixedlayer) call log_param(param_file, mod, "NKML", CS%nkml, &
                 "The number of sublayers within the mixed layer if \n"//&
                 "BULKMIXEDLAYER is true.", units="nondim")
  endif
...
! present.
    if (CS%id_tr(m)>0 .and. is_root_pe()) call GOLD_error(WARNING, &
