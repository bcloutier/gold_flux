...
!                     heights, in m or kg m-2.
  ...
  real :: Rcv(SZ1_(h),SZ2_(h),SZ3_(h)) ! The coordinate variable potential
                                       ! density, in kg m-3.
  real :: pres(SZ1_(h))
  real :: wt, wt_p
...
!       do j=js,je ; do i=is,ie ; z_bot(i,j) = -P_SURF(i,j)/G%H_to_Pa ; enddo ; enddo
      ...
      elseif (associated(tv%Rml)) then
        do k=1,tv%nk_Rml ; do j=js,je ; do i=is,ie
          mass(i,j) = mass(i,j) + (G%H_to_m*tv%Rml(i,j,k))*h(i,j,k)
        enddo ; enddo ; enddo
...
!                 diagnostics_init.
  ...
  integer :: lev
  real :: KE_u(SZ1_(u),SZ2_(u))
  real :: KE_v(SZ1_(v),SZ2_(v))
  real :: KE_h(SZ1_(h),SZ2_(h))
...
!                 for this module
  character(len=128) :: version = '$Id: GOLD_diagnostics.F90,v 13.0.2.9.2.20 2010/11/30 23:00:02 rwh Exp $'
