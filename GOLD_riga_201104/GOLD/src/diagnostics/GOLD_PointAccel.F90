...
!  if (.not.associated(CS)) return
  ...
  ! Determine which layers to write out accelerations for.
    ...
    write(file,'(/,"u(mp): ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') (CS%u(I,j,k,mp)); enddo
    write(file,'(/,"u(3):  ",$)')
    ...
    write(file,'(/,"v--:   ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') (CS%v(i,J-1,k,mp)); enddo
    write(file,'(/,"v-+:   ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') (CS%v(i,J,k,mp)); enddo
    write(file,'(/,"v+-:   ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') (CS%v(i+1,J-1,k,mp)); enddo
    write(file,'(/,"v++:   ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') (CS%v(i+1,J,k,mp)); enddo
  ...
  !  From here on, the normalized accelerations are written.
    ...
    do k=ks,ke ; if (do_k(k)) write(file,'(F10.6," ",$)') &
                      ((um(I,j,k)-CS%u(I,j,k,mp))*Inorm(k)); enddo
...
!  if (.not.associated(CS)) return
  ...
  ! Open up the file for output if this is the first call.
    ...
    write(file,'(/,"u--:   ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') CS%u(I-1,j,k,mp); enddo
    write(file,'(/,"u-+:   ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') CS%u(I-1,j+1,k,mp); enddo
    write(file,'(/,"u+-:   ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') CS%u(I,j,k,mp); enddo
    write(file,'(/,"u++:   ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') CS%u(I,j+1,k,mp); enddo
    ...
    write(file,'(/," uhCp--:",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') &
                        (0.5*CS%u(I-1,j,k,mp)*(hin(i-1,j,k) + hin(i,j,k))); enddo
    ...
    write(file,'(/," uhCp-+:",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') &
                  (0.5*CS%u(I-1,j+1,k,mp)*(hin(i-1,j+1,k) + hin(i,j+1,k))); enddo
    ...
    write(file,'(/," uhCp+-:",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') &
                          (0.5*CS%u(I,j,k,mp)*(hin(i,j,k) + hin(i+1,j,k))); enddo
    ...
    write(file,'(/," uhCp++:",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(ES10.3," ",$)') &
                    (0.5*CS%u(I,j+1,k,mp)*(hin(i,j+1,k) + hin(i+1,j+1,k))); enddo
  ...
  !  From here on, the normalized accelerations are written.
    ...
    write(file,'(/,"dv:    ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(F10.6," ",$)') &
                                    ((vm(i,J,k)-CS%v(i,J,k,mp))*Inorm(k)); enddo
    write(file,'(/,"CAv:   ",$)')
    do k=ks,ke ; if (do_k(k)) write(file,'(F10.6," ",$)') &
...
!                 for this module
  character(len=128) :: version = '$Id: GOLD_PointAccel.F90,v 13.0.2.8.2.13 2011/04/12 17:57:12 Robert.Hallberg Exp $'
  ...
  CS%max_writes = 50 ; CS%cols_written = 0
  ! There is no default for CS%split - it must be found in param_file.
  ...
  call read_param(param_file,"MAX_TRUNC_FILE_SIZE_PER_PE",CS%max_writes)
  ...
  call log_param(param_file, mod, "V_TRUNC_FILE", CS%v_trunc_file, &
                 ...
                 "Leave this empty for efficiency if this diagnostic is \n"//&
                 "not needed.", default="")
  call log_param(param_file, mod, "MAX_TRUNC_FILE_SIZE_PER_PE", CS%max_writes, &
