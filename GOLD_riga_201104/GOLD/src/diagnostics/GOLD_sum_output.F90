...
use GOLD_coms, only : sum_across_PEs, PE_here, root_PE, num_PEs, max_across_PEs
...
use GOLD_io, only : create_file, fieldtype, flush_file, open_file, reopen_file
use GOLD_io, only : file_exists, slasher, vardesc, write_field
use GOLD_io, only : APPEND_FILE, ASCII_FILE, SINGLE_FILE, WRITEONLY_FILE
use GOLD_time_manager, only : time_type, get_time, set_time, operator(>), operator(-)
use GOLD_tracer_flow_control, only : tracer_flow_control_CS, call_tracer_stocks
use GOLD_variables, only : forcing, surface, thermo_var_ptrs
...
public write_energy, accumulate_net_input, GOLD_sum_output_init
...
subroutine GOLD_sum_output_init(G, param_file, directory, ntrnc, &
                                Input_start_time, CS)
...
!                 for this module
  real :: Rho_0, maxvel
  character(len=128) :: version = '$Id: GOLD_sum_output.F90,v 13.0.2.7.2.42 2011/09/29 19:56:21 Robert.Hallberg Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_sum_output" ! This module's name.
  character(len=200) :: energyfile  ! The name of the energy file.

  if (associated(CS)) then
    call GOLD_error(WARNING, "GOLD_sum_output_init called with associated control structure.")
    return
  endif
  allocate(CS)

  CS%maxtrunc = 0 ; energyfile = "timestats" ; CS%write_stocks = .true.
  call read_param(param_file,"TEMPERATURE",CS%use_temperature,.true.)
  call read_param(param_file,"DT",CS%dt,.true.)
  call read_param(param_file,"MAXTRUNC",CS%maxtrunc)
  call read_param(param_file,"ENERGYFILE",energyfile)
  CS%write_stocks = .true.
  call read_param(param_file,"WRITE_STOCKS",CS%write_stocks)
  if (CS%use_temperature) call read_param(param_file,"C_P",CS%C_p,.true.)
  CS%norm = .true. ; call read_param(param_file,"NORMALIZED_SUM_OUT",CS%norm)
  CS%Timeunit = 86400.0 ; call read_param(param_file,"TIMEUNIT",CS%Timeunit)
  if (CS%Timeunit < 0.0) CS%Timeunit = 86400.0

  CS%max_Energy = 0.0 ; call read_param(param_file,"MAX_ENERGY",CS%max_Energy)
  if (CS%max_Energy <= 0.0) then
    maxvel = 1.0e6 ; call read_param(param_file,"MAXVEL",maxvel)
    CS%max_Energy = 10.0 * maxvel**2
  endif

  CS%use_repro_sum = .true.
  call read_param(param_file,"USE_REPRODUCING_SUM",CS%use_repro_sum)

  CS%do_APE_calc = .true.
  call read_param(param_file,"CALCULATE_APE",CS%do_APE_calc)
  if (CS%do_APE_calc) then
    CS%read_depth_list = .false.
    call read_param(param_file, "READ_DEPTH_LIST", CS%read_depth_list)
    CS%D_list_min_inc = G%Angstrom_Z ! = 0.0
    call read_param(param_file, "DEPTH_LIST_MIN_INC", CS%D_list_min_inc)
    if (CS%read_depth_list) then
      CS%depth_list_file = "Depth_list.nc"
      call read_param(param_file, "DEPTH_LIST_FILE", CS%depth_list_file)
  ...
  call log_param(param_file, mod, "CALCULATE_APE", CS%do_APE_calc, &
                 ...
                 default=.true.)
  call log_param(param_file, mod, "WRITE_STOCKS", CS%write_stocks, &
                 "If true, write the integrated tracer amounts to stdout \n"//&
                 "when the energy files are written.", default=.true.)
  call log_param(param_file, mod, "USE_REPRODUCING_SUM", CS%use_repro_sum, &
  ...
  call log_param(param_file, mod, "TEMPERATURE", CS%use_temperature, &
                 ...
                 "variables.")
  call log_param(param_file, mod, "DT", CS%dt, &
                 "The (baroclinic) dynamics time step.", units="s")
  call log_param(param_file, mod, "MAXTRUNC", CS%maxtrunc, &
  ...
  call log_param(param_file, mod, "MAX_ENERGY", CS%max_Energy, &
  ...
  if (CS%max_Energy <= 0.0) &
    call log_param(param_file, mod, "MAXVEL", maxvel, &
  ...
  call log_param(param_file, mod, "ENERGYFILE", energyfile, &
                 "The file to use to write the energies and globally \n"//&
                 "summed diagnostics.",default="timestats")
  call log_param(param_file, mod, "NORMALIZED_SUM_OUT",CS%norm, &
                 "If true, normalize the summed diagnostic output values \n"//&
                 "by the totals", default=.true.)
  call log_param(param_file, mod, "TIMEUNIT", CS%Timeunit, &
                 "The time unit in seconds a number of input fields", units="s")
  if (CS%use_temperature) then
    call log_param(param_file, mod, "C_P", CS%C_p, &
                 "The heat capacity of sea water, approximated as a \n"//&
                 "constant.", units="J kg-1 K-1")
  endif
  if (CS%do_APE_calc) then
    call log_param(param_file, mod, "READ_DEPTH_LIST", CS%read_depth_list, &
                   "Read the depth list from a file if it exists or \n"//&
                   "create that file otherwise.", default=.false.)
    call log_param(param_file, mod, "DEPTH_LIST_MIN_INC", CS%D_list_min_inc, &
                   "The minimum increment between the depths of the \n"//&
                   "entries in the depth-list file.", units="m", &
                   default=G%Angstrom_Z)
    if (CS%read_depth_list) &
      call log_param(param_file, mod, "DEPTH_LIST_FILE", CS%depth_list_file, &
                   "The name of the depth list file.", default="Depth_list.nc")
  endif

end subroutine GOLD_sum_output_init

subroutine GOLD_sum_output_end(CS)
  type(Sum_output_CS), pointer :: CS
...
end subroutine GOLD_sum_output_end
...
!                 GOLD_sum_output_init.
 ...
 ! A description for output of each of the fields.
  ...
  if (G%Boussinesq) then
    if (CS%use_repro_sum) then
      ...
      do k=1,nz ; do j=js,je ; do i=is,ie
        tmp1(i,j,k) = h(i,j,k) * G%DXDYh(i,j)
      enddo ; enddo ; enddo
    ...
    if (CS%use_repro_sum) then
      ...
      if (CS%do_APE_calc) then
        ...
        enddo ; enddo ; enddo
        mass_tot = reproducing_sum(tmp1, sums=mass_lay)
  ...
  if (CS%previous_calls == 0) then
    CS%mass_prev = 0.0 ; CS%fresh_water_input = 0.0
    do k=1,nz ; CS%mass_prev = CS%mass_prev + mass_lay(k) ; enddo
...
! mixed layer this calculation is only approximate.
  ...
  if (CS%use_temperature) then
    if (CS%use_repro_sum) then
      ...
      do k=1,nz ; do j=js,je ; do i=is,ie
        Salt_int(i,j) = Salt_int(i,j) +  tv%S(i,j,k)*(h(i,j,k)*G%DXDYh(i,j))
        Temp_int(i,j) = Temp_int(i,j) +  tv%T(i,j,k)*(h(i,j,k)*G%DXDYh(i,j))
      enddo ; enddo ; enddo
      Salt = G%H_to_kg_m2 * reproducing_sum(Salt_int)
      Heat = (G%H_to_kg_m2 * CS%C_p) * reproducing_sum(Temp_int)
    else
      do k=1,nz ; do j=js,je ; do i=is,ie
...
! decomposition.
  ...
  if (CS%use_temperature) then
    temp = heat / (mass_tot*CS%C_p) ; temp_anom = Heat_anom / (mass_tot*CS%C_p)
  endif
  ...
  if (is_root_pe()) then
    ...
    if (CS%ntrunc > 0) then
      if (reday .lt. 1.e5) then
        write(*,'(A,F9.3," Energy/Mass:",ES12.5," Truncations ",I)') &
          trim(mesg_intro), reday, En_mass, CS%ntrunc
      else
        write(*,'(A,F15.3," Energy/Mass:",ES12.5," Truncations ",I)') &
          trim(mesg_intro), reday, En_mass, CS%ntrunc
      endif
    endif
  ...
  if ((En_mass>CS%max_Energy) .or. &
    ...
    call GOLD_error(WARNING, "write_energy : "//trim(mesg))
    call GOLD_error(WARNING, &
      "write_energy : Time set to a large value to force model termination.")
  endif
  if (CS%ntrunc>CS%maxtrunc) then
    day = CS%Huge_time
    call GOLD_error(FATAL, "write_energy : Ocean velocity has been truncated too many times.")
  endif
  CS%ntrunc = 0
...
!                 GOLD_sum_output_init.
  ...
  if (CS%use_temperature) then
    ...
    if (ASSOCIATED(state%TempxPmE)) then
      do j=js,je ; do i=is,ie
        heat_in(i,j) = heat_in(i,j) + (CS%C_p * G%DXDYh(i,j)) * state%TempxPmE(i,j)
      enddo ; enddo
    elseif (ASSOCIATED(fluxes%evap)) then
      do j=js,je ; do i=is,ie
        heat_in(i,j) = heat_in(i,j) + (CS%C_p * state%SST(i,j)) * FW_in(i,j)
      enddo ; enddo
    endif
    ...
    if (ASSOCIATED(state%internal_heat)) then
      do j=js,je ; do i=is,ie
        heat_in(i,j) = heat_in(i,j) + (CS%C_p * G%DXDYh(i,j)) * &
...
!    enddo ; enddo ; endif
  ...
  if ((CS%use_temperature) .or. ASSOCIATED(fluxes%liq_precip) .or. &
    ...
    if (CS%use_repro_sum) then
      FW_input = reproducing_sum(FW_in)
      heat_input = reproducing_sum(heat_in)
      salt_input = reproducing_sum(salt_in)
    else
...
! or it might be created anew.  (For now only new creation occurs.
  ...
  if (CS%read_depth_list) then
    if (file_exists(CS%depth_list_file)) then
      ...
      if (is_root_pe()) call GOLD_error(WARNING, "depth_list_setup: "// &
...
! than each depth.
! Arguments: G - The ocean's grid structure.
!  (in)      CS - The control structure set up in GOLD_sum_output_init, in
...
! This subroutine writes out the depth list to the specified file.
  ...
  if (status /= NF90_NOERR) then
    call GOLD_error(WARNING, filename//trim(NF90_STRERROR(status)))
    return
  endif
  
  status = NF90_DEF_DIM(ncid, "list", list_size, dimid(1))
  if (status /= NF90_NOERR) call GOLD_error(WARNING, &
...
! and allocates and sets up CS%DL and CS%list_size .
  ...
  mod = "GOLD_sum_output read_depth_list:"

  status = NF90_OPEN(filename, NF90_NOWRITE, ncid);
  if (status /= NF90_NOERR) then
    call GOLD_error(FATAL,mod//" Difficulties opening "//trim(filename)// &
  ...
  if (status /= NF90_NOERR) call GOLD_error(FATAL,mod// &
        " Difficulties finding variable "//trim(var_msg)//&
        trim(NF90_STRERROR(status)))

  status = NF90_INQUIRE_VARIABLE(ncid, varid, ndims=ndim, dimids=var_dim_ids)
  if (status /= NF90_NOERR) then
    call GOLD_ERROR(FATAL,mod//" cannot inquire about "//trim(var_msg)//&
        trim(NF90_STRERROR(status)))
  elseif (ndim > 1) then
    call GOLD_ERROR(FATAL,mod//" "//trim(var_msg)//&
  ...
  status = NF90_CLOSE(ncid)
  if (status /= NF90_NOERR) call GOLD_error(WARNING, mod// &
