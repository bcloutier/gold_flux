...
! the M equations.
  ...
  real, pointer, dimension(:,:,:) :: T, S, Rml
  ...
  if (.not. associated(CS)) call GOLD_error(FATAL, "GOLD_wave_speed: "// &
         "Module must be initialized before it is used.")

  Rml => tv%Rml ; S => tv%S ; T => tv%T
    ...
    elseif (CS%bulkmixedlayer) then
      do k=1,tv%nk_Rml ; do i=is,ie
        if ((H_here(i) > hmin(i)) .and. (h(i,j,k)*G%H_to_m > hmin(i))) then
          ...
          ! Start a new layer
          H_here(i) = h(i,j,k)*G%H_to_m
          HxR_here(i) = (h(i,j,k)*G%H_to_m)*Rml(i,j,k)
        else
          H_here(i) = H_here(i) + h(i,j,k)*G%H_to_m
          HxR_here(i) = HxR_here(i) + (h(i,j,k)*G%H_to_m)*Rml(i,j,k)
        endif
      enddo ; enddo
      do k=1,nz ; do i=is,ie
        if ((H_here(i) > hmin(i)) .and. (h(i,j,k)*G%H_to_m > hmin(i))) then
...
!                  for this module
  character(len=128) :: version = '$Id: GOLD_wave_speed.F90,v 1.1.2.12 2010/09/29 18:22:47 rwh Exp $'
  character(len=128) :: tagname = '$Name: GOLD_ogrp $'
  character(len=40)  :: mod = "GOLD_wave_speed"  ! This module's name.
  integer :: nkml, nkbl

  if (associated(CS)) then
    call GOLD_error(WARNING, "wave_speed_init called with an "// &
