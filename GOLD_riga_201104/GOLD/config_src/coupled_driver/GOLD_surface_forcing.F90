...
use GOLD_coms, only : reproducing_sum
use GOLD_cpu_clock, only : cpu_clock_id, cpu_clock_begin, cpu_clock_end
use GOLD_cpu_clock, only : CLOCK_SUBCOMPONENT
...
use GOLD_domains, only : pass_vector, pass_var, global_field_sum, BITWISE_EXACT_SUM
use GOLD_error_handler, only : GOLD_error, WARNING, FATAL, is_root_pe, GOLD_mesg
...
type, public :: surface_forcing_CS ; private
  ...
  logical :: slow_bitwise_sums ! If true, all the global sums in this module
                             ! are bitwise identical across PE count, but may be
                             ! much slower on some machines.  The default is
                             ! to use the reproducing integer-conversion sums 
                             ! of Hallberg and Adcroft, which is much faster
...
!  (in)      restore_salt - if true, salinity is restored to a target value.
  ...
  if (CS%first_call) then
    ...
    enddo ; enddo
    if (CS%slow_bitwise_sums) then
      CS%area_surf = global_field_sum(G%Domain%mpp_domain, work_sum, BITWISE_EXACT_SUM)
    else
  ...
  if (restore_salinity) then
      ...
      if (CS%salt_restore_as_sflux) then
        do j=js,je ; do i=is,ie
          ...
          fluxes%salt_flux(i,j) = 1.e-3*G%hmask(i,j)*CS%Rho0*CS%Flux_const*CS%basin_mask(i,j)*open_ocn_mask(i,j)*delta_sss  ! kg Salt m-2 s-1
          work_sum(i,j) = G%DXDYh(i,j)*fluxes%salt_flux(i,j)
        enddo; enddo
        if (CS%slow_bitwise_sums) then
          Sflux_adj_total = global_field_sum(G%Domain%mpp_domain,work_sum(:,:),BITWISE_EXACT_SUM)/CS%area_surf
        else
        ...
        do j=js,je ; do i=is,ie
          if (G%hmask(i,j) > 0.5) then
            ...
            delta_sss = sign(1.0,delta_sss)*min(abs(delta_sss),CS%max_delta_srestore)
            pme_adj(i,j) = CS%basin_mask(i,j)*open_ocn_mask(i,j)*(CS%Rho0*CS%Flux_const) * &
        ...
        if (CS%slow_bitwise_sums) then
          PmE_adj_total = global_field_sum(G%Domain%mpp_domain,work_sum(:,:), &
                                           BITWISE_EXACT_SUM) / CS%area_surf
        else
...
!### endif
  ...
  if (CS%adjust_net_fresh_water_to_zero) then
    ...
    if (CS%slow_bitwise_sums) then
      net_FW_avg = global_field_sum(G%Domain%mpp_domain, net_FW(:,:), &
                                  BITWISE_EXACT_SUM) / CS%area_surf
    else
...
!                           applied in this model.
  ...
  character(len=128) :: version = '$Id: GOLD_surface_forcing.F90,v 13.0.2.5.2.36 2011/09/19 16:10:34 Robert.Hallberg Exp $'
  ...
  CS%max_p_surf = -1.0 ; CS%use_limited_P_SSH = .true.
  call read_param(param_file,"MAX_P_SURF",CS%max_p_surf)
  call read_param(param_file,"USE_LIMITED_PATM_SSH",CS%use_limited_P_SSH)
  CS%adjust_net_fresh_water_to_zero=.false.
  call read_param(param_file,"ADJUST_NET_FRESH_WATER_TO_ZERO",CS%adjust_net_fresh_water_to_zero)
  CS%slow_bitwise_sums = .false.
  call read_param(param_file, "SLOW_BITWISE_GLOBAL_FORCING_SUMS", CS%slow_bitwise_sums)
  CS%ice_salt_concentration = 0.005
  call read_param(param_file,"ICE_SALT_CONCENTRATION",CS%ice_salt_concentration)
  stagger = 'C' ; CS%wind_stagger = C_GRID
  call read_param(param_file,"WIND_STAGGER",stagger)
  if ((stagger(1:1) == 'a') .or. (stagger(1:1) == 'A')) then
    CS%wind_stagger = A_GRID
    call GOLD_error(FATAL,"surface_forcing_init: A-grid input wind stagger "// &
  ...
  elseif ((stagger(1:1) == 'c') .or. (stagger(1:1) == 'C')) then
    ...
    call GOLD_error(FATAL,"surface_forcing_init: #define WIND_STAGGER "// &
  ...
  if (restore_salt) then
    ...
    call read_param(param_file,"MAX_DELTA_SRESTORE",CS%max_delta_srestore)      
! Convert CS%Flux_const from m day-1 to m s-1.
    CS%Flux_const = CS%Flux_const / 86400.0
    CS%salt_restore_as_sflux = .false.
    call read_param(param_file,"SRESTORE_AS_SFLUX",CS%salt_restore_as_sflux,.false.)
    CS%mask_srestore_under_ice = .false.
    call read_param(param_file,"MASK_SRESTORE_UNDER_ICE",CS%mask_srestore_under_ice,.false.)
    CS%mask_srestore_marginal_seas = .false.
    call read_param(param_file,"MASK_SRESTORE_MARGINAL_SEAS",CS%mask_srestore_marginal_seas,.false.)
    basin_file='basin.nc'
    call read_param(param_file,"BASIN_FILE",basin_file)      
...
! work done against tides globally using OSU tidal amplitude.
  CS%read_TIDEAMP = .false.
  call read_param(param_file,"READ_TIDEAMP",CS%read_TIDEAMP)
  TideAmp_file='tideamp.nc'
  call read_param(param_file,"TIDEAMP_FILE",TideAmp_file)  
  CS%cd_tides=1.e-4
  call read_param(param_file,"CD_TIDES",CS%cd_tides)
  CS%utide=0.0
  call read_param(param_file,"UTIDE",CS%utide)
...
! See whether sufficiently thick sea ice should be treated as rigid.
  CS%rigid_sea_ice = .false.
  CS%density_sea_ice = 900. ; CS%Kv_sea_ice = 1e9
  CS%rigid_sea_ice_mass = 1000.0
  call read_param(param_file,"USE_RIGID_SEA_ICE",CS%rigid_sea_ice)
  if (CS%rigid_sea_ice) then
    call read_param(param_file,"SEA_ICE_MEAN_DENSITY",CS%density_sea_ice)
    call read_param(param_file,"SEA_ICE_VISCOSITY",CS%Kv_sea_ice)
    call read_param(param_file,"SEA_ICE_RIGID_MASS",CS%rigid_sea_ice_mass)
  endif

  mod = "GOLD_surface_forcing"
  ...
  call log_param(param_file, mod, "RHO_0", CS%Rho0, &
                 ...
                 units="kg m-3")
  call log_param(param_file, mod, "MAX_P_SURF", CS%max_p_surf, &
                 ...
                 default=-1.0)
  call log_param(param_file, mod, "ADJUST_NET_FRESH_WATER_TO_ZERO", &
  ...
  call log_param(param_file, mod, "SLOW_BITWISE_GLOBAL_FORCING_SUMS", CS%slow_bitwise_sums, &
                 "If true, use global sums that reproduce across PE count \n"//&
                 "but will be slow for large PE counts.  The alterative \n"//&
                 "uses the integer-conversion sums of Hallberg and Adcroft \n"//&
                 "which is much faster and equally accurate.", default=.false.)
  call log_param(param_file, mod, "ICE_SALT_CONCENTRATION", &
  ...
  call log_param(param_file, mod, "USE_LIMITED_PATM_SSH", CS%use_limited_P_SSH, &
  ...
  call log_param(param_file, mod, "WIND_STAGGER", stagger, &
  ...
  if (restore_salt) then
    ...
    call log_param(param_file, mod, "SRESTORE_AS_SFLUX", CS%salt_restore_as_sflux)
    call log_param(param_file, mod, "MAX_DELTA_SRESTORE", CS%max_delta_srestore)
    call log_param(param_file, mod, "MASK_SRESTORE_UNDER_ICE", CS%mask_srestore_under_ice)
    call log_param(param_file, mod, "MASK_SRESTORE_MARGINAL_SEAS", CS%mask_srestore_marginal_seas)
    call log_param(param_file, mod, "BASIN_FILE", basin_file)
  endif

  call log_param(param_file, mod, "CD_TIDES", CS%cd_tides, &
                 "The drag coefficient that applies to the tides.", &
                 units="nondim")
  call log_param(param_file, mod, "READ_TIDEAMP", CS%read_TIDEAMP, &
                 "If true, read a file (given by TIDEAMP_FILE) containing \n"//&
                 "the tidal amplitude with INT_TIDE_DISSIPATION.", default=.false.)
  if (CS%read_TIDEAMP) then
    call log_param(param_file, mod, "TIDEAMP_FILE", TideAmp_file, &
  ...
  if (CS%rigid_sea_ice) then
    ...
    call log_param(param_file, mod, "SEA_ICE_RIGID_MASS", CS%rigid_sea_ice_mass, &
