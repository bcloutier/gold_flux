...
!                              flow control module.
  ...
  character(len=128) :: version = '$Id: GOLD_surface_forcing.F90,v 13.0.2.10.2.28 2011/10/07 22:36:34 Robert.Hallberg Exp $'
  ...
  if (trim(CS%wind_config) == "file") then
    ...
    CS%stress_x_var = "STRESS_X" ; CS%stress_y_var = "STRESS_Y"
    call read_param(param_file,"WINDSTRESS_X_VAR",CS%stress_x_var)
    call read_param(param_file,"WINDSTRESS_Y_VAR",CS%stress_y_var)
    CS%wind_stagger = "A" ; call read_param(param_file,"WINDSTRESS_STAGGER",CS%wind_stagger)
    CS%wind_scale = 1.0 ; call read_param(param_file, "WINDSTRESS_SCALE", CS%wind_scale)
  endif
  ...
  if (trim(CS%wind_config) == "file") then
    ...
    call log_param(param_file, mod, "WINDSTRESS_X_VAR",CS%stress_x_var, &
                 "The name of the x-wind stress variable in WIND_FILE.", &
                 default="STRESS_X")
    call log_param(param_file, mod, "WINDSTRESS_Y_VAR", CS%stress_y_var, &
                 "The name of the y-wind stress variable in WIND_FILE.", &
                 default="STRESS_Y")
    call log_param(param_file, mod, "WINDSTRESS_STAGGER",CS%wind_stagger, &
                 "A character indicating how the wind stress components \n"//&
                 "are staggered in WIND_FILE.  This may be A or C for now.", &
                 default="A")
    call log_param(param_file, mod, "WINDSTRESS_SCALE", CS%wind_scale, &
