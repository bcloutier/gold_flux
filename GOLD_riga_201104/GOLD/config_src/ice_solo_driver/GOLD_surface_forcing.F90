...
public set_forcing, surface_forcing_init, average_forcing
...
type, public :: surface_forcing_CS ; private
  ...
  real :: T_north, T_south   ! Target temperatures at north and south used in buoyancy_forcing_linear
  real :: S_north, S_south   ! Target salinity at north and south used in buoyancy_forcing_linear
  ...
  if ((CS%variable_buoyforce .or. CS%first_call_set_forcing) .and. &
    ...
    elseif (CS%variable_buoyforce .and. .not.CS%first_call_set_forcing) then
      call GOLD_error(FATAL, &
       ...
       call GOLD_error(FATAL, &
       "GOLD_surface_forcing: Unrecognized bouy config "//trim(CS%buoy_config))
    endif
  endif
...
!                 call to surface_forcing_init.
  ...
  if (CS%bulkmixedlayer .and. .not.associated(fluxes%ustar)) then
...
!   Set fiction velocities when using bulk mixed layer
...
!           time_lev-1,CS%wind_last_lev_read-1
    call read_data(filename,"STRESS_X",temp_x(:,:),domain=G%Domain%mpp_domain,timelevel=time_lev)
    call read_data(filename,"STRESS_Y",temp_y(:,:),domain=G%Domain%mpp_domain,timelevel=time_lev)
...
!    Latent heat of vaporization at 15 deg C according to appendix of Gill
  ...
  if (CS%use_temperature) rhoXcp = CS%Rho0 * CS%C_p
  Irho0 = 1.0/CS%Rho0
...
!   Mask out land points.
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then
      ...
      do j=js,je ; do i=is,ie
        if (G%hmask(i,j) > 0) then
          fluxes%buoy(i,j) = (CS%Dens_Restore(i,j) - state%Rml(i,j)) * &
    ...
    if (.not.CS%use_temperature) then
      call GOLD_error(FATAL, "bouyancy_forcing in GOLD_surface_forcing: "// &
  ...
  if (CS%restorebuoy) then
    if (CS%use_temperature) then
      do j=js,je ; do i=is,ie
        ...
        if (G%hmask(i,j) > 0) then
          fluxes%heat_restore(i,j) = G%hmask(i,j) * &
              ((T_Restore - state%SST(i,j)) * ((CS%Rho0 * CS%C_p) * CS%Flux_const))
          fluxes%virt_precip(i,j) = - (CS%Rho0*CS%Flux_const) * &
      ...
      call GOLD_error(FATAL, "bouyancy_forcing_linear in GOLD_surface_forcing: "// &
...
subroutine register_forcing_restarts(CS, restart_CS)
  type(surface_forcing_CS), pointer :: CS
  type(GOLD_restart_CS),     pointer :: restart_CS
! Arguments: CS - A pointer to the control structure returned by a previous
!                 call to surface_forcing_init.
!  (in)      restart_CS - A pointer to the restart control structure.
! This subroutine is used to register any fields in GOLD_surface_forcing.F90
! that should be written to or read from the restart file.

  ! This subroutine is deliberately empty.

!  type(vardesc) :: vdH
!  vdH = vardesc("Heat","Low Frequency Heating",'h','1','s',"W m-2", 'd')

!  call register_restart_field(CS%Mean_heat, CS%Mean_heat, vdH, &
!                              .true., restart_CS)

end subroutine register_forcing_restarts
...
!                              flow control module.
  character(len=128) :: version = '$Id: GOLD_surface_forcing.F90,v 1.1.2.1 2011/07/27 21:40:10 Robert.Hallberg Exp $'
  ...
  CS%adiabatic = .false.
  CS%variable_winds = .true. ; CS%variable_buoyforce = .true.
  CS%inputdir = "." ;  call read_param(param_file,"INPUTDIR",CS%inputdir)
  CS%inputdir = slasher(CS%inputdir)
  call read_param(param_file,"ADIABATIC",CS%adiabatic)
  call read_param(param_file,"VARIABLE_WINDS",CS%variable_winds)
  call read_param(param_file,"VARIABLE_BUOYFORCE",CS%variable_buoyforce)
  call read_param(param_file,"BUOY_CONFIG",CS%buoy_config)
  if (trim(CS%buoy_config) == "file") then
    call read_param(param_file,"LONGWAVEDOWN_FILE",CS%longwavedown_file)
    call read_param(param_file,"LONGWAVEUP_FILE",CS%longwaveup_file)
    call read_param(param_file,"EVAPORATION_FILE",CS%evaporation_file)
    call read_param(param_file,"SENSIBLEHEAT_FILE",CS%sensibleheat_file)
    call read_param(param_file,"SHORTWAVEUP_FILE",CS%shortwaveup_file)
    call read_param(param_file,"SHORTWAVEDOWN_FILE",CS%shortwavedown_file)
    call read_param(param_file,"SNOW_FILE",CS%snow_file)
    call read_param(param_file,"PRECIP_FILE",CS%precip_file)
    call read_param(param_file,"FRESHDISCHARGE_FILE",CS%freshdischarge_file)
    call read_param(param_file,"SSTRESTORE_FILE",CS%SSTrestore_file)
    call read_param(param_file,"SALINITYRESTORE_FILE",CS%salinityrestore_file)
  endif
  call read_param(param_file,"WIND_CONFIG",CS%wind_config)
  if (trim(CS%wind_config) == "file") &
    call read_param(param_file,"WIND_FILE",CS%wind_file)
  if (trim(CS%wind_config) == "gyres") then
    CS%gyres_taux_const=0.; call read_param(param_file,"TAUX_CONST",CS%gyres_taux_const)
    CS%gyres_taux_sin_amp=0.; call read_param(param_file,"TAUX_SIN_AMP",CS%gyres_taux_sin_amp)
    CS%gyres_taux_cos_amp=0.; call read_param(param_file,"TAUX_COS_AMP",CS%gyres_taux_cos_amp)
    CS%gyres_taux_n_pis=0.; call read_param(param_file,"TAUX_N_PIS",CS%gyres_taux_n_pis)
  endif
  CS%south_lat = 0.0 ; call read_param(param_file,"SOUTHLAT",CS%south_lat)
  call read_param(param_file,"LENLAT",CS%len_lat,.true.)
  call read_param(param_file,"RHO_0",CS%Rho0,.true.)
  if (CS%use_temperature) call read_param(param_file,"C_P",CS%C_p,.true.)
  CS%restorebuoy = .false.
  ...
  if (CS%restorebuoy) then
    ...
    if (trim(CS%buoy_config) == "linear") then
      CS%T_north=0.; call read_param(param_file,"SST_NORTH",CS%T_north)
      CS%T_south=0.; call read_param(param_file,"SST_SOUTH",CS%T_south)
      CS%S_north=35.; call read_param(param_file,"SSS_NORTH",CS%S_north)
      CS%S_south=35.; call read_param(param_file,"SSS_SOUTH",CS%S_south)
    endif
  endif
  ...
  CS%read_gust_2d = .false.
  call read_param(param_file,"READ_GUST_2D",CS%read_gust_2d)
  if (CS%read_gust_2d) then
    call read_param(param_file,"GUST_2D_FILE",gust_file)
  ...
  call log_param(param_file, mod, "INPUTDIR", CS%inputdir, &
                 "The directory in which all input files are found.", &
                 default=".")
  call log_param(param_file, mod, "ADIABATIC", CS%adiabatic, &
                 "There are no diapycnal mass fluxes if ADIABATIC is \n"//&
                 "true. This assumes that KD = KDML = 0.0 and that \n"//&
                 "there is no buoyancy forcing, but makes the model \n"//&
                 "faster by eliminating subroutine calls.", default=.false.)
  call log_param(param_file, mod, "VARIABLE_WINDS", CS%variable_winds, &
                 "If true, the winds vary in time after the initialization.", &
                 default=.true.)
  call log_param(param_file, mod, "VARIABLE_BUOYFORCE", CS%variable_buoyforce, &
                 "If true, the buoyancy forcing varies in time after the \n"//&
                 "initialization of the model.", default=.true.)
  call log_param(param_file, mod, "BUOY_CONFIG", CS%buoy_config, &
                 "The character string that indicates how buoyancy forcing \n"//&
                 "is specified. Valid options include (file), (zero), \n"//&
                 "(linear), (USER), and (NONE).")
  if (trim(CS%buoy_config) == "file") then
    call log_param(param_file, mod, "LONGWAVEDOWN_FILE", CS%longwavedown_file, &
                 "The file with the downward longwave heat flux, in \n"//&
                 "variable lwdn_sfc.")
    call log_param(param_file, mod, "LONGWAVEUP_FILE", CS%longwaveup_file, &
                 "The file with the upward longwave heat flux, in variable \n"//&
                 "lwup_sfc.")
    call log_param(param_file, mod, "EVAPORATION_FILE", CS%evaporation_file, &
                 "The file with the evaporative moisture flux, in variable evap.")
    call log_param(param_file, mod, "SENSIBLEHEAT_FILE", CS%sensibleheat_file, &
                 "The file with the sensible heat flux, in variable shflx.")
    call log_param(param_file, mod, "SHORTWAVEUP_FILE", CS%shortwaveup_file, &
                 "The file with the upward shortwave heat flux.")
    call log_param(param_file, mod, "SHORTWAVEDOWN_FILE", CS%shortwavedown_file, &
                 "The file with the downward shortwave heat flux.")
    call log_param(param_file, mod, "SNOW_FILE", CS%snow_file, &
                 "The file with the downward frozen precip flux, in variable snow.")
    call log_param(param_file, mod, "PRECIP_FILE", CS%precip_file, &
                 "The file with the downward total precip flux, in variable precip.")
    call log_param(param_file, mod, "FRESHDISCHARGE_FILE", CS%freshdischarge_file, &
                 "The file with the fresh and frozen runoff/calving fluxes, \n"//&
                 "variables disch_w and disch_s.")
    call log_param(param_file, mod, "SSTRESTORE_FILE", CS%SSTrestore_file, &
                 "The file with the SST toward which to restore in variable TEMP.")
    call log_param(param_file, mod, "SALINITYRESTORE_FILE", CS%salinityrestore_file, &
                 "The file with the surface salinity toward which to \n"//&
                 "restore in variable SALT.")
  endif
  call log_param(param_file, mod, "WIND_CONFIG", CS%wind_config, &
  ...
  if (trim(CS%wind_config) == "file") &
    call log_param(param_file, mod, "WIND_FILE", CS%wind_file, &
                 "The file in which the wind stresses are found in \n"//&
                 "variables STRESS_X and STRESS_Y.")
  if (trim(CS%wind_config) == "gyres") then
    call log_param(param_file, mod, "TAUX_CONST", CS%gyres_taux_const, &
  ...
  if (CS%restorebuoy) then
    call log_param(param_file, mod, "FLUXCONST", CS%Flux_const, &
                 "The constant that relates the restoring surface fluxes \n"//&
                 "to the relative surface anomalies (akin to a piston \n"//&
                 "velocity).  Note the non-MKS units.", units="m day-1")
    if (trim(CS%buoy_config) == "linear") then
      call log_param(param_file, mod, "SST_NORTH", CS%T_north, &
      ...
      call log_param(param_file, mod, "SST_SOUTH", CS%T_south, &
                 "With buoy_config linear, the sea surface temperature \n"//&
                 "at the southern end of the domain toward which to \n"//&
                 "to restore.", units="deg C", default=0.0)
      call log_param(param_file, mod, "SSS_NORTH", CS%S_north, &
                 "With buoy_config linear, the sea surface salinity \n"//&
                 "at the northern end of the domain toward which to \n"//&
                 "to restore.", units="PSU", default=35.0)
      call log_param(param_file, mod, "SSS_SOUTH", CS%S_south, &
                 "With buoy_config linear, the sea surface salinity \n"//&
                 "at the southern end of the domain toward which to \n"//&
                 "to restore.", units="PSU", default=35.0)
    endif
  ...
  call log_param(param_file, mod, "GUST_CONST", CS%gust_const, &
                 ...
                 default=0.02)
  call log_param(param_file, mod, "READ_GUST_2D", CS%read_gust_2d, &
                 "If true, use a 2-dimensional gustiness supplied from \n"//&
                 "an input file", default=.false.)
  if (CS%read_gust_2d) &
    call log_param(param_file, mod, "GUST_2D_FILE", gust_file, &
                 "The file in which the wind gustiness is found in \n"//&
                 "variable gustiness.")

!  All parameter settings are now known.

  if (trim(CS%wind_config) == "USER" .or. trim(CS%buoy_config) == "USER" ) &
